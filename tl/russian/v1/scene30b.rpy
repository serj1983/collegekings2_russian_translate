# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene30b.rpy:17
translate russian v1s30b_a7785a95:

    # cl "Alright... Let's get a good look at these."
    cl "Хорошо... Давай рассмотрим их как следует."

# game/v1/scene30b.rpy:22
translate russian v1s30b_68d6c17a:

    # u "Do you like what you see so far?"
    u "Тебе нравится то, что ты видишь?"

# game/v1/scene30b.rpy:27
translate russian v1s30b_ff795ce1:

    # cl "I do. I also think this helps the campaign a lot."
    cl "Да. Я также думаю, что это очень поможет моей кампании."

# game/v1/scene30b.rpy:29
translate russian v1s30b_3ea8c1f7:

    # cl "Of the two frats, the Wolves are way more respected than the Apes and so..."
    cl "Из двух братств Волки пользуются большим уважением, чем Приматы, и поэтому..."

# game/v1/scene30b.rpy:31
translate russian v1s30b_1b8ebf1f:

    # cl "Just strategically speaking, this helps a lot."
    cl "В стратегическом плане это очень поможет."

# game/v1/scene30b.rpy:36
translate russian v1s30b_022fae55:

    # u "That's what I like to hear."
    u "Это то, что мне нравится слышать."

# game/v1/scene30b.rpy:41
translate russian v1s30b_49a1fdb7:

    # cl "My posing definitely isn't as good as Aubrey's though."
    cl "Хотя моё позирование не такое хорошее, как у Обри."

# game/v1/scene30b.rpy:46
translate russian v1s30b_689f11ca:

    # u "So you know about Lew's and all that?"
    u "Так ты знаешь про Лью?"

# game/v1/scene30b.rpy:51
translate russian v1s30b_bc8e8300:

    # cl "Haha, I know more about Aubrey than most."
    cl "Хаха, я знаю об Обри больше, чем многие."

# game/v1/scene30b.rpy:56
translate russian v1s30b_7f86fb53:

    # u "She is your VP..."
    u "Она - твой вице-президент..."

# game/v1/scene30b.rpy:61
translate russian v1s30b_6745bec8:

    # cl "Exactly, and hopefully she chooses to stay on after we win."
    cl "Именно, и надеюсь, что она решит остаться после нашей победы."

# game/v1/scene30b.rpy:66
translate russian v1s30b_88b1e04f:

    # u "I'm sure she will."
    u "Уверен, что так и будет."

# game/v1/scene30b.rpy:71
translate russian v1s30b_091961ec:

    # cl "Ha, glad you think so. Now..."
    cl "Ха, рада, что ты так думаешь. Теперь..."

# game/v1/scene30b.rpy:74
translate russian v1s30b_23ee5808:

    # cl "I'm not sure how these will be perceived and all you know? With us having a fake wolf, but I still think the pictures are really good."
    cl "Не уверена, как они будут восприняты и всё такое? Учитывая, что у нас есть фальшивый волк, но я все равно думаю, что фотографии очень хорошие."

# game/v1/scene30b.rpy:84
translate russian v1s30b_116f61c9:

    # cl "Which one is your favorite?"
    cl "Какая из них тебе нравится?"

# game/v1/scene30b.rpy:99
translate russian v1s30b_632b2440:

    # u "That one."
    u "Вот эта."

# game/v1/scene30b.rpy:104
translate russian v1s30b_bb197012:

    # cl "Good call. I'm gonna post that one on Kiwii then."
    cl "Хорошее решение. Тогда размещу её на Kiwii."

# game/v1/scene30b.rpy:107
translate russian v1s30b_d53f265e:

    # cl "I know the entire school is gonna be off the wall about these photos, haha."
    cl "Знаю, вся школа будет вне себя от радости из-за этой фотографии, хаха."

# game/v1/scene30b.rpy:117
translate russian v1s30b_116f61c9_1:

    # cl "Which one is your favorite?"
    cl "Какая из них тебе нравится?"

# game/v1/scene30b.rpy:132
translate russian v1s30b_632b2440_1:

    # u "That one."
    u "Вот эта."

# game/v1/scene30b.rpy:137
translate russian v1s30b_bb197012_1:

    # cl "Good call. I'm gonna post that one on Kiwii then."
    cl "Хорошее решение. Тогда размещу её на Kiwii."

# game/v1/scene30b.rpy:142
translate russian v1s30b_4c31b52b:

    # u "What about your caption?"
    u "А как насчёт подписи?"

# game/v1/scene30b.rpy:147
translate russian v1s30b_30efcb12:

    # cl "Well, I'll definitely include \"#PresidentialStatus\". Then something about voting for me."
    cl "Что ж, я обязательно включу \"#PresidentialStatus\". Потом что-то насчет голосования за меня."

# game/v1/scene30b.rpy:157
translate russian v1s30b_292be443:

    # u "Yeah, that's perfect. Standing tall and proud with a short and sweet caption, the main feature is you."
    u "Да, это идеально. Высокая и гордая, с короткой и милой надписью, и главная - это ты."

# game/v1/scene30b.rpy:162
translate russian v1s30b_fb64509d:

    # cl "*Chuckles* Exactly."
    cl "*Смешок* Точно."

# game/v1/scene30b.rpy:169
translate russian v1s30b_d08d5928:

    # u "What about something else?"
    u "А как насчет чего-то другого?"

# game/v1/scene30b.rpy:174
translate russian v1s30b_a09bcf6f:

    # cl "Something else?"
    cl "Что-то другое?"

# game/v1/scene30b.rpy:179
translate russian v1s30b_3d605b0a:

    # u "Well, yeah. Like something about the Chicks and the Wolves, so it feel like a team effort?"
    u "Ну, да. Например, что-нибудь про Чикс и Волках, чтобы было ощущение командной работы?"

# game/v1/scene30b.rpy:184
translate russian v1s30b_271fb9bf:

    # cl "I guess that makes sense considering it's an alliance announcement, as well as a good pic for my feed, haha."
    cl "Думаю, это имеет смысл, учитывая, что это объявление альянса, а также хорошая картинка для моей ленты, хаха."

# game/v1/scene30b.rpy:189
translate russian v1s30b_42beb98c:

    # u "Exactly, so maybe something like, \"#TheChicksWhoCriedWolves\"."
    u "Именно, так что может быть что-то вроде: \"#TheChicksWhoCriedWolves\"."

# game/v1/scene30b.rpy:194
translate russian v1s30b_d068caf6:

    # cl "You're right! That's perfect, [name]."
    cl "Ты прав! Это идеально, [name]."

# game/v1/scene30b.rpy:199
translate russian v1s30b_55fa0bf7:

    # u "*Laughs* You're welcome."
    u "*Смеётся* Не за что."

# game/v1/scene30b.rpy:204
translate russian v1s30b_99c453ab:

    # cl "Well, this has been a lot better than I had imagined. I'm off to get it all set in stone."
    cl "Это оказалось намного лучше, чем я могла себе представить. Я ухожу, чтобы всё оформить."

# game/v1/scene30b.rpy:206
translate russian v1s30b_81d681f2:

    # cl "Thanks again for everything."
    cl "Ещё раз спасибо за всё."

# game/v1/scene30b.rpy:211
translate russian v1s30b_f7fa0177:

    # u "Of course! I'll be on the lookout for your big announcement."
    u "Конечно! Я буду ждать твоего громкого объявления."

# game/v1/scene30b.rpy:216
translate russian v1s30b_108b353e:

    # cl "*Chuckles*"
    cl "*Хихикает*"

# game/v1/scene30b.rpy:358
translate russian v1s30b_a0e91df7:

    # u "Let me check how the announcement came out."
    u "Позволь мне проверить, как вышло объявление."

translate russian strings:

    # game/v1/scene30b.rpy:86
    old "Standing next to the plush"
    new "Стоя рядом с плюшевым"

    # game/v1/scene30b.rpy:86
    old "Holding the plush and cheering"
    new "Держа плюшевого и радуясь"

    # game/v1/scene30b.rpy:119
    old "Standing next to the wolf"
    new "Стоя рядом с волком"

    # game/v1/scene30b.rpy:119
    old "High-fiving the wolf"
    new "Давая пять, волку"

    # game/v1/scene30b.rpy:149
    old "It's perfect"
    new "Это прекрасно"

    # game/v1/scene30b.rpy:149
    old "What about something else?"
    new "А как насчет чего-то другого?"

    # game/v1/scene30b.rpy:263
    old "What's a Chick without her Wolf? <3 #PresidentialStatus #Vote4ChloeVote4Wolves"
    new "Что за Чикс без своего Волка? <3 #PresidentialStatus #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:265
    old "Haha, perfect! #Vote4Chloe"
    new "Хаха, отлично! #Vote4Chloe"

    # game/v1/scene30b.rpy:266
    old "Aww! Hell yeah! This is so cute, Chloe <3"
    new "Вау! Да, чёрт возьми! Это так мило, Хлоя <3"

    # game/v1/scene30b.rpy:267
    old "Yessss!!!!!"
    new "Дааааа!!!!!"

    # game/v1/scene30b.rpy:268
    old "LMAO you're down bad, huh?"
    new "LMAO ты плохо себя чувствуешь, да?"

    # game/v1/scene30b.rpy:269
    old "Be civil at least, Grayson"
    new "Будь хотя бы вежливым, Грейсон."

    # game/v1/scene30b.rpy:270
    old "Omg! Can I have that thing?"
    new "Омг! Можно мне эту штуку?"

    # game/v1/scene30b.rpy:271
    old "These turned out perfect! #Vote4Chloe"
    new "Это идеально! #Vote4Chloe"

    # game/v1/scene30b.rpy:272
    old "Aww, haha. Congrats!"
    new "Оу, хаха. Поздравляю!"

    # game/v1/scene30b.rpy:275
    old "It's official! The Wolves and The Chicks are uniting! #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"
    new "Это официально! Волки и Чикс объединяются! #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:287
    old "I'd like to officially announce The Chicks' partnership with The Wolves! <3 #PresidentialStatus #Vote4ChloeVote4Wolves"
    new "Я хочу официально объявить о партнёрстве Чикс с Волками! <3 #PresidentialStatus #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:323
    old "   A vote for me is a vote for The Chicks AND a vote for The Wolves :) #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"
    new "   Голос за меня - это голос за Чикс и голос за Волков :) #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:335
    old "Teamwork makes the dreamwork! <3 #PresidentialStatus #Vote4ChloeVote4Wolves"
    new "Командная работа делает мечту реальностью! <3 #PresidentialStatus #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:347
    old "Teamwork makes the dreamwork! <3 #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"
    new "Командная работа делает мечту реальностью! <3 #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene30b.rpy:41
translate russian v1s30b_8ef7def7:

    # cl "My posing definitely isn't as good as Aubrey's, though."
    cl "Хотя моё позирование не такое хорошее, как у Обри."

# game/v1/scene30b.rpy:51
translate russian v1s30b_a32a3272:

    # cl "I know more about Aubrey than most."
    cl "Я знаю об Обри больше, чем многие."

# game/v1/scene30b.rpy:71
translate russian v1s30b_0bebfb0a:

    # cl "Glad you think so. Now..."
    cl "Рада, что ты так думаешь. Теперь..."

# game/v1/scene30b.rpy:74
translate russian v1s30b_427f28f7:

    # cl "I'm not sure how these will be perceived, you know? What with us having a fake wolf. But I still think the pictures are really good."
    cl "Не уверена, как они будут восприняты и всё такое? Учитывая, что у нас есть фальшивый волк, но я все равно думаю, что фотографии очень хорошие."

# game/v1/scene30b.rpy:104
translate russian v1s30b_8bcdb4cd:

    # cl "Good call. I'm gonna post that one on Kiwii, then."
    cl "Хорошее решение. Тогда размещу её на Kiwii."

# game/v1/scene30b.rpy:107
translate russian v1s30b_4854a4a3:

    # cl "I know the entire school is gonna be off the wall about these photos."
    cl "Я знаю, что вся школа будет вне себя от радости из-за этих фотографий."

# game/v1/scene30b.rpy:137
translate russian v1s30b_8bcdb4cd_1:

    # cl "Good call. I'm gonna post that one on Kiwii, then."
    cl "Хорошее решение. Тогда размещу её на Kiwii."

# game/v1/scene30b.rpy:157
translate russian v1s30b_3b4dcadc:

    # u "Yeah, that's perfect. Standing tall and proud with a short and sweet caption. The main feature is you."
    u "Да, это идеально. Высокая и гордая, с короткой и милой надписью, и главная - это ты."

# game/v1/scene30b.rpy:162
translate russian v1s30b_c9f71855:

    # cl "Exactly."
    cl "Именно."

# game/v1/scene30b.rpy:179
translate russian v1s30b_aacd3584:

    # u "Well, yeah. Like, something about the Chicks and the Wolves, so it feels like a team effort?"
    u "Ну, да. Например, что-нибудь про Чикс и Волках, чтобы было ощущение командной работы?"

# game/v1/scene30b.rpy:184
translate russian v1s30b_36b4367a:

    # cl "I guess that makes sense, considering it's an alliance announcement as well as a good pic for my feed."
    cl "Думаю, это имеет смысл, учитывая, что это объявление альянса, а также хорошая картинка для моей ленты."

# game/v1/scene30b.rpy:199
translate russian v1s30b_ad7b006b:

    # u "You're welcome."
    u "Не за что."

# game/v1/scene30b.rpy:216
translate russian v1s30b_2f758ea1:

    # cl "Okay!"
    cl "Хорошо!"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v1.0.1s
# TODO: Translation updated at 2022-06-28 14:25

# game/v1/scene30b.rpy:147
translate russian v1s30b_ec541876:

    # cl "Well, I'll definitely include \"#presidentialStatus\". Then something about voting for me."
    cl "Что ж, я обязательно включу \"#presidentialStatus\". Потом что-нибудь о голосовании за меня."

translate russian strings:

    # game/v1/scene30b.rpy:263
    old "What's a Chick without her Wolf? <3 #presidentialStatus #Vote4ChloeVote4Wolves"
    new "Что за Чикс без своего волка? <3 #presidentialStatus #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:287
    old "I'd like to officially announce The Chicks' partnership with The Wolves! <3 #presidentialStatus #Vote4ChloeVote4Wolves"
    new "Я хочу официально объявить о партнёрстве Чикс с Волками! <3 #presidentialStatus #Vote4ChloeVote4Wolves"

    # game/v1/scene30b.rpy:335
    old "Teamwork makes the dreamwork! <3 #presidentialStatus #Vote4ChloeVote4Wolves"
    new "Командная работа делает мечту реальностью! <3 #presidentialStatus #Vote4ChloeVote4Wolves"

# TODO: Translation updated at 2023-04-11 15:01

# game/v1/scene30b.rpy:17
translate russian v1s30b_54198d81:

    # cl "All right... Let's get a good look at these."
    cl "Хорошо... Давай рассмотрим их как следует."

# TODO: Translation updated at 2023-06-23 11:22

translate russian strings:

    # game/v1/scene30b.rpy:306
    old "A vote for me is a vote for The Chicks AND a vote for The Wolves :) #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"
    new "Голос за меня - это голос за Чикс и голос за Волков :) #TheChicksWhoCriedWolves #Vote4ChloeVote4Wolves"

