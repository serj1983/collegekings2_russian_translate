# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene31.rpy:18
translate russian v1s31_fcfea652:

    # u "Hello?"
    u "Алло?"

# game/v1/scene31.rpy:23
translate russian v1s31_81a6d608:

    # cl "Hey! I just wanted to call and let you know that everyone is on their way to the Apes house for the get together."
    cl "Привет! Я просто хотела позвонить и сообщить тебе, что все едут в дом Приматов, чтобы собраться вместе."

# game/v1/scene31.rpy:28
translate russian v1s31_1dabfa72:

    # u "Okay, sounds good. I'll meet you there?"
    u "Хорошо, звучит неплохо. Встретимся там?"

# game/v1/scene31.rpy:33
translate russian v1s31_f41eb92b:

    # cl "Yeah... Of course. Also, I know you know already, but please be very cautious of what you say."
    cl "Да... Конечно. Знаю, что ты уже знаешь, но, пожалуйста, будь очень осторожен в своих словах."

# game/v1/scene31.rpy:38
translate russian v1s31_48eaceaf:

    # cl "You know my relationship with the Apes is rocky because of me and Grayson."
    cl "Ты знаешь, что мои отношения с Приматами нестабильны из-за меня и Грейсона."

# game/v1/scene31.rpy:43
translate russian v1s31_179fafc0:

    # u "I'm sure it'll be fine, Chloe."
    u "Уверен, что всё будет хорошо, Хлоя."

# game/v1/scene31.rpy:48
translate russian v1s31_dc136ea0:

    # cl "Please take it seriously, [name]."
    cl "Пожалуйста, отнеситесь к этому серьёзно, [name]."

# game/v1/scene31.rpy:50
translate russian v1s31_7ffb25f7:

    # cl "The slightest misstep can send him overboard and cause complete hell for the both of us."
    cl "Малейший неверный шаг может отправить его за борт и привести к полному аду для нас обоих"

# game/v1/scene31.rpy:58
translate russian v1s31_5a512fdb:

    # u "Chloe, I got this. I know this is serious for you. I'm going to be on my best behavior."
    u "Хлоя, я разберусь. Я знаю, что для тебя это серьезно. Я буду вести себя наилучшим образом."

# game/v1/scene31.rpy:62
translate russian v1s31_24b3f9af:

    # u "Chloe, relax. I'm doing the best I can for you, and that's exactly what I promised."
    u "Хлоя, расслабься. Я делаю для тебя всё, что могу, и это именно то, что я обещал."

# game/v1/scene31.rpy:68
translate russian v1s31_caeb03fa:

    # cl "I know, I know. I'm sorry... Thank you. I'll see you soon."
    cl "Знаю, знаю. Прости... Спасибо. Скоро увидимся."

# game/v1/scene31.rpy:73
translate russian v1s31_d152c937:

    # u "Alrighty."
    u "Ладно."

# game/v1/scene31.rpy:90
translate russian v1s31_7391eb0c:

    # u "(Damn, if I didn't know this campaign was important to her, I know now. She's not even thinking about us.)"
    u "(Чёрт, если бы я не знал, что эта кампания важна для неё... Она даже не думает о нас)."

# game/v1/scene31.rpy:99
translate russian v1s31_1b5e698d:

    # u "(I'm glad she's focused, this is how it should be right now.)"
    u "(Я рад, что она сосредоточена, так и должно быть сейчас)."

# game/v1/scene31.rpy:107
translate russian v1s31_45730942:

    # u "(And it's kind of a turn-off...)"
    u "(И это немного отталкивает...)"

translate russian strings:

    # game/v1/scene31.rpy:55
    old "I got this"
    new "Я понял это"

    # game/v1/scene31.rpy:55
    old "Relax"
    new "Расслабься"

    # game/v1/scene31.rpy:92
    old "I'm glad she's focused"
    new "Я рад, что она сосредоточена"

    # game/v1/scene31.rpy:92
    old "That's a turn-off"
    new "Это отталкивает"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene31.rpy:23
translate russian v1s31_de348817:

    # cl "Hey! I just wanted to call and let you know that everyone is on their way to the Apes house for the get-together."
    cl "Эй! Я просто хотела позвонить и сообщить тебе, что все едут в дом Приматов, чтобы собраться вместе."

