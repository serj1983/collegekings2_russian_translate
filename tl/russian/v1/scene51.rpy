# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene51.rpy:25
translate russian v1s51_fdf6b2cb:

    # u "(Listening in for so long definitely cost me a lot of time. I won't be able to look through most of Chloe's stuff, so I really need to prioritize which locations I want to search.)"
    u "(Долгое прослушивание определенно стоило мне уймы времени. Я не смогу просмотреть большую часть вещей Хлои, поэтому мне нужно расставить приоритеты, в каких местах я хочу искать.)"

# game/v1/scene51.rpy:28
translate russian v1s51_b0c23926:

    # u "(Listening in may have cost me a bit of time, hopefully I'll still have enough to look through most Chloe's stuff, but I definitely won't be able to search everywhere.)"
    u "(Прослушивание, возможно, стоило мне немного времени, надеюсь, у меня всё ещё будет достаточно времени, чтобы просмотреть большинство вещей Хлои, но я определенно не смогу искать везде.)"

# game/v1/scene51.rpy:31
translate russian v1s51_f0e227ab:

    # u "(I think Lindsey will be able to hold Aubrey off a bit, so hopefully I'll have enough time to look everywhere.)"
    u "(Думаю, Линдси сможет немного задержать Обри, так что, надеюсь, у меня будет достаточно времени, чтобы осмотреться повсюду.)"

# game/v1/scene51.rpy:33
translate russian v1s51_c554238b:

    # u "(So... where would Chloe keep the money?)"
    u "(Итак... где бы Хлоя хранила деньги?)"

# game/v1/scene51.rpy:51
translate russian v1s51_bedside_table_28792f53:

    # u "(No money here. But there is a vibrator.)"
    u "(Здесь денег нет. Но есть вибратор.)"

# game/v1/scene51.rpy:53
translate russian v1s51_bedside_table_5db286c4:

    # u "(Pretty basic bedside table for a woman... Wonder how often she uses that thing... Let's search somewhere else.)"
    u "(Довольно простая прикроватная тумбочка для женщины... Интересно, как часто она пользуется ей... Поищем в другом месте)."

# game/v1/scene51.rpy:67
translate russian v1s51_text_86327f11:

    # u "(That's probably Chloe texting me, fuck.)"
    u "(Это, наверное, Хлоя мне пишет, блин.)"

# game/v1/scene51.rpy:82
translate russian v1s51_PhoneContinue_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v1/scene51.rpy:88
translate russian v1s51_PhoneContinue_d764307a:

    # u "(*Sighs* I can't worry right now about Chloe being angry with me. Back to the search.)"
    u "(*Вздох* Я не могу сейчас беспокоиться о том, что Хлоя сердится на меня. Вернемся к поиску)."

# game/v1/scene51.rpy:96
translate russian v1s51_desk_drawer_6c799bbc:

    # u "(Let's see what's hiding in this drawer.)"
    u "(Давай посмотрим, что скрывается в этом ящике)."

# game/v1/scene51.rpy:106
translate russian v1s51_desk_drawer_0d4904f9:

    # u "(A list of Chloe's passwords... This could probably help Lindsey in some way, but I'm not comfortable with something like that.)"
    u "(Список паролей Хлои... Возможно, это может как-то помочь Линдси, но мне не по себе от этого)."

# game/v1/scene51.rpy:111
translate russian v1s51_desk_drawer_ca3fe95d:

    # u "(And what's this? I didn't know Chloe was into old-school video games...? Hmm, that knowledge might come in handy someday...)"
    u "(И что это? Я не знал, что Хлоя увлекается ретро видеоиграми...? Хм, когда-нибудь это знание может пригодиться...)"

# game/v1/scene51.rpy:116
translate russian v1s51_desk_drawer_3a3d20d5:

    # u "(This must be Chloe's parents. And she has a pig as a pet? I'm learning a lot about her today...)"
    u "(Это, должно быть, родители Хлои. И у неё есть свинка в качестве домашнего животного? Сегодня я узнаю о ней много нового...)"

# game/v1/scene51.rpy:121
translate russian v1s51_desk_drawer_5635871f:

    # u "(No money here though, so that's more time wasted. *Sighs*)"
    u "(Хотя денег здесь нет, так что время потрачено зря. *Вздох*)."

# game/v1/scene51.rpy:134
translate russian v1s51_closet_f28db2d9:

    # u "(A mystery box? Perfect.)"
    u "(Тайная коробочка? Идеально.)"

# game/v1/scene51.rpy:150
translate russian v1s51_closet_54335fe9:

    # u "(Jackpot! That's $500, baby! *Laughs*)"
    u "(Джекпот! Это $500, детка! *Смеётся*)"

# game/v1/scene51.rpy:152
translate russian v1s51_closet_5b24999a:

    # u "(Jackpot! That's $900, baby! *Laughs*)"
    u "(Джекпот! Это 900 долларов, детка! *Смеётся*)"

# game/v1/scene51.rpy:154
translate russian v1s51_closet_b3c940d3:

    # u "(I'm here for the money and all, but now that it's in my hands...)"
    u "(Я здесь ради денег и всё такое, но теперь, когда они в моих руках...)"

# game/v1/scene51.rpy:156
translate russian v1s51_closet_d9c287c9:

    # u "(Can I really do this to her?)"
    u "(Неужели я могу так поступить с ней?)"

# game/v1/scene51.rpy:167
translate russian v1s51_closet_9a54d146:

    # u "(I've come this far already. I'm here to help Lindsey, and that's what I'm going to do.)"
    u "(Я уже зашёл так далеко. Я здесь, чтобы помочь Линдси, и именно это я и собираюсь сделать)."

# game/v1/scene51.rpy:173
translate russian v1s51_closet_b60b0aa7:

    # u "(I guess I'm having second thoughts about this... I can't take nearly a thousand dollars from Chloe's private closet. What the fuck was I thinking?)"
    u "(Думаю, что у меня есть сомнения по этому поводу... Я не могу взять почти тысячу долларов из личного шкафа Хлои. О чем, блядь, я думал?)"

# game/v1/scene51.rpy:184
translate russian v1s51_closet_9a54d146_1:

    # u "(I've come this far already. I'm here to help Lindsey, and that's what I'm going to do.)"
    u "(Я уже зашёл так далеко. Я здесь, чтобы помочь Линдси, и именно это я и собираюсь сделать)."

# game/v1/scene51.rpy:190
translate russian v1s51_closet_b60b0aa7_1:

    # u "(I guess I'm having second thoughts about this... I can't take nearly a thousand dollars from Chloe's private closet. What the fuck was I thinking?)"
    u "(Думаю, что у меня есть сомнения по этому поводу... Я не могу взять почти тысячу долларов из личного шкафа Хлои. О чем, блядь, я думал?)"

# game/v1/scene51.rpy:202
translate russian v1s51_purse_b6ff6426:

    # u "(Surely this is too obvious...)"
    u "(Конечно, это слишком очевидно...)"

# game/v1/scene51.rpy:232
translate russian v1s51_purse_21f189cd:

    # u "(Oh shit! Cash in a purse? Who would have thought?)"
    u "(Вот чёрт! Наличные в кошельке? Кто бы мог подумать?)"

# game/v1/scene51.rpy:234
translate russian v1s51_purse_27c7384a:

    # u "($300 is a lot of money... Lindsey really needs it, but ultimately this is my choice.)"
    u "(300 долларов - это большие деньги... Линдси это действительно нужно, но в конечном итоге это мой выбор)."

# game/v1/scene51.rpy:246
translate russian v1s51_purse_7898847f:

    # u "(If I don't take this money, I could be the reason Lindsey loses this race. I don't want it to be my fault.)"
    u "(Если я не возьму эти деньги, я могу стать причиной проигрыша Линдси в этой гонке. Я не хочу, чтобы это была моя вина)."

# game/v1/scene51.rpy:253
translate russian v1s51_purse_8fd28fcd:

    # u "(I guess my conscience has caught up with me. It doesn't feel right to take it.)"
    u "(Наверное, совесть меня догнала. Как-то не по себе от этого)."

# game/v1/scene51.rpy:270
translate russian v1s51_pillow_f08a8661:

    # u "Hey, man. You don't have a secret camera in your eye, do you? *Chuckles*"
    u "Эй, парень. У тебя ведь нет секретной камеры в глазу? *Хихикает*"

# game/v1/scene51.rpy:272
translate russian v1s51_pillow_01394f4a:

    # u "(Let's fucking hope not.)"
    u "(Будем надеяться, что нет.)"

# game/v1/scene51.rpy:277
translate russian v1s51_pillow_7d243392:

    # u "(There could be something under her pillow.)"
    u "(Под её подушкой может что-то быть)."

# game/v1/scene51.rpy:292
translate russian v1s51_pillow_bca6df02:

    # u "(Chloe's diary... Shit. I wonder what secrets are waiting to be found in here.)"
    u "(Дневник Хлои... Чёрт. Интересно, какие секреты ждут здесь.)."

# game/v1/scene51.rpy:294
translate russian v1s51_pillow_e453d3e9:

    # u "(It's locked but I'm sure it wouldn't be too hard to open, and Lindsey could find something useful to help her win.)"
    u "(Он заперт, но я уверен, что открыть его будет несложно, и Линдси сможет найти что-нибудь полезное, что поможет ей победить)."

# game/v1/scene51.rpy:296
translate russian v1s51_pillow_fdcd15be:

    # u "(But this contains all of Chloe's secrets... Should I take it?)"
    u "(Но здесь содержатся все секреты Хлои... Стоит ли мне брать его?)"

# game/v1/scene51.rpy:305
translate russian v1s51_pillow_85abedd0:

    # u "(I think Lindsey will appreciate this little bonus item. Plus, I'm curious to see what we'll learn.)"
    u "(Думаю, Линдси оценит этот небольшой бонус. К тому же, мне любопытно посмотреть, что мы узнаем)."

# game/v1/scene51.rpy:312
translate russian v1s51_pillow_b48f19dc:

    # u "(There's going to be some extremely private things in here... It's best if I leave it.)"
    u "(Здесь хранятся довольно личные вещи... Будет лучше, если я оставлю)."

# game/v1/scene51.rpy:333
translate russian v1s51_continue_e929ff41:

    # u "(Huh? What was that?)"
    u "(А? Что это было?)."

# game/v1/scene51.rpy:338
translate russian v1s51_continue_af58d902:

    # unknown "*Distant chatter*"
    unknown "*Дальние разговоры*"

# game/v1/scene51.rpy:340
translate russian v1s51_continue_9f3001f7:

    # u "(Sounds like some of the girls just got home. I need to get out of here, NOW!)"
    u "(Звучит так, будто другие девушки только что вернулись домой. Мне нужно убираться отсюда, СЕЙЧАС!)"

# game/v1/scene51.rpy:359
translate russian v1s51_continue_fef3be4d:

    # u "('Our secret place'?)"
    u "('Наше тайное место'?)"

# game/v1/scene51.rpy:366
translate russian v1s51_continue_656da2d4:

    # u "(Locked. Damnit! What the hell is in here, though?)"
    u "(Заперто. Чёрт! Что здесь, чёрт возьми, находится?)"

# game/v1/scene51.rpy:381
translate russian v1s51_continue_7b44f9c3:

    # unknown "*Whistling badly*"
    unknown "*Сильно насвистывает*"

# game/v1/scene51.rpy:383
translate russian v1s51_continue_c0a23535:

    # u "(Shit, shit, shit!!! There's someone in the shower...) *Whispers* Fuck."
    u "(Чёрт, чёрт, чёрт!!! В душе кто-то есть...) *Шёпотом* Чёрт."

# game/v1/scene51.rpy:385
translate russian v1s51_continue_6918a268:

    # unknown "Hello? Who's there?"
    unknown "Ей? Кто здесь?"

# game/v1/scene51.rpy:387
translate russian v1s51_continue_38e97bf3:

    # u "(OH. SHIT.)"
    u "(ОХ. ЧЁРТ.)"

# game/v1/scene51.rpy:389
translate russian v1s51_continue_ab597e93:

    # unknown "Catie, is it you?"
    unknown "Кэти, это ты?"

# game/v1/scene51.rpy:391
translate russian v1s51_continue_ca1ee76a:

    # u "(Quick! Think of something before she steps out and sees me!)"
    u "(Быстро! Надо придумать что-нибудь, пока она не вышла и не увидела меня!)"

# game/v1/scene51.rpy:397
translate russian v1s51_continue_e920ea0e:

    # u "(Why am I even in here? I need to get the fuck out.)"
    u "(Почему я вообще здесь нахожусь? Мне нужно убираться отсюда нахрен.)"

# game/v1/scene51.rpy:399
translate russian v1s51_continue_f855f2ee:

    # unknown "If it's you, make sure you lock the door this time..."
    unknown "Если это ты, проверь, что ты заперла дверь..."

# game/v1/scene51.rpy:401
translate russian v1s51_continue_586696f1:

    # u "(These Chicks are wildin'.)"
    u "(Эти девчонки просто дикие.)"

# game/v1/scene51.rpy:406
translate russian v1s51_continue_f77df04f:

    # u "(I don't hear those girls talking anymore. Maybe I can creep downstairs...)"
    u "(Я больше не слышу разговора этих девушек. Может, я смогу прокрасться вниз...)"

# game/v1/scene51.rpy:426
translate russian v1s51_continue_b57f2c08:

    # u "(Okay, now what? Go out the same way we came in? Or try the kitchen?"
    u "(Хорошо, что теперь? Выйти так же, как мы вошёл? Или попробовать через кухню?"

# game/v1/scene51.rpy:435
translate russian v1s51_continue_8253698b:

    # unknown "*Muffled chatter*"
    unknown "*Приглушенная болтовня*"

# game/v1/scene51.rpy:437
translate russian v1s51_continue_3d665997:

    # u "(Fuck me! Whoever that is, it sounds like they're about to come in. Let's try the kitchen!)"
    u "(Твою мать! Кто бы это ни был, похоже, они собираются войти. Давай попробуем через кухню!)"

# game/v1/scene51.rpy:453
translate russian v1s51_continue_6ac34614:

    # u "*Whispers* Please be unlocked, please be unlocked..."
    u "*Шёпотом* Пожалуйста, открывайся, пожалуйста, открывайся..."

# game/v1/scene51.rpy:460
translate russian v1s51_continue_0218aa3e:

    # u "(YOU'RE FUCKING KIDDING ME?! Why is the back door locked?!)"
    u "(ТЫ, БЛЯДЬ, ИЗДЕВАЕШЬСЯ НАДО МНОЙ?! Почему задняя дверь заперта?!)"

# game/v1/scene51.rpy:481
translate russian v1s51_continue_f2cd13de:

    # u "*Grunts* (Come... On...)"
    u "*Ворчит* (Давай... Давай...)"

# game/v1/scene51.rpy:486
translate russian v1s51_continue_c9334358:

    # u "(YES! FREEDOM!)"
    u "(ДА! СВОБОДА!)"

# game/v1/scene51.rpy:496
translate russian v1s51_continue_60074598:

    # u "*Whispers* Damnit..."
    u "*Шёпотом* Черт возьми..."

# game/v1/scene51.rpy:523
translate russian v1s51_continue_1a60acaa:

    # u "(So lucky that the vase didn't smash... Holy shit.)"
    u "(Так повезло, что ваза не разбилась... Срань господня.)"

# game/v1/scene51.rpy:530
translate russian v1s51_continue_55bfa5ad:

    # u "(I need to get the fuck out this house ASAP! Let's try the window.)"
    u "(Мне нужно убраться к черту из этого дома как можно скорее! Давай попробуем открыть окно.)"

# game/v1/scene51.rpy:532
translate russian v1s51_continue_f855f2ee_1:

    # unknown "If it's you, make sure you lock the door this time..."
    unknown "Если это ты, проверь, что ты заперла дверь..."

# game/v1/scene51.rpy:534
translate russian v1s51_continue_54f8acca:

    # u "(Lock the door... What do these Chicks get up to in private?)"
    u "(Запереть дверь... Чем эти Чикс занимаются наедине?)"

# game/v1/scene51.rpy:541
translate russian v1s51_continue_30b0f619:

    # unknown "Ow- Fuck! My toe! *Whines* Damn you, shampoo..."
    unknown "Ай-Блядь! Мой палец! *Скулит* Чёрт бы побрал, этот шампунь..."

# game/v1/scene51.rpy:554
translate russian v1s51_continue_95e8a69c:

    # u "(Sheeeeeesh...)"
    u "(Шииииш...)"

# game/v1/scene51.rpy:564
translate russian v1s51_continue_4994c728:

    # u "(What a nice little treat... Now it's time to get the fuck out of here.)"
    u "(Какое приятное маленькое удовольствие... А теперь пора убираться отсюда.)"

# game/v1/scene51.rpy:571
translate russian v1s51_continue_78c78ef8:

    # u "(No time for peeking, [name]. Don't even think about it. FOCUS!)"
    u "(Не время для подглядывания, [name]. Даже не думай об этом. СОСРЕДОТОЧЬСЯ!)"

# game/v1/scene51.rpy:588
translate russian v1s51_continue_8d40b265:

    # u "(Fuck! That was loud.)"
    u "(Бля! Это было громко.)"

# game/v1/scene51.rpy:593
translate russian v1s51_continue_17f13d0b:

    # u "(And it's not over yet... I need to get down without breaking my legs.)"
    u "(И это еще не конец... Мне нужно спуститься, не сломав ноги)."

# game/v1/scene51.rpy:598
translate russian v1s51_continue_5e97c402:

    # u "*Grunts* (This better... have been... worth it...) *Panting*"
    u "*Ворчит* (Лучше бы... это... того... стоило...) *Задыхается*"

# game/v1/scene51.rpy:603
translate russian v1s51_continue_f2880612:

    # u "Phew!"
    u "Уф!"

# game/v1/scene51.rpy:612
translate russian v1s51_continue_0ab11823:

    # u "(And now, a quick text to Lindsey...)"
    u "(А теперь короткое сообщение Линдси...)"

# game/v1/scene51.rpy:621
translate russian v1s51Lindsey_PhoneContinue_f9e9813d:

    # u "(I should reply to Lindsey.)"
    u "(Я должен ответить Линдси.)"

# game/v1/scene51.rpy:627
translate russian v1s51Lindsey_PhoneContinue_e81c1e19:

    # u "(Uh oh... it's a message from Chloe...)"
    u "(О-о-о... это сообщение от Хлои...)"

# game/v1/scene51.rpy:636
translate russian v1s51Chloe_PhoneContinue_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v1/scene51.rpy:657
translate russian v1s51Chloe_PhoneContinue_dfcbd50d:

    # li "*Whispers* Thank God that's over!"
    li "*Шёпотом* Слава Богу, всё закончилось!"

# game/v1/scene51.rpy:660
translate russian v1s51Chloe_PhoneContinue_a552f471:

    # li "*Whispers* What was that loud bang?! Was that you?"
    li "*Шёпотом* Что это был за громкий хлопок?! Это был ты?"

# game/v1/scene51.rpy:665
translate russian v1s51Chloe_PhoneContinue_af549b83:

    # u "*Whispers* Yeah, that was me risking my life by jumping out of the bathroom window!"
    u "*Шёпотом* Да, это я рисковал своей жизнью, выпрыгивая из окна ванной!"

# game/v1/scene51.rpy:670
translate russian v1s51Chloe_PhoneContinue_91298ff4:

    # li "*Whispers* Oh shit..."
    li "*Шёпотом* Вот дерьмо..."

# game/v1/scene51.rpy:675
translate russian v1s51Chloe_PhoneContinue_b8ac56de:

    # u "*Whispers* Oh shit is right, haha."
    u "*Шёпотом* Вот дерьмо, это точно, ха-ха."

# game/v1/scene51.rpy:680
translate russian v1s51Chloe_PhoneContinue_9bc94be1:

    # li "*Whispers* Aubrey heard it."
    li "*Шёпотом* Обри слышала это."

# game/v1/scene51.rpy:685
translate russian v1s51Chloe_PhoneContinue_fdbce207:

    # u "Oh..."
    u "Ох..."

# game/v1/scene51.rpy:690
translate russian v1s51Chloe_PhoneContinue_746f0beb:

    # li "I thought it must have been you, so I just said something about how I think we've got a nest of pigeons."
    li "Я подумала, что это, должно быть, ты, поэтому я сказала, что, у нас есть голубиное гнездо."

# game/v1/scene51.rpy:692
translate russian v1s51Chloe_PhoneContinue_fb866a8f:

    # li "I always hear them banging like that."
    li "Я всегда слышу, как они так стучат."

# game/v1/scene51.rpy:697
translate russian v1s51Chloe_PhoneContinue_06f38abc:

    # u "*Whispers* Fucking pigeons."
    u "*Шёпотом* Ёбаные голуби."

# game/v1/scene51.rpy:702
translate russian v1s51Chloe_PhoneContinue_eafa4053:

    # li "*Quiet laugh* *Whispers* I'm glad you survived."
    li "*Тихий смех* *Шёпотом* Я рада, что ты выжил."

# game/v1/scene51.rpy:707
translate russian v1s51Chloe_PhoneContinue_201fd9bb:

    # u "*Whispers* Yeah, just about."
    u "*Шепотом* Да, почти."

# game/v1/scene51.rpy:713
translate russian v1s51Chloe_PhoneContinue_a692497c:

    # li "*Whispers* Happy to see you survived!"
    li "*Шепотом* Рада видеть, что ты выжил!"

# game/v1/scene51.rpy:718
translate russian v1s51Chloe_PhoneContinue_498f1b58:

    # u "*Whispers* Yeah, my body is intact, but my nerves are shot."
    u "*Шёпотом* Да, физически цел, но нервишки шалят."

# game/v1/scene51.rpy:723
translate russian v1s51Chloe_PhoneContinue_5c35686d:

    # li "*Quiet chuckle* *Whispers* I'm proud of you."
    li "*Тихая усмешка* *Шёпотом* Я горжусь тобой."

# game/v1/scene51.rpy:729
translate russian v1s51Chloe_PhoneContinue_69844942:

    # u "*Whispers* I'm sorry, but I didn't find any money."
    u "*Шёпотом* Прости, но я не нашёл денег."

# game/v1/scene51.rpy:734
translate russian v1s51Chloe_PhoneContinue_2bd0d87c:

    # li "*Whispers* Seriously? Ugh! She must have hidden it somewhere insane."
    li "*Шёпотом* Серьезно? Ух! Она, должно быть, спрятала его где-то в безумном месте."

# game/v1/scene51.rpy:739
translate russian v1s51Chloe_PhoneContinue_7cfe10b4:

    # u "*Whispers* Yeah, I did my best. Sorry, Linds."
    u "*Шёпотом* Да, я старался изо всех сил. Прости, Линдс."

# game/v1/scene51.rpy:744
translate russian v1s51Chloe_PhoneContinue_915db9ad:

    # li "*Whispers* I know, yeah... My campaign is really going to suffer without that money."
    li "*Шёпотом* Я знаю, да... Моя кампания очень пострадает без этих денег."

# game/v1/scene51.rpy:749
translate russian v1s51Chloe_PhoneContinue_e18e3349:

    # u "*Whispers* Well, I did find this..."
    u "*Шёпотом* Ну, я нашел это..."

# game/v1/scene51.rpy:754
translate russian v1s51Chloe_PhoneContinue_b34f7b8d:

    # li "*Whispers* Chloe's diary?!"
    li "*Шёпотом* Дневник Хлои?!"

# game/v1/scene51.rpy:759
translate russian v1s51Chloe_PhoneContinue_fd25579d:

    # u "*Whispers* It might have some useful information for you."
    u "*Шёпотом* Там может быть полезная информация для тебя."

# game/v1/scene51.rpy:764
translate russian v1s51Chloe_PhoneContinue_ffdbbd62:

    # li "*Whispers* True. That's a great idea, [name]. Thanks for getting this at least. Really sucks about the money though."
    li "*Шёпотом* Правда. Отличная идея, [name]. Спасибо, что хоть это получилось. Хотя с деньгами действительно хреново."

# game/v1/scene51.rpy:769
translate russian v1s51Chloe_PhoneContinue_a320bbe2:

    # u "*Whispers* Yeah. I know, I'm really sorry."
    u "*Шёпотом* Да. Я знаю, мне очень жаль."

# game/v1/scene51.rpy:771
translate russian v1s51Chloe_PhoneContinue_01a2a4c9:

    # u "*Whispers* I should really get home now, though. Getting a little tired."
    u "*Шёпотом* Мне уже пора домой. Немного устал."

# game/v1/scene51.rpy:776
translate russian v1s51Chloe_PhoneContinue_ca7332dd:

    # li "*Whispers* Yeah, okay. Thanks again. See you..."
    li "*Шёпотом* Да, хорошо. Еще раз спасибо. Увидимся..."

# game/v1/scene51.rpy:785
translate russian v1s51Chloe_PhoneContinue_3fdbc86f:

    # u "*Whispers* I'm so sorry, Linds. I didn't find anything."
    u "*Шёпотом* Мне так жаль, Линдс. Я ничего не нашёл."

# game/v1/scene51.rpy:790
translate russian v1s51Chloe_PhoneContinue_0ee82982:

    # li "*Whispers* What? How?! That money must be in her room somewhere, [name]."
    li "*Шёпотом* Что? Почему?! Эти деньги должны быть где-то в её комнате, [name]."

# game/v1/scene51.rpy:795
translate russian v1s51Chloe_PhoneContinue_8ea48fa9:

    # u "*Whispers* I don't know what to say. I did my best... I'm sorry"
    u "*Шёпотом* Я не знаю, что сказать. Я сделал всё, что мог... Мне жаль."

# game/v1/scene51.rpy:800
translate russian v1s51Chloe_PhoneContinue_d40a8ae6:

    # li "*Whispers* I'm sure you did, [name], but that doesn't help my campaign. *Sighs*"
    li "*Шёпотом* Я уверена, что ты сделал это, [name], но это не поможет моей кампании. *Вздыхает*"

# game/v1/scene51.rpy:802
translate russian v1s51Chloe_PhoneContinue_7f1bf42b:

    # li "Thanks for trying anyway."
    li "Всё равно спасибо за попытку."

# game/v1/scene51.rpy:807
translate russian v1s51Chloe_PhoneContinue_01a2a4c9_1:

    # u "*Whispers* I should really get home now, though. Getting a little tired."
    u "*Шёпотом* Мне уже пора домой. Немного устал."

# game/v1/scene51.rpy:812
translate russian v1s51Chloe_PhoneContinue_ca7332dd_1:

    # li "*Whispers* Yeah, okay. Thanks again. See you..."
    li "*Шёпотом* Да, хорошо. Еще раз спасибо. Увидимся..."

# game/v1/scene51.rpy:821
translate russian v1s51Chloe_PhoneContinue_ca8f77cb:

    # u "*Whispers* Here's what I managed to find."
    u "*Шёпотом* Вот что мне удалось найти."

# game/v1/scene51.rpy:826
translate russian v1s51Chloe_PhoneContinue_7bd8212d:

    # li "*Whispers* This is... amazing! This is going to help tremendously, [name]! You're incredible."
    li "*Шёпотом* Это... потрясающе! Это очень поможет, [name]! Ты невероятен."

# game/v1/scene51.rpy:836
translate russian v1s51Chloe_PhoneContinue_d61475a5:

    # u "*Whispers* So it was worth risking my life for, then?"
    u "*Шёпотом* Значит, ради этого стоило рисковать жизнью?"

# game/v1/scene51.rpy:841
translate russian v1s51Chloe_PhoneContinue_5f9ac0d3:

    # li "*Whispers* Of course! *Chuckles*"
    li "*Шёпотом* Конечно! *Хихикает*"

# game/v1/scene51.rpy:849
translate russian v1s51Chloe_PhoneContinue_9454ebe1:

    # u "*Whispers* I also found this..."
    u "*Шёпотом* Я также нашел это..."

# game/v1/scene51.rpy:854
translate russian v1s51Chloe_PhoneContinue_b2f7775b:

    # li "*Whispers* What's that- Wait, Chloe's... diary?"
    li "*Шёпотом* Что это... Подожди, дневник Хлои?"

# game/v1/scene51.rpy:859
translate russian v1s51Chloe_PhoneContinue_7300dfe9:

    # u "*Whispers* Yeah. You might find something useful there, but it is locked."
    u "*Шёпотом* Да. Ты можешь найти там что-нибудь полезное, но он заперт."

# game/v1/scene51.rpy:864
translate russian v1s51Chloe_PhoneContinue_bccb21a3:

    # li "*Whispers* I'm sure I can get it open, this is crazy!"
    li "*Шёпотом* Я уверена, что смогу открыть его, это безумие!"

# game/v1/scene51.rpy:869
translate russian v1s51Chloe_PhoneContinue_0dacddb5:

    # u "*Whispers* Your best friend, the janitor, can probably help."
    u "*Шёпотом* Твой лучший друг, уборщик, может помочь."

# game/v1/scene51.rpy:874
translate russian v1s51Chloe_PhoneContinue_24edef7f:

    # li "*Whispers* True, he probably can."
    li "*Шёпотом* Правда, возможно, он сможет."

# game/v1/scene51.rpy:879
translate russian v1s51Chloe_PhoneContinue_f76e2718:

    # u "*Whispers* Anyways, I better get home. I'm getting a little tired after all that, haha."
    u "*Шёпотом* В общем, мне пора домой. Я немного устал после всего этого, хаха."

# game/v1/scene51.rpy:884
translate russian v1s51Chloe_PhoneContinue_cfb3f935:

    # li "*Whispers* Okay. Thank you, [name]. I really mean it. You did an amazing job."
    li "*Шёпотом* Хорошо. Спасибо, [name]. Я действительно это имею в виду. Ты проделал потрясающую работу."

# game/v1/scene51.rpy:889
translate russian v1s51Chloe_PhoneContinue_135005ae:

    # u "*Whispers* Anytime, Linds."
    u "*Шёпотом* В любое время, Линдс."

translate russian strings:

    # game/v1/scene51.rpy:8
    old "I'm already full on breadsticks... Ugh! Hurry up, [name]. Please?"
    new "Я уже наелся хлебных палочек... Ух! Поторопись, [name]. Пожалуйста?"

    # game/v1/scene51.rpy:9
    old "I'll be there as soon as I can, Chloe."
    new "Я приеду, как только смогу, Хлоя."

    # game/v1/scene51.rpy:10
    old "I'll give you 10 more minutes. If you're still not here by then, I'm going home."
    new "Я дам тебе еще 10 минут. Если к тому времени ты не придешь, я пойду домой."

    # game/v1/scene51.rpy:72
    old "Hey, where are you??"
    new "Эй, где ты?"

    # game/v1/scene51.rpy:73
    old "Sorry, I'm running late. The cab broke down, waiting on another."
    new "Извини, опаздываю. Такси сломалось, жду другое."

    # game/v1/scene51.rpy:74
    old "I thought you'd already be here? Are you lying to me?"
    new "Я думала, ты уже здесь? Ты лжешь мне?"

    # game/v1/scene51.rpy:75
    old "I'm coming, I promise. Just order your starter."
    new "Я приду, обещаю. Просто закажи стартер."

    # game/v1/scene51.rpy:76
    old "I'd never lie to you, I'll be there soon."
    new "Я бы никогда не солгал тебе, я скоро приеду."

    # game/v1/scene51.rpy:159
    old "Take the $500"
    new "Возьми $500"

    # game/v1/scene51.rpy:159
    old "Don't take the money"
    new "Не брать денег"

    # game/v1/scene51.rpy:176
    old "Take the $900"
    new "Возьми $900"

    # game/v1/scene51.rpy:236
    old "Take the $300"
    new "Возьми $300"

    # game/v1/scene51.rpy:298
    old "Take Chloe's diary"
    new "Возьми дневник Хлои"

    # game/v1/scene51.rpy:298
    old "Don't take the diary"
    new "Не брать дневник Хлои"

    # game/v1/scene51.rpy:393
    old "Return to the hallway"
    new "Вернуться в коридор"

    # game/v1/scene51.rpy:393
    old "Go out the window"
    new "Выйти через окно"

    # game/v1/scene51.rpy:428
    old "Front door"
    new "Входная дверь"

    # game/v1/scene51.rpy:428
    old "Kitchen"
    new "Кухня"

    # game/v1/scene51.rpy:448
    old "Back door"
    new "Задняя дверь"

    # game/v1/scene51.rpy:448
#    old "Window"
#    new "Окно"

    # game/v1/scene51.rpy:543
    old "Peek"
    new "Заглянуть"

    # game/v1/scene51.rpy:543
    old "Don't peek"
    new "Не заглядывать"

    # game/v1/scene51.rpy:614
    old "I'm around the back of the house. Come quick!"
    new "Я нахожусь за домом. Иди скорее!"

    # game/v1/scene51.rpy:615
    old "OMW!"
    new "ОГО!"

    # game/v1/scene51.rpy:628
    old "What the hell is going on?!"
    new "Что, чёрт возьми, происходит?!"

    # game/v1/scene51.rpy:629
    old "[name]??? I'm about to order food to go"
    new "[name]??? Я собираюсь заказать еду на вынос"

    # game/v1/scene51.rpy:630
    old "Well, you were right. The food was to die for. You fucking prick."
    new "Что ж, ты был прав. Еда была просто умопомрачительной. Ты долбаный урод."

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene51.rpy:53
translate russian v1s51_bedside_table_6995d0eb:

    # u "(Pretty basic bedside table for a woman... wonder how often she uses that thing... let's search somewhere else.)"
    u "(Довольно простая прикроватная тумбочка для женщины... Интересно, как часто она пользуется ей... Поищем в другом месте.)"

# game/v1/scene51.rpy:106
translate russian v1s51_desk_drawer_0813f31c:

    # u "(A list of Chloe's passwords... this could probably help Lindsey in some way, but I'm not comfortable with something like that.)"
    u "(Список паролей Хлои... Возможно, это может как-то помочь Линдси, но мне не по себе от этого.)"

# game/v1/scene51.rpy:150
translate russian v1s51_closet_a273e5e5:

    # u "(Jackpot! That's $500, baby!)"
    u "(Джекпот! Это $500, детка!)"

# game/v1/scene51.rpy:152
translate russian v1s51_closet_81868e32:

    # u "(Jackpot! That's $900, baby!)"
    u "(Джекпот! Это $900, детка!)"

# game/v1/scene51.rpy:278
translate russian v1s51_pillow_a9382bc8:

    # u "Hey, man. You don't have a secret camera in your eye, do you?"
    u "Эй, парень. У тебя ведь нет секретной камеры в глазу?"

# game/v1/scene51.rpy:300
translate russian v1s51_pillow_9d7d5cd3:

    # u "(Chloe's diary... shit. I wonder what secrets are waiting to be found in here.)"
    u "(Дневник Хлои... Чёрт. Интересно, какие секреты ждут здесь.)"

# game/v1/scene51.rpy:302
translate russian v1s51_pillow_129d3375:

    # u "(It's locked, but I'm sure it wouldn't be too hard to open, and Lindsey could find something useful to help her win.)"
    u "(Он заперт, но я уверен, что открыть его будет несложно, и Линдси сможет найти что-нибудь полезное, что поможет ей победить.)"

# game/v1/scene51.rpy:304
translate russian v1s51_pillow_76d99f04:

    # u "(But this contains all of Chloe's secrets... should I take it?)"
    u "(Но здесь содержатся все секреты Хлои... Стоит ли мне брать его?)"

# game/v1/scene51.rpy:322
translate russian v1s51_pillow_74c34afd:

    # u "(There's going to be some extremely private things in here... it's best if I leave it.)"
    u "(Здесь хранятся довольно личные вещи... Будет лучше, если я оставлю.)"

# game/v1/scene51.rpy:551
translate russian v1s51_continue_463fe45a:

    # unknown "Ow, fuck! My toe! *Whines* Damn you, shampoo..."
    unknown "Ой, блядь! Мой палец! *Хнычет* Чёрт бы тебя побрал, шампунь..."

# game/v1/scene51.rpy:685
translate russian v1s51Chloe_PhoneContinue_24057a0f:

    # u "*Whispers* Oh shit is right."
    u "*Шепотом* Ох, дерьмо, это верно."

# game/v1/scene51.rpy:712
translate russian v1s51Chloe_PhoneContinue_84e77792:

    # li "*Whispers* I'm glad you survived."
    li "*Шёпотом* Я рада, что ты выжил."

# game/v1/scene51.rpy:733
translate russian v1s51Chloe_PhoneContinue_1fe05248:

    # li "*Whispers* I'm proud of you."
    li "*Шёпотом* Я горжусь тобой."

# game/v1/scene51.rpy:754
translate russian v1s51Chloe_PhoneContinue_1ad9c34d:

    # li "*Whispers* I know, yeah... my campaign is really going to suffer without that money."
    li "*Шёпотом* Я знаю, да... Моя кампания очень пострадает без этих денег."

# game/v1/scene51.rpy:774
translate russian v1s51Chloe_PhoneContinue_253b0bab:

    # li "*Whispers* True. That's a great idea, [name]. Thanks for getting this, at least. Really sucks about the money, though."
    li "*Шёпотом* Правда. Отличная идея, [name]. Спасибо, что хоть это получилось. Хотя с деньгами действительно хреново."

# game/v1/scene51.rpy:781
translate russian v1s51Chloe_PhoneContinue_ae49f2f9:

    # u "*Whispers* I should really get home now. Getting a little tired."
    u "*Шёпотом* Мне уже пора домой. Немного устал."

# game/v1/scene51.rpy:805
translate russian v1s51Chloe_PhoneContinue_15988bd5:

    # u "*Whispers* I don't know what to say. I did my best... I'm sorry."
    u "*Шёпотом* Я не знаю, что сказать. Я сделал всё, что мог... Мне жаль."

# game/v1/scene51.rpy:817
translate russian v1s51Chloe_PhoneContinue_ae49f2f9_1:

    # u "*Whispers* I should really get home now. Getting a little tired."
    u "*Шёпотом* Мне уже пора домой. Немного устал."

# game/v1/scene51.rpy:851
translate russian v1s51Chloe_PhoneContinue_a80c446e:

    # li "*Whispers* Of course!"
    li "*Шёпотом* Конечно!"

# game/v1/scene51.rpy:864
translate russian v1s51Chloe_PhoneContinue_440fb94f:

    # li "*Whispers* What's that... wait, Chloe's... diary?"
    li "*Шёпотом* Что это... Подожди, дневник Хлои?"

# game/v1/scene51.rpy:869
translate russian v1s51Chloe_PhoneContinue_0224825d:

    # u "*Whispers* Yeah. You might find something useful there, but it's locked."
    u "*Шёпотом* Да. Ты можешь найти там что-нибудь полезное, но он заперт."

# game/v1/scene51.rpy:874
translate russian v1s51Chloe_PhoneContinue_73adc116:

    # li "*Whispers* I'm sure I can get it open. This is crazy!"
    li "*Шёпотом* Я уверена, что смогу открыть его, это безумие!"

# game/v1/scene51.rpy:889
translate russian v1s51Chloe_PhoneContinue_bb53a663:

    # u "*Whispers* Anyways, I better get home. I'm getting a little tired after all that action."
    u "*Шёпотом* В общем, мне пора домой. Я немного устал после всего экшена."

