# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene43d.rpy:18
translate russian v1s43d_7a2b2045:

    # ri "Thank you for pairing with me by the way."
    ri "Кстати, спасибо, что составил мне пару."

# game/v1/scene43d.rpy:23
translate russian v1s43d_86ece32b:

    # u "Oh, of course."
    u "О, конечно."

# game/v1/scene43d.rpy:28
translate russian v1s43d_372327d1:

    # ri "I saw Imre floating around the room and I thought I was gonna have to be with him for a second, but thankfully... You came to the rescue. *Laughs*"
    ri "Я увидела Имре, бегающего по аудитории, и на секунду подумала, что мне придется составить ему пару, но, к счастью... Ты пришёл на помощь. *Смеётся*"

# game/v1/scene43d.rpy:33
translate russian v1s43d_d0934b1b:

    # u "Haha, it's no problem. He actually told me what he did and begged me to work with you so that it wasn't awkward for either of you."
    u "Хаха, это не проблема. На самом деле он рассказал мне, что он сделал, и умолял меня поработать с тобой, чтобы вам обоим не было неловко."

# game/v1/scene43d.rpy:38
translate russian v1s43d_4d86bf54:

    # ri "Ah, I see. Did he go into detail at all? About what he did?"
    ri "А, понятно. Он вообще вдавался в подробности? О том, что он сделал?"

# game/v1/scene43d.rpy:43
translate russian v1s43d_bef5d205:

    # u "No, he just said that he was drunk texting you last night and you never responded."
    u "Нет, он просто сказал, что вчера вечером по пьяни написал тебе смс, а ты не ответила."

# game/v1/scene43d.rpy:48
translate russian v1s43d_797a5f53:

    # ri "Well, I hope he was drunk... 'cause that was some harsh shit, ha."
    ri "Надеюсь, он был пьян... потому что это было грубое дерьмо, ха."

# game/v1/scene43d.rpy:53
translate russian v1s43d_7f2e94d7:

    # u "*Sighs* Do I wanna know?"
    u "*Вздох* Я могу узнать?"

# game/v1/scene43d.rpy:58
translate russian v1s43d_6eac890d:

    # ri "You don't."
    ri "Ты не хочешь."

# game/v1/scene43d.rpy:63
translate russian v1s43d_fcc18885:

    # u "*Laughs*"
    u "*Смеётся*"

# game/v1/scene43d.rpy:68
translate russian v1s43d_e8294151:

    # ri "Let's get started on this before I get bored. I don't plan on being here long."
    ri "Давай начнём, пока мне не стало скучно. Я не планирую оставаться здесь долго."

# game/v1/scene43d.rpy:74
translate russian v1s43d_aa96b2c7:

    # u "No problem. Really though, you okay?"
    u "Без проблем. Правда, ты в порядке?"

# game/v1/scene43d.rpy:79
translate russian v1s43d_8d3fc135:

    # ri "Yeah, ha. I am now... Yeah."
    ri "Да, ха. Сейчас я... Да."

# game/v1/scene43d.rpy:84
translate russian v1s43d_3a265791:

    # u "Alright, well... What ideas did you have for the scene?"
    u "Ладно, хорошо... Какие у тебя идеи для сцены?"

# game/v1/scene43d.rpy:89
translate russian v1s43d_8720e12b:

    # ri "None, really. I just know one thing I wanna do."
    ri "Никаких, правда. Я просто знаю одну вещь, которую хочу сделать."

# game/v1/scene43d.rpy:94
translate russian v1s43d_cb6c1c75:

    # u "And that is?"
    u "И что же?"

# game/v1/scene43d.rpy:99
translate russian v1s43d_e9b07e0b:

    # ri "*Clears throat* Drumroll please!"
    ri "*Прочистила горло* Барабанную дробь, пожалуйста!"

# game/v1/scene43d.rpy:104
translate russian v1s43d_7b1c1392:

    # u "..."
    u "..."

# game/v1/scene43d.rpy:109
translate russian v1s43d_c5da343c:

    # ri "Please?"
    ri "Пожалуйста?"

# game/v1/scene43d.rpy:114
translate russian v1s43d_61693fc1:

    # u "C'mon already, weirdo. I thought you didn't wanna be here long? *Chuckles*"
    u "Давай уже, чудачка. Я думал, ты не хочешь быть здесь долго? *Хихикает*"

# game/v1/scene43d.rpy:119
translate russian v1s43d_d42df6cd:

    # ri "*Sighs* You're no fun..."
    ri "*Вздыхает* Ты не весёлый..."

# game/v1/scene43d.rpy:121
translate russian v1s43d_2edfa4cc:

    # ri "The one thing I really wanna do is improv."
    ri "Единственное, чего я хочу - это импровизировать."

# game/v1/scene43d.rpy:126
translate russian v1s43d_f656edf3:

    # u "You mean you wanna wing it? *Chuckles* Be lazy and come up with nothing?"
    u "Ты хочешь сказать, что хочешь сделать это наобум? *Хихикает* Лениться и ничего не придумывать?"

# game/v1/scene43d.rpy:131
translate russian v1s43d_24c21af4:

    # ri "Well... We can come up with a premise."
    ri "Ну... Мы можем придумать предпосылку."

# game/v1/scene43d.rpy:136
translate russian v1s43d_b6c00206:

    # u "Explain..."
    u "Объясни..."

# game/v1/scene43d.rpy:141
translate russian v1s43d_b8f6cc12:

    # ri "First off, I'm the queen. So obviously, I expect to be treated like one."
    ri "Во-первых, я - королева. Поэтому, очевидно, я ожидаю, что со мной будут обращаться как с королевой."

# game/v1/scene43d.rpy:146
translate russian v1s43d_b53bf97a:

    # u "Ha, okay. And how is that?"
    u "Ха, хорошо. И как это?"

# game/v1/scene43d.rpy:152
translate russian v1s43d_9c535bd9:

    # ri "Well, the King thinks that above all, the Queen should always be treated."
    ri "Король считает, что, прежде всего, с королевой всегда нужно обращаться."

# game/v1/scene43d.rpy:157
translate russian v1s43d_9d0ef8a3:

    # u "*Gulp* Always?"
    u "*Глоток* Всегда?"

# game/v1/scene43d.rpy:162
translate russian v1s43d_f5226c6b:

    # ri "*Laughs* Yeah, always."
    ri "*Смеётся* Да, всегда."

# game/v1/scene43d.rpy:167
translate russian v1s43d_6e2045ff:

    # ri "*Whispers* And he loves treating her just as much as she loves being treated..."
    ri "*Шёпотом* И он любит обращаться с ней так же сильно, как она любит, когда с ней обращаются..."

# game/v1/scene43d.rpy:172
translate russian v1s43d_197f382f:

    # u "Hmm... Okay, I think I understand. (This scene is gonna be hot as fuck...)"
    u "Хм... Ладно, думаю, я понимаю. (Эта сцена будет горячей, блядь...)"

# game/v1/scene43d.rpy:177
translate russian v1s43d_2057c186:

    # ri "*Chuckles*"
    ri "*Хихикает*"

# game/v1/scene43d.rpy:183
translate russian v1s43d_f85b5ff8:

    # ri "I don't need to explain myself to you! King or not, you will do as the Queen demands. And that's final."
    ri "Мне не нужно объясняться с тобой! Король ты или нет, но ты будешь делать то, что требует королева. И это окончательно."

# game/v1/scene43d.rpy:188
translate russian v1s43d_f3130810:

    # u "Oh, shit... How the hell am I supposed to compete with an A-list actress?"
    u "Ох, черт... Как, чёрт возьми, я могу конкурировать с актрисой из списка А?"

# game/v1/scene43d.rpy:193
translate russian v1s43d_cf5c7ea1:

    # ri "*Laughs*"
    ri "*Смеётся*"

# game/v1/scene43d.rpy:198
translate russian v1s43d_15c346fc:

    # ri "I really wanna make the whole class squirm, and Mr. Lee too for hitting me with that bomb today."
    ri "Я очень хочу заставить весь класс корчиться, и мистера Ли тоже, за то, что он сегодня запустил в меня бомбой."

# game/v1/scene43d.rpy:203
translate russian v1s43d_d6380cb0:

    # u "Ha, okay. How are you planning to do that?"
    u "Ха, хорошо. Как ты планируешь это сделать?"

# game/v1/scene43d.rpy:208
translate russian v1s43d_54f5d168:

    # ri "You couldn't tell? Obviously I wanna do a little sub/dom action."
    ri "Ты не мог сказать? Очевидно, что я хочу немного заняться саб/дом."

# game/v1/scene43d.rpy:217
translate russian v1s43d_d26c21c9:

    # u "Well if that's the case, I'm the dominant."
    u "Ну, если это так, то я доминант."

# game/v1/scene43d.rpy:222
translate russian v1s43d_2b28720e:

    # ri "Oh... You like taking control, huh?"
    ri "Ох... Тебе нравится всё контролировать, да?"

# game/v1/scene43d.rpy:230
translate russian v1s43d_a4ff39d2:

    # u "If that's the case, I'll play the submissive."
    u "В таком случае я буду играть роль сабмиссива."

# game/v1/scene43d.rpy:235
translate russian v1s43d_3e98df23:

    # ri "You like being told what to do, huh?"
    ri "Тебе нравится, когда тебе говорят, что делать, да?"

# game/v1/scene43d.rpy:240
translate russian v1s43d_b25d5702:

    # u "Haha. Umm..."
    u "Хаха. Ммм..."

# game/v1/scene43d.rpy:243
translate russian v1s43d_5a5b8457:

    # u "You already know the answer to that question, I think."
    u "Думаю, ты уже знаешь ответ на этот вопрос."

# game/v1/scene43d.rpy:253
translate russian v1s43d_a6ed3529:

    # ri "Haha, that I do..."
    ri "Хаха, это так..."

# game/v1/scene43d.rpy:258
translate russian v1s43d_9abe18e1:

    # ri "Okay, nevermind. Focus!"
    ri "Ладно, не бери в голову. Сосредоточься!"

# game/v1/scene43d.rpy:263
translate russian v1s43d_f6031cf2:

    # u "Yes, my Queen."
    u "Да, моя королева."

# game/v1/scene43d.rpy:268
translate russian v1s43d_c1a09845:

    # ri "Good start."
    ri "Хорошее начало."

# game/v1/scene43d.rpy:270
translate russian v1s43d_9514a71e:

    # ri "Also, we'll have to include at least something about history so that we don't mess it up by giving Mr. Lee the chance to interrupt."
    ri "Кроме того, нам придется включить хотя бы что-нибудь об истории, чтобы не испортить сцену, дав мистеру Ли возможность прервать её."

# game/v1/scene43d.rpy:275
translate russian v1s43d_99bd25a7:

    # u "*Laughs* Fair enough."
    u "*Смеётся* Справедливо."

# game/v1/scene43d.rpy:280
translate russian v1s43d_b24cb673:

    # ri "Alright, so... We're good for next week then?"
    ri "Ладно, итак... Значит, на следующей неделе у нас всё хорошо?"

# game/v1/scene43d.rpy:285
translate russian v1s43d_87be9757:

    # u "Haha, yep. We're all good."
    u "Хаха, да. У нас всё хорошо."

# game/v1/scene43d.rpy:290
translate russian v1s43d_1a8bbbb8:

    # ri "Great, I gotta go chat with Aubrey. Catch you later?"
    ri "Отлично, мне нужно поболтать с Обри. Увидимся позже?"

# game/v1/scene43d.rpy:296
translate russian v1s43d_3362403b:

    # u "Don't have too much fun without me. *Chuckles*"
    u "Не слишком веселись без меня. *Хихикает*"

# game/v1/scene43d.rpy:301
translate russian v1s43d_49cdb406:

    # ri "No promises, hehe."
    ri "Не обещаю, хехе."

# game/v1/scene43d.rpy:306
translate russian v1s43d_9d24938a:

    # u "*Laughs* (She can't get enough of her Aubs, can she?)"
    u "*Смеётся* (Она не может насытиться Обри, не так ли?)"

# game/v1/scene43d.rpy:311
translate russian v1s43d_9475b827:

    # u "Later!"
    u "Позже!"

# game/v1/scene43d.rpy:317
translate russian v1s43d_acc8f678:

    # u "Yeah, later."
    u "Да, позже."

# game/v1/scene43d.rpy:322
translate russian v1s43d_e7f7bc19:

    # u "(Should've known she'd choose to do the whole thing on a whim...)"
    u "(Надо было догадаться, что она решит сделать все это по прихоти...)"

# game/v1/scene43d.rpy:323
translate russian v1s43d_2be99962:

    # u "(As long as I get a passing grade, she can have as much fun as she wants with the class, haha.)"
    u "(Пока я получаю проходной балл, она может развлекаться с классом сколько угодно, хаха)."

translate russian strings:

    # game/v1/scene43d.rpy:210
    old "I'm dominant"
    new "Я доминирую"

    # game/v1/scene43d.rpy:210
    old "I'm submissive"
    new "Я покорный"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene43d.rpy:18
translate russian v1s43d_5753b9a4:

    # ri "Thank you for pairing with me, by the way."
    ri "Кстати, спасибо, что составил мне пару."

# game/v1/scene43d.rpy:28
translate russian v1s43d_2d0305ae:

    # ri "I saw Imre floating around the room and I thought I was gonna have to be with him for a second, but thankfully... you came to the rescue."
    ri "Я увидела Имре, бегающего по аудитории, и на секунду подумала, что мне придется составить ему пару, но, к счастью... Ты пришёл на помощь."

# game/v1/scene43d.rpy:33
translate russian v1s43d_c7666659:

    # u "It's no problem. He actually told me what he did and begged me to work with you so that it wasn't awkward for either of you."
    u "Это не проблема. Он на самом деле рассказал мне, что он сделал, и умолял меня поработать с вами, чтобы вам обоим не было неловко."

# game/v1/scene43d.rpy:63
translate russian v1s43d_46b6e599:

    # u "Got it."
    u "Понял."

# game/v1/scene43d.rpy:79
translate russian v1s43d_e367f220:

    # ri "Yeah, ha. I am now... yeah."
    ri "Да, ха. Сейчас я... да."

# game/v1/scene43d.rpy:99
translate russian v1s43d_3fe000b7:

    # ri "*Clears throat* Drumroll, please!"
    ri "*Прочистила горло* Барабанную дробь, пожалуйста!"

# game/v1/scene43d.rpy:114
translate russian v1s43d_aadb9ce3:

    # u "Ha, c'mon already, weirdo. I thought you didn't wanna be here long?"
    u "Ха, давай уже, чудачка. Я думал, ты не хочешь быть здесь долго?"

# game/v1/scene43d.rpy:126
translate russian v1s43d_62dddef3:

    # u "You mean you wanna wing it? Be lazy and come up with nothing?"
    u "Ты хочешь сказать, что хочешь сделать это наобум? Лениться и ничего не придумывать?"

# game/v1/scene43d.rpy:131
translate russian v1s43d_648bc37b:

    # ri "Well... we can come up with a premise."
    ri "Ну... Мы можем придумать предпосылку."

# game/v1/scene43d.rpy:146
translate russian v1s43d_875325dc:

    # u "Okay. And how is that?"
    u "Хорошо. И как это?"

# game/v1/scene43d.rpy:152
translate russian v1s43d_196ba007:

    # ri "Well, the king thinks that above all, the queen should always be treated."
    ri "Король считает, что, прежде всего, с королевой всегда нужно обращаться."

# game/v1/scene43d.rpy:162
translate russian v1s43d_ba734852:

    # ri "Yeah, always."
    ri "Да, всегда."

# game/v1/scene43d.rpy:172
translate russian v1s43d_98cddfb4:

    # u "Hmm... okay, I think I understand. (This scene is gonna be hot as fuck...)"
    u "Хм... Ладно, думаю, я понимаю. (Эта сцена будет горячей, блядь...)"

# game/v1/scene43d.rpy:177
translate russian v1s43d_a3bf8c92:

    # ri "Mmm, good."
    ri "Ммм, хорошо."

# game/v1/scene43d.rpy:188
translate russian v1s43d_a5f9f709:

    # u "Oh, shit... how the hell am I supposed to compete with an A-list actress?"
    u "Ох, черт... Как, чёрт возьми, я могу конкурировать с актрисой из списка А?"

# game/v1/scene43d.rpy:193
translate russian v1s43d_03e8f121:

    # ri "Ha!"
    ri "Ха!"

# game/v1/scene43d.rpy:198
translate russian v1s43d_f0d69703:

    # ri "I really wanna make the whole class squirm. And Mr. Lee, too, for hitting me with that bomb today."
    ri "Я очень хочу заставить весь класс корчиться, и мистера Ли тоже, за то, что он сегодня запустил в меня бомбой."

# game/v1/scene43d.rpy:203
translate russian v1s43d_fe2cf6ac:

    # u "How are you planning to do that?"
    u "Как ты планируешь это сделать?"

# game/v1/scene43d.rpy:240
translate russian v1s43d_46a425b3:

    # u "Wellll, umm..."
    u "Ну ладно, ммм..."

# game/v1/scene43d.rpy:253
translate russian v1s43d_4b0fc063:

    # ri "That I do..."
    ri "Это так..."

# game/v1/scene43d.rpy:263
translate russian v1s43d_a2ec139a:

    # u "Yes, my queen."
    u "Да, моя королева."

# game/v1/scene43d.rpy:270
translate russian v1s43d_88ae8f3f:

    # ri "Also, we'll have to include something linked to history so that we don't give Mr. Lee a reason to interrupt."
    ri "Кроме того, нам придется включить хотя бы что-нибудь об истории, чтобы не испортить сцену, дав мистеру Ли возможность прервать её."

# game/v1/scene43d.rpy:275
translate russian v1s43d_5e38262e:

    # u "Fair enough."
    u "Справедливо."

# game/v1/scene43d.rpy:280
translate russian v1s43d_21bb6518:

    # ri "Alright, so... we're good for next week, then?"
    ri "Ладно, итак... Значит, на следующей неделе у нас всё хорошо?"

# game/v1/scene43d.rpy:285
translate russian v1s43d_69cc4711:

    # u "Yep. We're all good."
    u "Да. У нас всё хорошо."

# game/v1/scene43d.rpy:296
translate russian v1s43d_61ad5939:

    # u "Don't have too much fun without me."
    u "Не слишком веселись без меня."

# game/v1/scene43d.rpy:301
translate russian v1s43d_2b2fb3e7:

    # ri "No promises!"
    ri "Не обещаю!"

# game/v1/scene43d.rpy:306
translate russian v1s43d_528e081b:

    # u "(She can't get enough of her Aubs, can she?)"
    u "(Она не может насытиться Обри, не так ли?)"

# game/v1/scene43d.rpy:323
translate russian v1s43d_f5cc5138:

    # u "(As long as I get a passing grade, she can have as much fun as she wants with the class.)"
    u "(Пока я получаю проходной балл, она может развлекаться с классом сколько угодно.)"

