# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene44.rpy:32
translate russian v1s44_990bd683:

    # u "So you're supporting Chloe???"
    u "Так ты поддерживаешь Хлою?????"

# game/v1/scene44.rpy:47
translate russian v1s44_f957467d:

    # el "Yes. She's the logical choice."
    el "Да. Она - логичный выбор."

# game/v1/scene44.rpy:52
translate russian v1s44_b8fbb209:

    # u "What makes you say that?"
    u "Почему ты так говоришь?"

# game/v1/scene44.rpy:57
translate russian v1s44_ce3dcacb:

    # el "She's already President, so she's experienced. Again, logical choice."
    el "Она уже была президентом, так что у неё есть опыт. Опять же, логичный выбор."

# game/v1/scene44.rpy:62
translate russian v1s44_5cc79439:

    # u "Is logic the only reason why you vote for things? *Chuckles*"
    u "Логика - это единственная причина, по которой ты голосуешь за те или иные вещи? *Хихикает*"

# game/v1/scene44.rpy:67
translate russian v1s44_d2a1b2f9:

    # el "Why should we vote any differently? If the choice doesn't make logical sense, then why would I choose it?"
    el "Почему мы должны голосовать по-другому? Если выбор не имеет логического смысла, то почему я должен выбирать?"

# game/v1/scene44.rpy:72
translate russian v1s44_101c876c:

    # u "New opportunities... Personal feelings... Etc?"
    u "Новые возможности... Личные чувства... И т.д.?"

# game/v1/scene44.rpy:77
translate russian v1s44_ce893dff:

    # el "Nah, I'm sticking with Chloe. I trust tested waters over new boots."
    el "Нет, я останусь с Хлоей. Я доверяю проверенной воде, а не новым ботинкам."

# game/v1/scene44.rpy:82
translate russian v1s44_fcc18885:

    # u "*Laughs*"
    u "*Смеётся*"

# game/v1/scene44.rpy:87
translate russian v1s44_dd6b8acd:

    # el "What?"
    el "Что?"

# game/v1/scene44.rpy:92
translate russian v1s44_07d4206a:

    # u "Just the way you said that, it was funny."
    u "Просто то, как ты это сказал, было смешно."

# game/v1/scene44.rpy:97
translate russian v1s44_10ef6153:

    # el "*Scoffs* Anyway... Who are you supporting?"
    el "*С насмешкой* В таком случае... Кого ты поддерживаешь?"

# game/v1/scene44.rpy:109
translate russian v1s44_d0141848:

    # u "I'm also supporting Chloe."
    u "Я тоже поддерживаю Хлою."

# game/v1/scene44.rpy:115
translate russian v1s44_703460ca:

    # el "See? Good, logical choice."
    el "Видишь? Хороший, логичный выбор."

# game/v1/scene44.rpy:120
translate russian v1s44_e3767532:

    # u "Ha. Yeah."
    u "Ха. Да."

# game/v1/scene44.rpy:130
translate russian v1s44_6d7995b4:

    # u "I'm gonna see what Lindsey has to offer."
    u "Я собираюсь посмотреть, что может предложить Линдси."

# game/v1/scene44.rpy:135
translate russian v1s44_5f3a155f:

    # el "*Scoffs* Good luck with that."
    el "*Усмехается* Удачи с этим."

# game/v1/scene44.rpy:140
translate russian v1s44_101a0289:

    # u "What do you have against her?"
    u "Что ты имеешь против неё?"

# game/v1/scene44.rpy:145
translate russian v1s44_1d2c61df:

    # el "It's not that I have anything against her, dude."
    el "Не то чтобы я имел что-то против неё, чувак."

# game/v1/scene44.rpy:147
translate russian v1s44_baa9a6da:

    # el "I'm used to Chloe being President, you know?"
    el "Я привык, что Хлоя - президент, понимаешь?"

# game/v1/scene44.rpy:152
translate russian v1s44_68e51484:

    # u "Yeah, I think everyone is. That's why most people are voting for Lindsey, ha."
    u "Да, думаю, все так думают. Вот почему большинство людей голосуют за Линдси, ха."

# game/v1/scene44.rpy:158
translate russian v1s44_b6304111:

    # el "Hmm..."
    el "Хм..."

# game/v1/scene44.rpy:175
translate russian v1s44_6a1de0e6:

    # elm "Elijah, are you coming? I've been waiting over there for-"
    elm "Элай, ты идешь? Я ждала там уже несколько-"

# game/v1/scene44.rpy:180
translate russian v1s44_7df87120:

    # el "Yes mom, I'm coming. I was just talking to-"
    el "Да, мама, иду. Я разговаривал с-"

# game/v1/scene44.rpy:185
translate russian v1s44_55f8999e:

    # elm "Wait a minute..."
    elm "Подожди минуту..."

# game/v1/scene44.rpy:195
translate russian v1s44_055a46f7:

    # elm "You didn't tell me you had a friend with you?!"
    elm "Ты не говорил мне, что с тобой был друг?!"

# game/v1/scene44.rpy:200
translate russian v1s44_420d299b:

    # el "I don't..."
    el "Я не..."

# game/v1/scene44.rpy:205
translate russian v1s44_7daa96fd:

    # elm "Well, for fuck's sake Eli. It wouldn't kill you to try and make one."
    elm "Ну, ради бога, Элай. Это не убьет тебя, если ты попробуешь его сделать."

# game/v1/scene44.rpy:210
translate russian v1s44_b84514a9:

    # el "*Sighs* Can we just go? Come on. Get back in the car before more people see you..."
    el "*Вздыхает* Мы можем просто уйти? Давай. Возвращайся в машину, пока тебя не увидели другие."

# game/v1/scene44.rpy:215
translate russian v1s44_df04c0e6:

    # elm "I apologize for my son, he can be rude from time to time. It was nice meeting you..."
    elm "Прошу прощения за своего сына, он может быть грубым время от времени. Было приятно познакомиться с вами..."

# game/v1/scene44.rpy:223
translate russian v1s44_906208de:

    # u "[name]."
    u "[name]."

# game/v1/scene44.rpy:228
translate russian v1s44_b5e335cf:

    # elm "[name], very good."
    elm "[name], очень хорошо."

# game/v1/scene44.rpy:233
translate russian v1s44_3c7a9c9f:

    # el "*Sighs*"
    el "*Вздыхает*"

# game/v1/scene44.rpy:255
translate russian v1s44_15931e4f:

    # u "[name]. Eli's friend."
    u "[name]. Друг Элая."

# game/v1/scene44.rpy:280
translate russian v1s44_dfbae6a0:

    # elm "[name]. I'll remember that."
    elm "[name]. Я запомню это."

# game/v1/scene44.rpy:285
translate russian v1s44_892c559d:

    # u "I hope so."
    u "Надеюсь, что да."

# game/v1/scene44.rpy:305
translate russian v1s44_8d37e3a3:

    # elm "Alright, we're leaving. Bye, [name]!"
    elm "Хорошо, мы уходим. Пока, [name]!"

# game/v1/scene44.rpy:319
translate russian v1s44_9f4e5ec5:

    # u "(I did not expect Elijah's mom to look like that. Damn...)"
    u "(Я не ожидал, что мама Элая будет выглядеть так. Чёрт...)"

# game/v1/scene44.rpy:344
translate russian v1s44_8c047491:

    # u "WHAT THE FUCK?! Imre!"
    u "КАКОГО ХРЕНА?! Имре!"

# game/v1/scene44.rpy:346
translate russian v1s44_ceacdc9f:

    # u "Why the fuck are you running up on me like that?!"
    u "Какого хрена ты ко мне так подбегаешь?!"

# game/v1/scene44.rpy:351
translate russian v1s44_a125cd67:

    # imre "*Laughs* Sorry, bro! My bad, my bad..."
    imre "*Смеётся* Прости, бро! Виноват, виноват..."

# game/v1/scene44.rpy:353
translate russian v1s44_8f7c0e4e:

    # imre "Now, who the FUCK was that with Elijah? His sister?"
    imre "А кто, блядь, был с Элаем? Его сестра?"

# game/v1/scene44.rpy:358
translate russian v1s44_04a2245a:

    # u "*Chuckles* Nah, dude. Try his Mom."
    u "*Хихикает* Не, чувак. Это его мама."

# game/v1/scene44.rpy:363
translate russian v1s44_33211fab:

    # imre "His Mom?! You're fucking kidding-"
    imre "Его мама?! Ты, блядь, шутишь.."

# game/v1/scene44.rpy:365
translate russian v1s44_e8cd660b:

    # imre "Her? The one that just got in the car?"
    imre "Она? Та, которая только что села в машину?"

# game/v1/scene44.rpy:370
translate russian v1s44_52bdc68c:

    # u "Haha, yes. Her."
    u "Хаха, да. Она."

# game/v1/scene44.rpy:375
translate russian v1s44_e5d759dd:

    # imre "That's the finest mom I've ever seen, [name]."
    imre "Это самая лучшая мама, которую я когда-либо видел, [name]."

# game/v1/scene44.rpy:384
translate russian v1s44_8395e69d:

    # u "She was pretty fucking hot. *Laughs*"
    u "Она была чертовски сексуальна. *Смеётся*"

# game/v1/scene44.rpy:395
translate russian v1s44_0880f098:

    # u "Meh, she's not really my type."
    u "Да, она не в моем вкусе."

# game/v1/scene44.rpy:400
translate russian v1s44_7fbf823b:

    # imre "Are you kidding me???"
    imre "Ты что, шутишь????"

# game/v1/scene44.rpy:402
translate russian v1s44_964e3586:

    # imre "I'm gonna have to apologize to my mom..."
    imre "Мне придется извиниться перед моей мамой..."

# game/v1/scene44.rpy:407
translate russian v1s44_22f9cf2c:

    # u "Why? *Chuckles*"
    u "Почему? *Хихикает*"

# game/v1/scene44.rpy:412
translate russian v1s44_6a8dbb7f:

    # imre "'Cause I'm gonna have a new mommy here soon. Haha!"
    imre "Потому что скоро у меня здесь будет новая мамочка. Хаха!"

# game/v1/scene44.rpy:417
translate russian v1s44_6f826349:

    # u "Oh my god, no. *Laughs* Imre."
    u "О боже, нет. *Смеётся* Имре."

# game/v1/scene44.rpy:419
translate russian v1s44_3b83cba4:

    # u "Don't say it like that. Ever again."
    u "Не говори так. Никогда больше."

# game/v1/scene44.rpy:424
translate russian v1s44_7dca4922:

    # imre "It's the truth, bro."
    imre "Это правда, бро."

# game/v1/scene44.rpy:429
translate russian v1s44_34ec2b8b:

    # u "*Laughs* Okay. Then tell me another truth, what did you send-"
    u "*Смеется* Хорошо. Тогда скажи мне другую правду, что ты послал..."

# game/v1/scene44.rpy:434
translate russian v1s44_8869efff:

    # imre "Sorry, [name]! I'll catch you later. I've got something fun planned and I'm about to be late."
    imre "Прости, [name]! Я поймаю тебя позже. У меня запланировано кое-что интересное, и я опоздаю."

# game/v1/scene44.rpy:439
translate russian v1s44_6ef758af:

    # u "(Forgot all about the Riley thing, I guess? Either that or he's avoiding the hell out of it, ha.)"
    u "(Наверное, совсем забыл о том, что случилось с Райли? Либо так, либо он избегает всего этого, ха.)"

translate russian strings:

    # game/v1/scene44.rpy:99
#    old "Chloe"
#    new "Хлоя"

    # game/v1/scene44.rpy:99
#    old "Lindsey"
#    new "Линдси"

    # game/v1/scene44.rpy:217
    old "Introduce yourself"
    new "Представься"

    # game/v1/scene44.rpy:217
    old "I'm Eli's friend"
    new "Я друг Элая"

    # game/v1/scene44.rpy:380
    old "She was pretty hot"
    new "Она довольно горячая"

    # game/v1/scene44.rpy:380
    old "Meh, not my type"
    new "Нет, не в моём вкусе"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene44.rpy:57
translate russian v1s44_316d0daf:

    # el "She's already president, so she's experienced. Again, logical choice."
    el "Она уже была президентом, так что у неё есть опыт. Опять же, логичный выбор."

# game/v1/scene44.rpy:62
translate russian v1s44_860579d7:

    # u "Is logic the only reason why you vote for things?"
    u "Логика - это единственная причина, по которой ты голосуешь за те или иные вещи?"

# game/v1/scene44.rpy:72
translate russian v1s44_3662f636:

    # u "New opportunities... personal feelings... other stuff, you know?"
    u "Новые возможности... Личные чувства... другие вещи, понимаешь?"

# game/v1/scene44.rpy:82
translate russian v1s44_437382fd:

    # u "Ha, okay, sure."
    u "Ха, хорошо, конечно."

# game/v1/scene44.rpy:147
translate russian v1s44_f66e7457:

    # el "I'm used to Chloe being president, you know?"
    el "Я привык, что Хлоя - президент, понимаешь?"

# game/v1/scene44.rpy:152
translate russian v1s44_933d66dd:

    # u "Yeah, I think everyone is. That's why most people are voting for Lindsey."
    u "Да, думаю, все так думают. Вот почему большинство людей голосуют за Линдси, ха."

# game/v1/scene44.rpy:350
translate russian v1s44_15ef5a49:

    # imre "Sorry, bro! My bad, my bad..."
    imre "Прости, бро! Виноват, виноват..."

# game/v1/scene44.rpy:357
translate russian v1s44_a4e326b7:

    # u "Nah, dude. Try his mom."
    u "Не, чувак. Это его мама."

# game/v1/scene44.rpy:362
translate russian v1s44_ffc6c189:

    # imre "His mom?! You're fucking kidding-"
    imre "Его мама?! Ты, блядь, шутишь-"

# game/v1/scene44.rpy:369
translate russian v1s44_7b41eee7:

    # u "Ohhhh yeah. Her."
    u "Оооо, да. Это так."

# game/v1/scene44.rpy:383
translate russian v1s44_9fa86d66:

    # u "She was pretty fucking hot."
    u "Она была чертовски сексуальна."

# game/v1/scene44.rpy:406
translate russian v1s44_48c99239:

    # u "Why?"
    u "Почему?"

# game/v1/scene44.rpy:411
translate russian v1s44_f497ac11:

    # imre "'Cause I'm gonna have a new mommy here soon."
    imre "Потому что скоро у меня здесь будет новая мамочка."

# game/v1/scene44.rpy:416
translate russian v1s44_7abf0c12:

    # u "Ha! Oh my God, no. Imre. You're killing me."
    u "Ха! О Боже, нет. Имре. Ты убиваешь меня."

# game/v1/scene44.rpy:423
translate russian v1s44_9b2bc0c3:

    # imre "Mommy milkers or bust, bro. In fact, make that \"AND bust.\" I'm not ashamed, it's the truth."
    imre "Мамочкины дойки или бюст, бро. На самом деле, пусть будет \"и бюст\". Мне не стыдно, это правда."

# game/v1/scene44.rpy:428
translate russian v1s44_b62243d0:

    # u "Okay, bud. Then tell me another truth, what did you send-"
    u "Хорошо, дружище. Тогда скажи мне другую правду, что ты послал..."

# TODO: Translation updated at 2023-04-11 15:01

# game/v1/scene44.rpy:301
translate russian v1s44_3054bacc:

    # elm "All right, we're leaving. Bye, [name]!"
    elm "Хорошо, мы уходим. Пока, [name]!"

