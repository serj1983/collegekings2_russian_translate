# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene22.rpy:27
translate russian v1s22_2dbeaba7:

    # u "This is an office right here!"
    u "Этот офис прямо здесь!"

# game/v1/scene22.rpy:37
translate russian v1s22_ffcd966f:

    # li "Hey, you! Thanks for coming. You like the spot, huh?"
    li "Эй! Спасибо, что пришёл. Тебе нравится место, да?"

# game/v1/scene22.rpy:42
translate russian v1s22_4123e3fe:

    # u "Hell yeah, I love this."
    u "Черт возьми, да, мне нравится."

# game/v1/scene22.rpy:44
translate russian v1s22_8ec6c897:

    # u "The real question is, how did you get permission to use it?"
    u "Главный вопрос, как ты получила разрешение на его использование?"

# game/v1/scene22.rpy:49
translate russian v1s22_4f6ed81c:

    # li "I did a little favor for the janitor and now..."
    li "Я оказал небольшую услугу уборщику и теперь..."

# game/v1/scene22.rpy:51
translate russian v1s22_c4c9c87a:

    # li "Well, let's just say he's a big Lindsey supporter now."
    li "Ну, скажем так, теперь он большой сторонник Линдси."

# game/v1/scene22.rpy:56
translate russian v1s22_1fb9efb2:

    # u "Wait... Lindsey, did you... You know...?"
    u "Подожди... Линдси, ты... Ты знаешь...?"

# game/v1/scene22.rpy:66
translate russian v1s22_b3692b10:

    # li "God, no! *Laughs*"
    li "Боже, нет! *Смеётся*"

# game/v1/scene22.rpy:71
translate russian v1s22_ff703841:

    # u "*Whispers* Thank fucking god..."
    u "*Шёпотом* Слава богу, блядь..."

# game/v1/scene22.rpy:76
translate russian v1s22_5d302df2:

    # li "Mr. Rose and I helped the janitor get all of his kids into SVC for free."
    li "Мы с мистером Роузом помогли уборщику устроить всех его детей в SVC бесплатно."

# game/v1/scene22.rpy:81
translate russian v1s22_0e3df597:

    # u "Oh... Wow, Lindsey."
    u "О... Вау, Линдси."

# game/v1/scene22.rpy:86
translate russian v1s22_c95bc0dd:

    # li "I just helped them with their applications but Mr. Rose dealt with all the admission stuff."
    li "Я просто помогала им подавать документы, а мистер Роуз занимался всеми вопросами, связанных с поступлением."

# game/v1/scene22.rpy:91
translate russian v1s22_65ebf88e:

    # u "\"Just helped\"? You're an angel, Lindsey!"
    u "\"Просто помогла\"? Ты ангел, Линдси!"

# game/v1/scene22.rpy:93
translate russian v1s22_be8cfb18:

    # u "What did Mr. Rose get out of all of this?"
    u "Что получил мистер Роуз от всего этого?"

# game/v1/scene22.rpy:98
translate russian v1s22_46d65ef0:

    # li "That man's like the devil... All he said to the janitor was \"you owe me a favor\"."
    li "Этот человек похож на дьявола... Всё, что он сказал уборщику, \"ты должен мне услугу\"."

# game/v1/scene22.rpy:103
translate russian v1s22_d65c015b:

    # u "(What a weird dude...)"
    u "(Какой странный чувак...)"

# game/v1/scene22.rpy:108
translate russian v1s22_ba9c6b96:

    # li "He said it with a smile and the janitor laughed, but it really seemed like a \"deal with the devil\" type of moment..."
    li "Он сказал это с улыбкой, и уборщик рассмеялся, но это действительно выглядело как момент типа \"сделки с дьяволом\"..."

# game/v1/scene22.rpy:109
translate russian v1s22_54462e1d:

    # li "We should be careful about making deals with him in the future."
    li "Мы должны быть осторожны, заключая с ним сделки в будущем."

# game/v1/scene22.rpy:114
translate russian v1s22_24ca1be7:

    # u "That's definitely noted."
    u "Это точно подмечено."

# game/v1/scene22.rpy:130
translate russian v1s22_4949867b:

    # li "Okay, so... the reason we're here."
    li "Ладно, итак... причина, по которой мы здесь."

# game/v1/scene22.rpy:135
translate russian v1s22_a46d5982:

    # u "Planning."
    u "Планировка."

# game/v1/scene22.rpy:140
translate russian v1s22_3e38f0aa:

    # li "Bingo!"
    li "В яблочко!"

# game/v1/scene22.rpy:142
translate russian v1s22_017f6c24:

    # li "Thankfully, I've planned out most of the campaign already. I just have a few areas where I could use your opinion."
    li "К счастью, я уже спланировала большую часть кампании. У меня есть только несколько областей, где я могла бы использовать твоё умение."

# game/v1/scene22.rpy:147
translate russian v1s22_fdde8da0:

    # li "I'm gonna give you some choices and I just need to know what you feel is best for us and the campaign."
    li "Я дам тебе несколько вариантов, и мне просто нужно знать, что ты считаешь лучшим для нас и кампании."

# game/v1/scene22.rpy:152
translate russian v1s22_5ff818a9:

    # u "Roger that. Can do."
    u "Понял. Можно."

# game/v1/scene22.rpy:157
translate russian v1s22_77c10b5d:

    # li "Before we start, I wanna run you through some things because this isn't as easy as I originally thought."
    li "Прежде чем мы начнем, я хочу рассказать тебе о некоторых вещах, потому что это не так просто, как я думала вначале."

# game/v1/scene22.rpy:162
translate russian v1s22_bc51034b:

    # u "Of course, shoot."
    u "Конечно, выкладывай."

# game/v1/scene22.rpy:167
translate russian v1s22_6ff319a4:

    # li "Phase one of my plan is to secure funding. We don't have hidden funds or special resources like Chloe does."
    li "Первая фаза моего плана - это обеспечение финансирования. У нас нет скрытых фондов или специальных ресурсов, как у Хлои."

# game/v1/scene22.rpy:169
translate russian v1s22_3e9e197d:

    # li "So money is priority number one, aka \"secure funding\"."
    li "Поэтому деньги - это приоритет номер один, оно же \"надежное финансирование\"."

# game/v1/scene22.rpy:174
translate russian v1s22_05a2892a:

    # u "Gotcha, we have to actually put in some work."
    u "Нам придется потрудиться."

# game/v1/scene22.rpy:179
translate russian v1s22_f98d8442:

    # li "Right, so with that in mind now just have a look at my board and choose what you think is best."
    li "Итак, теперь посмотри на мою доску и выбери то, что ты считаешь лучшим."

# game/v1/scene22.rpy:181
translate russian v1s22_11ab5f31:

    # li "Both plans work and I'm comfortable with either one so... Whatever you decide is what I'll go with."
    li "Оба плана работают, и меня устраивает любой из них, так что... Что бы ты ни решил, я соглашусь."

# game/v1/scene22.rpy:186
translate russian v1s22_6560969f:

    # u "That's some heavy pressure, Linds."
    u "Это серьезное давление, Линдс."

# game/v1/scene22.rpy:191
translate russian v1s22_406434fe:

    # li "I trust you with my life, [name]. Now, choose."
    li "Я доверяю тебе свою жизнь, [name]. Теперь выбирай."

# game/v1/scene22.rpy:196
translate russian v1s22_c424203a:

    # u "*Gulps*"
    u "*Глоток*"

# game/v1/scene22.rpy:253
translate russian v1s22_sell_car_6ba4a140:

    # li "Haha, good choice."
    li "Хаха, хороший выбор."

# game/v1/scene22.rpy:258
translate russian v1s22_sell_car_7b245da7:

    # u "What's so funny?"
    u "Что тут смешного?"

# game/v1/scene22.rpy:264
translate russian v1s22_sell_car_8bc17536:

    # li "I just kinda figured that you wouldn't have chosen the other option. Based on your relationship with her, you know?"
    li "Я просто подумала, что ты не выбрал бы другой вариант. Исходя из ваших с ней отношений, да?"

# game/v1/scene22.rpy:266
translate russian v1s22_sell_car_8574be76:

    # li "No worries though, we should get a good chunk of cash either way."
    li "Не беспокойся, мы в любом случае получим хорошую сумму денег."

# game/v1/scene22.rpy:271
translate russian v1s22_sell_car_155aed1f:

    # u "Yeah, I think we'll be alright. I'm quite confident in myself."
    u "Да, думаю, всё будет хорошо. Я уверен в себе."

# game/v1/scene22.rpy:277
translate russian v1s22_sell_car_72e4bbf4:

    # li "I was wondering if that second plan was a little too much, haha. Now I have the answer. *Chuckles*"
    li "Я думала, не слишком ли много для второго плана, хаха. Теперь у меня есть ответ. *Хихикает*"

# game/v1/scene22.rpy:282
translate russian v1s22_sell_car_30ca127d:

    # u "*Laughs* I mean, you gotta do whatcha gotta do to win, right?"
    u "*Смеётся* Я имею в виду, ты должна делать то, что должна делать, чтобы выиграть, верно?"

# game/v1/scene22.rpy:287
translate russian v1s22_sell_car_8cf7c109:

    # li "Right. But, your decision is probably the smartest of the two."
    li "Верно. Но твоё решение, вероятно, самое разумное из двух."

# game/v1/scene22.rpy:292
translate russian v1s22_sell_car_03601517:

    # u "Mmm... Probably."
    u "Ммм... Возможно."

# game/v1/scene22.rpy:300
translate russian v1s22_steal_2e307620:

    # li "Haha! This is going to be insane..."
    li "Хаха! Это будет безумие..."

# game/v1/scene22.rpy:305
translate russian v1s22_steal_d5924503:

    # u "It's a really smart plan, I just hope we can pull it off."
    u "Это действительно умный план, я просто надеюсь, что мы сможем его осуществить."

# game/v1/scene22.rpy:311
translate russian v1s22_steal_39a657c1:

    # li "I know, me too. And..."
    li "Я знаю, я тоже. И..."

# game/v1/scene22.rpy:313
translate russian v1s22_steal_f90fa36c:

    # li "I really do appreciate the lengths you're willing to go for me, [name]."
    li "Я очень ценю то, на что ты готов пойти ради меня, [name]."

# game/v1/scene22.rpy:318
translate russian v1s22_steal_2e27a7fb:

    # li "I wasn't exactly sure if you'd be \"all in\" when it came to helping me, you know?"
    li "Я не была уверена, что ты будешь \"полностью в теме\", когда дело дойдет до помощи мне, понимаешь?"

# game/v1/scene22.rpy:323
translate russian v1s22_steal_bb2b52e3:

    # u "I wouldn't have chosen to help you if I was too afraid to make hard decisions. My personal relationship with Chloe has nothing to do with this."
    u "Я бы не решил помогать тебе, если бы боялся принимать трудные решения. Мои личные отношения с Хлоей не имеют к этому никакого отношения."

# game/v1/scene22.rpy:329
translate russian v1s22_steal_9ab5d514:

    # u "Or you. I'm here for us. Me and you."
    u "Или тебя. Я здесь ради нас. Я и ты."

# game/v1/scene22.rpy:335
translate russian v1s22_steal_301a5aa7:

    # li "Me too, this amount of money could help us tremendously with the rest of the ideas I have for the campaign. Even though it's a hard pill to swallow..."
    li "Я тоже, эта сумма денег помогла бы нам помочь с остальными идеями, которые у меня есть для кампании. Даже если это трудная пилюля, которую нужно проглотить..."

# game/v1/scene22.rpy:340
translate russian v1s22_steal_71eeace6:

    # u "There's nothing hard to swallow, Linds. It's the Chicks' Treasury, right? To help fund the Chicks?"
    u "Нет ничего сложного, Линдс. Это казна Чикс, верно? Чтобы помочь финансировать Чикс?"

# game/v1/scene22.rpy:345
translate russian v1s22_steal_5430a5a5:

    # li "Yeah, but..."
    li "Да, но..."

# game/v1/scene22.rpy:350
translate russian v1s22_steal_a230ea19:

    # u "You're a Chick. No buts. It all adds up to me."
    u "Ты Чикс. Никаких но. Для меня всё сходится."

# game/v1/scene22.rpy:364
translate russian v1s22_end_58cff27e:

    # li "Thank you."
    li "Спасибо."

# game/v1/scene22.rpy:369
translate russian v1s22_end_1b28a406:

    # u "Of course."
    u "Конечно."

# game/v1/scene22.rpy:374
translate russian v1s22_end_80ebd9d5:

    # li "I can't say I'm very surprised by your choices, though."
    li "Не могу сказать, что я очень удивлена твоим выбором."

# game/v1/scene22.rpy:379
translate russian v1s22_end_05e8f70d:

    # u "Oh, really?"
    u "О, правда?"

# game/v1/scene22.rpy:384
translate russian v1s22_end_a6c21403:

    # li "Really."
    li "Правда."

# game/v1/scene22.rpy:389
translate russian v1s22_end_530fbe87:

    # li "Guess I've gotten to know you well enough that I can pretty much guess your opinion on most things."
    li "Думаю, я узнала тебя достаточно хорошо, чтобы угадать твоё мнение по большинству вопросов."

# game/v1/scene22.rpy:394
translate russian v1s22_end_5ca790c6:

    # u "Haha. Right... Either that, you spy on me, or you're a psychic."
    u "Хаха. Точно... Либо ты шпионишь за мной, либо ты экстрасенс."

# game/v1/scene22.rpy:399
translate russian v1s22_end_b4936a28:

    # li "Guess we'll never know."
    li "Думаю, мы никогда не узнаем."

# game/v1/scene22.rpy:401
translate russian v1s22_end_1e087f81:

    # li "Let's get out of here before the night crew comes in to work."
    li "Давайте убираться отсюда, пока ночная смена не приступила к работе."

# game/v1/scene22.rpy:406
translate russian v1s22_end_19d5aed7:

    # li "I may have permission to use this place, but I don't like being in their way when they're trying to get their job done, haha."
    li "Возможно, у меня есть разрешение на использование этого места, но мне не нравится стоять у них на пути, когда они пытаются выполнить свою работу, ха-ха."

# game/v1/scene22.rpy:411
translate russian v1s22_end_17f358ef:

    # u "Good idea."
    u "Хорошая идея."

# game/v1/scene22.rpy:421
translate russian v1s22_end_bfa1bce3:

    # u "(Always thinking of others, isn't she?)"
    u "(Всегда думает о других, не так ли?)"

