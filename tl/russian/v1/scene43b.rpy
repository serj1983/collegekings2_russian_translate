# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene43b.rpy:57
translate russian v1s43b_ae066a0a:

    # lee "Good morning, students!"
    lee "Доброе утро, студенты!"

# game/v1/scene43b.rpy:64
translate russian v1s43b_3b608183:

    # u "(What's he so excited about?)"
    u "(Чем он так взволнован?)"

# game/v1/scene43b.rpy:69
translate russian v1s43b_b44a1631:

    # lee "Today is a very good day, everyone. Although, I understand that you're unaware."
    lee "Сегодня у всех очень хороший день. Хотя, я понимаю, что вы не в курсе."

# game/v1/scene43b.rpy:74
translate russian v1s43b_99f26781:

    # ri "Mind letting us know what's happening?"
    ri "Не могли бы вы сообщить нам, что происходит?"

# game/v1/scene43b.rpy:79
translate russian v1s43b_47b74055:

    # lee "Riley! Yes! I'm so glad you asked."
    lee "Райли! Да! Я так рад, что ты спросила."

# game/v1/scene43b.rpy:84
translate russian v1s43b_8cce4a6a:

    # lee "The college has recently begun to advance its usage of the teacher assistant program and so have I."
    lee "Недавно колледж начал расширять использование программы ассистента преподавателя, и я тоже."

# game/v1/scene43b.rpy:89
translate russian v1s43b_e8464b91:

    # u "(Oh, haha... That's why Riley is here.)"
    u "(О, хаха... Вот почему Райли здесь)."

# game/v1/scene43b.rpy:94
translate russian v1s43b_e4a7433f:

    # lee "Let me introduce my new student assistant..."
    lee "Позвольте представить вам моего нового студенческого помощника..."

# game/v1/scene43b.rpy:99
translate russian v1s43b_cbec7a37:

    # u "Oh!"
    u "Ох!"

# game/v1/scene43b.rpy:104
translate russian v1s43b_d266a549:

    # lee "Riley!"
    lee "Райли!"

# game/v1/scene43b.rpy:109
translate russian v1s43b_536f3e2a:

    # ri "I, thank you... I mean, you could've just told me that's what was happening. *Chuckles* But really, thanks. Im excited!"
    ri "Спасибо... Я имею в виду, ты мог бы просто сказать мне, что так и было. *Хихикает* Но, правда, спасибо. Я в восторге!"

# game/v1/scene43b.rpy:114
translate russian v1s43b_78847f99:

    # lee "So are we. Right class?"
    lee "Как и мы. Правда класс?"

# game/v1/scene43b.rpy:119
translate russian v1s43b_ebb0294d:

    # ri "Haha, it's okay."
    ri "Хаха, всё в порядке."

# game/v1/scene43b.rpy:124
translate russian v1s43b_d35f1fd9:

    # u "(This should be interesting... haha.)"
    u "(Это должно быть интересно... хаха)."

# game/v1/scene43b.rpy:129
translate russian v1s43b_d924f118:

    # lee "Come introduce yourself to the class."
    lee "Представься классу."

# game/v1/scene43b.rpy:134
translate russian v1s43b_216a225a:

    # ri "Really? Now?"
    ri "Правда? Сейчас?"

# game/v1/scene43b.rpy:139
translate russian v1s43b_bbbb0ca0:

    # lee "Really. Now."
    lee "Правда. Сейчас."

# game/v1/scene43b.rpy:144
translate russian v1s43b_613ad5e2:

    # ri "Okay..."
    ri "Хорошо..."

# game/v1/scene43b.rpy:149
translate russian v1s43b_f398a57a:

    # ri "For all of you who don't know me, my name is Riley. I'm a freshman and a free spirit..."
    ri "Для всех, кто меня не знает, меня зовут Райли. Я первокурсница и свободная духом..."

# game/v1/scene43b.rpy:154
translate russian v1s43b_fea00bf8:

    # lee "True, tried, and tested!"
    lee "Истинно, проверено и испытано!"

# game/v1/scene43b.rpy:159
translate russian v1s43b_abf2719f:

    # ri "Ahem, I uh... I'm very approachable, so I don't know what all my responsibilities will be as a teacher's assistant, but feel free to talk to me."
    ri "Кхм, я... я очень контактная, поэтому я не знаю, каковы будут мои обязанности в качестве помощника учителя, но не стесняйтесь говорить со мной."

# game/v1/scene43b.rpy:169
translate russian v1s43b_dab92700:

    # lee "Very good! Thank you. Now, on to class..."
    lee "Очень хорошо! Спасибо. Теперь перейдем к занятиям..."

# game/v1/scene43b.rpy:174
translate russian v1s43b_256c2e50:

    # lee "Today we will once again be doing a roleplay."
    lee "Сегодня мы снова будем проводить ролевую игру."

# game/v1/scene43b.rpy:179
translate russian v1s43b_1366510c:

    # ca "*Mumbles* Fuck!"
    ca "*Бормочет* Блядь!"

# game/v1/scene43b.rpy:184
translate russian v1s43b_46346231:

    # lee "It's all about the relationship between a king and a queen during medieval times."
    lee "В ней рассказывается об отношениях между королем и королевой в средневековые времена."

# game/v1/scene43b.rpy:185
translate russian v1s43b_8105b141:

    # lee "Each gentleman will be paired with a lady, but we will need Riley to take part to make sure everyone can be paired up."
    lee "Каждый джентльмен будет в паре с леди, но нам понадобится участие Райли, чтобы убедиться, что все могут быть разбиты на пары."

# game/v1/scene43b.rpy:190
translate russian v1s43b_f23e0afd:

    # ri "Oh."
    ri "Ох."

# game/v1/scene43b.rpy:195
translate russian v1s43b_f515f860:

    # lee "Start pairing up!"
    lee "Начинайте объединяться!"

# game/v1/scene43b.rpy:215
translate russian v1s43b_b1f024dc:

    # u "Guess we were meant to work together."
    u "Видимо, нам суждено работать вместе"

# game/v1/scene43b.rpy:230
translate russian v1s43b_4f51850b:

    # imre "Hey."
    imre "Хэй."

# game/v1/scene43b.rpy:235
translate russian v1s43b_ef2f1671:

    # u "What?"
    u "Что?"

# game/v1/scene43b.rpy:240
translate russian v1s43b_c16cfaed:

    # imre "Were you getting ready to go over there and work with Penelope?"
    imre "Ты собирался пойти туда и работать с Пенелопой?"

# game/v1/scene43b.rpy:245
translate russian v1s43b_c1f8129a:

    # u "Yeah. Why?"
    u "Да. А что?"

# game/v1/scene43b.rpy:250
translate russian v1s43b_f5e83b4d:

    # imre "I was actually already planning to."
    imre "Вообще-то я планировал тоже."

# game/v1/scene43b.rpy:255
translate russian v1s43b_10ccbb51:

    # u "Why is it that you're always trying to stop me?"
    u "Почему ты всегда пытаешься остановить меня?"

# game/v1/scene43b.rpy:260
translate russian v1s43b_1c959f8e:

    # imre "Look man... Ugh, please? Just let me work with her."
    imre "Слушай, мужик... Пожалуйста? Просто дай мне поработать с ней."

# game/v1/scene43b.rpy:265
translate russian v1s43b_57a296dc:

    # u "I need an answer first. Why?"
    u "Сначала мне нужен ответ. Почему?"

# game/v1/scene43b.rpy:270
translate russian v1s43b_cb7b6ef8:

    # imre "*Sighs* Okay fine, I sent some drunk texts to Riley last night and she hasn't replied, so it'd be awkward having to work with her. You know?"
    imre "*Вздыхает* Ладно, я послал несколько пьяных сообщений Райли прошлой ночью, но она не ответила, так что будет неловко работать с ней. Понимаешь?"

# game/v1/scene43b.rpy:275
translate russian v1s43b_10f43815:

    # u "*Laughs* Yeah. I get that..."
    u "*Смеётся* Да. Я понял..."

# game/v1/scene43b.rpy:287
translate russian v1s43b_36f78d5c:

    # u "But sorry, man. This will be too good to miss."
    u "Но извини, мужик. Это будет слишком хорошо, чтобы пропустить."

# game/v1/scene43b.rpy:292
translate russian v1s43b_13f0c38a:

    # imre "Bro, please."
    imre "Бро, пожалуйста."

# game/v1/scene43b.rpy:297
translate russian v1s43b_d7c3ea57:

    # u "Nope."
    u "Нет."

# game/v1/scene43b.rpy:307
translate russian v1s43b_a3cb1b2a:

    # u "Me and you?"
    u "Мы с тобой?"

# game/v1/scene43b.rpy:312
translate russian v1s43b_bd8082b1:

    # pe "You and me?"
    pe "Я и ты?"

# game/v1/scene43b.rpy:317
translate russian v1s43b_f2538aab:

    # u "No, no, no. Me and you."
    u "Нет, нет, нет. Ты и я."

# game/v1/scene43b.rpy:323
translate russian v1s43b_937379fc:

    # pe "No, no, no. You..."
    pe "Нет, нет, нет. Ты..."

# game/v1/scene43b.rpy:328
translate russian v1s43b_9de1e5cd:

    # pe "*Whispers* And me. *Chuckles*"
    pe "*Шепотом* И я. *Хихикает*"

# game/v1/scene43b.rpy:334
translate russian v1s43b_44d4e085:

    # pe "No, no, no. You and me. *Chuckles*"
    pe "Нет, нет, нет. Ты и я. *Хихикает*"

# game/v1/scene43b.rpy:339
translate russian v1s43b_fe33d72c:

    # u "Fine... You and me."
    u "Хорошо... Ты и я."

# game/v1/scene43b.rpy:344
translate russian v1s43b_84f1c6cc:

    # pe "Yay."
    pe "Ура."

# game/v1/scene43b.rpy:354
translate russian v1s43b_ff7ee2fa:

    # lee "Everyone's paired up? Good!"
    lee "Все разбились на пары? Хорошо!"

# game/v1/scene43b.rpy:356
translate russian v1s43b_042b57fd:

    # lee "Now, once I'm finished speaking you may leave the classroom and begin reading pages 100-103 in your textbook."
    lee "Теперь, как только я закончу говорить, вы покинете аудиторию и начнёте читать страницы 100-103 вашего учебника."

# game/v1/scene43b.rpy:358
translate russian v1s43b_56fc6f0c:

    # lee "After that, you'll come up with a short scene involving a king and a queen interaction that you'll present next week to the rest of the class."
    lee "После этого вы придумаете короткую сцену взаимодействия короля и королевы, которую представите на следующей неделе остальным членам класса."

# game/v1/scene43b.rpy:364
translate russian v1s43b_6cea4070:

    # pe "Ooh, this sounds fun."
    pe "О, будет забавно."

# game/v1/scene43b.rpy:371
translate russian v1s43b_11d52f58:

    # u "Hmm? Let me check this real quick."
    u "Хм? Позволь я очень быстро проверю."

# game/v1/scene43b.rpy:411
translate russian v1s43Chloe_PhoneContinue1_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v1/scene43b.rpy:416
translate russian v1s43Chloe_PhoneContinue1_812c6164:

    # u "(Fuck... Where did that come from?)"
    u "(Блядь... Откуда это взялось?)"

# game/v1/scene43b.rpy:419
translate russian v1s43Chloe_PhoneContinue1_9929f69d:

    # u "(Fuck... I'm glad I didn't go through with posting that shit.)"
    u "(Блядь... я рад, что не стал постить это дерьмо)."

# game/v1/scene43b.rpy:424
translate russian v1s43Chloe_PhoneContinue1_82a24ae7:

    # pe "Ready to go?"
    pe "Готов к работе?"

# game/v1/scene43b.rpy:429
translate russian v1s43Chloe_PhoneContinue1_3cab5643:

    # u "*Sighs* Yeah, sorry. Where to?"
    u "*Вздох* Да, извини. Куда пойдём?"

# game/v1/scene43b.rpy:434
translate russian v1s43Chloe_PhoneContinue1_3d48716d:

    # pe "We can just go sit outside on the stairs and work, if that's cool."
    pe "Мы можем посидеть снаружи на лестнице и поработать, если ты не против."

# game/v1/scene43b.rpy:439
translate russian v1s43Chloe_PhoneContinue1_5619d9e0:

    # u "Sounds good, lead the way."
    u "Звучит хорошо, показывай путь."

# game/v1/scene43b.rpy:464
translate russian v1s43Chloe_PhoneContinue1_6cea4070:

    # pe "Ooh, this sounds fun."
    pe "О, это будет забавно."

# game/v1/scene43b.rpy:466
translate russian v1s43Chloe_PhoneContinue1_3d48716d_1:

    # pe "We can just go sit outside on the stairs and work, if that's cool."
    pe "Мы можем посидеть снаружи на лестнице и поработать, если ты не против."

# game/v1/scene43b.rpy:471
translate russian v1s43Chloe_PhoneContinue1_5619d9e0_1:

    # u "Sounds good, lead the way."
    u "Звучит хорошо, показывай путь."

# game/v1/scene43b.rpy:494
translate russian v1s43Chloe_PhoneContinue1_0513c611:

    # u "But sorry, bro..."
    u "Извини, бро..."

# game/v1/scene43b.rpy:499
translate russian v1s43Chloe_PhoneContinue1_9538cb51:

    # u "(Let's mess with him a bit.) I'm working with Penelope."
    u "(Давай немного пошутим с ним.) Я работаю с Пенелопой."

# game/v1/scene43b.rpy:504
translate russian v1s43Chloe_PhoneContinue1_13f0c38a:

    # imre "Bro, please."
    imre "Бро, пожалуйста."

# game/v1/scene43b.rpy:509
translate russian v1s43Chloe_PhoneContinue1_89955d67:

    # u "Sorry, man. What are you gonna do about the Riley situation?"
    u "Прости, мужик. Что ты собираешься делать с ситуацией с Райли?"

# game/v1/scene43b.rpy:514
translate russian v1s43Chloe_PhoneContinue1_911296f7:

    # imre "Fuck, dude... I don't know. Maybe she'll pretend like it never happened? Or should I bring it up? What the fuck should i do here?"
    imre "Блядь, чувак... Я не знаю. Может, она сделает вид, что ничего не было? Или мне стоит поднять этот вопрос? Что, блядь, мне тут делать?"

# game/v1/scene43b.rpy:519
translate russian v1s43Chloe_PhoneContinue1_3175571f:

    # u "Haha... I just wanted to see you squirm for a second. I wouldn't actually do you like that, have fun with Penelope."
    u "Хаха... Я просто хотел посмотреть, как ты будешь извиваться. На самом деле я бы не стал так с тобой поступать, повеселись с Пенелопой."

# game/v1/scene43b.rpy:524
translate russian v1s43Chloe_PhoneContinue1_887433de:

    # imre "Oh shit, haha... Thanks man."
    imre "Вот дерьмо, хаха... Спасибо, мужик."

# game/v1/scene43b.rpy:529
translate russian v1s43Chloe_PhoneContinue1_fc1e19ab:

    # u "No problem. Just be more careful, yeah?"
    u "Нет проблем. Просто будь осторожнее, хорошо?"

# game/v1/scene43b.rpy:534
translate russian v1s43Chloe_PhoneContinue1_3747a9f6:

    # imre "No shit."
    imre ""

# game/v1/scene43b.rpy:544
translate russian v1s43Chloe_PhoneContinue1_a2a9a789:

    # u "*Whisper* I'll be the \"additional gentleman\", just for you."
    u "*Шёпотом* Я буду \"дополнительным джентльменом\", только для тебя."

# game/v1/scene43b.rpy:549
translate russian v1s43Chloe_PhoneContinue1_92264795:

    # ri "*Whispers* Thank you, so so so much."
    ri "*Шепотом* Спасибо тебе большое."

# game/v1/scene43b.rpy:565
translate russian v1s43Chloe_PhoneContinue1_ff7ee2fa:

    # lee "Everyone's paired up? Good!"
    lee "Все разбились на пары? Хорошо!"

# game/v1/scene43b.rpy:567
translate russian v1s43Chloe_PhoneContinue1_042b57fd:

    # lee "Now, once I'm finished speaking you may leave the classroom and begin reading pages 100-103 in your textbook."
    lee "Теперь, как только я закончу говорить, вы покинете аудиторию и начнёте читать страницы 100-103 вашего учебника."

# game/v1/scene43b.rpy:569
translate russian v1s43Chloe_PhoneContinue1_56fc6f0c:

    # lee "After that, you'll come up with a short scene involving a king and a queen interaction that you'll present next week to the rest of the class."
    lee "После этого вы придумаете короткую сцену взаимодействия короля и королевы, которую представите на следующей неделе остальным членам класса."

# game/v1/scene43b.rpy:575
translate russian v1s43Chloe_PhoneContinue1_0a0ded35:

    # ri "Oh, that seems easy enough."
    ri "О, это кажется достаточно простым."

# game/v1/scene43b.rpy:582
translate russian v1s43Chloe_PhoneContinue1_11d52f58:

    # u "Hmm? Let me check this real quick."
    u "Хм? Позволь я очень быстро проверю."

# game/v1/scene43b.rpy:622
translate russian v1s43Chloe_PhoneContinue2_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v1/scene43b.rpy:630
translate russian v1s43Chloe_PhoneContinue2_812c6164:

    # u "(Fuck... Where did that come from?)"
    u "(Блядь... Откуда это взялось?)"

# game/v1/scene43b.rpy:633
translate russian v1s43Chloe_PhoneContinue2_9929f69d:

    # u "(Fuck... I'm glad I didn't go through with posting that shit.)"
    u "(Блядь... я рад, что не стал постить это дерьмо)."

# game/v1/scene43b.rpy:638
translate russian v1s43Chloe_PhoneContinue2_7b24288c:

    # u "*Sighs* Okay, are you ready to go?"
    u "*Вздох* Ладно, ты готова?"

# game/v1/scene43b.rpy:643
translate russian v1s43Chloe_PhoneContinue2_27544913:

    # ri "Yeah, but where do you wanna work?"
    ri "Да, но где ты хочешь работать?"

# game/v1/scene43b.rpy:648
translate russian v1s43Chloe_PhoneContinue2_077b7471:

    # u "Let's just work in the classroom across the hall, it's always empty."
    u "Давай поработаем в классе напротив, там всегда пусто."

# game/v1/scene43b.rpy:653
translate russian v1s43Chloe_PhoneContinue2_3bd922ae:

    # ri "Okay, then. Sure."
    ri "Хорошо, тогда. Ладно."

# game/v1/scene43b.rpy:678
translate russian v1s43Chloe_PhoneContinue2_0a0ded35:

    # ri "Oh, that seems easy enough."
    ri "О, это кажется достаточно простым."

# game/v1/scene43b.rpy:683
translate russian v1s43Chloe_PhoneContinue2_8abc1659:

    # u "Okay, are you ready to go?"
    u "Ладно, ты готова?"

# game/v1/scene43b.rpy:688
translate russian v1s43Chloe_PhoneContinue2_27544913_1:

    # ri "Yeah, but where do you wanna work?"
    ri "Да, но где ты хочешь работать?"

# game/v1/scene43b.rpy:693
translate russian v1s43Chloe_PhoneContinue2_077b7471_1:

    # u "Let's just work in the classroom across the hall, it's always empty."
    u "Давай поработаем в классе напротив, там всегда пусто."

# game/v1/scene43b.rpy:698
translate russian v1s43Chloe_PhoneContinue2_3bd922ae_1:

    # ri "Okay, then. Sure."
    ri "Хорошо, тогда. Ладно."

translate russian strings:

    # game/v1/scene43b.rpy:10
    old "I'm more than happy that we matched, especially after knowing that you're just as excited as I am, haha!"
    new "Я более чем счастлива, что мы совпали, особенно зная, что ты так же взволнован, как и я, хаха!"

    # game/v1/scene43b.rpy:13
    old "So, how are you? Who are you? What are you up to? Lmao"
    new "Ну, как ты? Кто ты? Что ты задумала? Lmao"

    # game/v1/scene43b.rpy:14
    old "You're single, right?"
    new "Ты одинок, верно?"

    # game/v1/scene43b.rpy:19
    old "You're in France?! We literally just got back to America from a class trip to Europe..."
    new "Ты во Франции?! Мы только что вернулись в Америку из поездки с классом в Европу..."

    # game/v1/scene43b.rpy:24
    old "Uh, yeah. Of course! I just felt like I needed to ask for some reason."
    new "О, да. Конечно! Я просто чувствовала, что мне нужно спросить по какой-то причине."

    # game/v1/scene43b.rpy:43
    old "Sure thing, hottie."
    new "Да, так и есть, красотка."

    # game/v1/scene43b.rpy:52
    old "Hey, Iris. So sorry if you felt like I wasn't interested, my Simplr has just been glitchy and I've been stuck out of it for a while. Back for good now."
    new "Привет, Айрис. Извини, если тебе показалось, что я не заинтересована, просто мой Simplr глючил, и я на некоторое время выпала из него. Теперь я снова вернулась."

    # game/v1/scene43b.rpy:277
    old "Work with Penelope"
    new "Работать с Пенелопой"

    # game/v1/scene43b.rpy:277
    old "Work with Riley"
    new "Работать с Райли"

