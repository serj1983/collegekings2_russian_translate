# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene22a.rpy:12
translate russian v1s22a_95984d5c:

    # li "So... People have been treating me like the Vice President of the United States."
    li "Итак... Люди относятся ко мне как к вице-президенту Соединенных Штатов."

# game/v1/scene22a.rpy:17
translate russian v1s22a_c01f1d52:

    # u "*Laughs* What makes you say that?"
    u "*Смеётся* С чего ты это взяла?"

# game/v1/scene22a.rpy:22
translate russian v1s22a_f3472fdb:

    # li "They either respect, love and pray that I succeed, or they completely despise me and won't even look in my direction."
    li "Они либо уважают, любят и молятся, чтобы я преуспела, либо полностью презирают меня и даже не смотрят в мою сторону."

# game/v1/scene22a.rpy:27
translate russian v1s22a_f58c55b3:

    # u "Welcome to the world of fame, baby! *Chuckles*"
    u "Добро пожаловать в мир славы, детка! *Хихикает*"

# game/v1/scene22a.rpy:29
translate russian v1s22a_afc84cd0:

    # u "Everyone knows who you are now, Lindsey. That's what you wanted, right?"
    u "Теперь все знают, кто ты, Линдси. Это то, чего ты хотела, верно?"

# game/v1/scene22a.rpy:34
translate russian v1s22a_48505638:

    # li "It's what I needed, yes. If you ask me what I wanted we'd be speaking of fantasies."
    li "Это то, что мне было нужно, да. Если бы ты спросил меня, чего я хотела, мы бы говорили о фантазиях."

# game/v1/scene22a.rpy:39
translate russian v1s22a_bd260dad:

    # u "That I can understand."
    u "Это я могу понять."

# game/v1/scene22a.rpy:44
translate russian v1s22a_3653424b:

    # li "At the end of the day, as long as all of this was worth something, I'll be happy. People's radical emotions will calm down after the election, right?"
    li "В конце концов, если всё это чего-то стоит, я буду счастлива. Радикальные эмоции людей успокоятся после выборов, верно?"

# game/v1/scene22a.rpy:49
translate russian v1s22a_8154a7cc:

    # u "Let's hope so, yeah. But regardless, these next few weeks are gonna be rough for you."
    u "Будем надеяться, что да. Но независимо от этого, следующие несколько недель будут для тебя тяжелыми."

# game/v1/scene22a.rpy:54
translate russian v1s22a_2c7d6c52:

    # li "*Sighs* You're preaching to the choir."
    li "*вздох* Я и сама об этом думаю."

# game/v1/scene22a.rpy:59
translate russian v1s22a_44b0c613:

    # u "You know I'm here for you though."
    u "Но ты же знаешь, что я здесь ради тебя."

# game/v1/scene22a.rpy:64
translate russian v1s22a_3430565a:

    # li "Yeah, I know that."
    li "Да, я знаю это."

# game/v1/scene22a.rpy:68
translate russian v1s22a_3f89b3b1:

    # u "*Phone vibrates*"
    u "*Телефон вибрирует*"

# game/v1/scene22a.rpy:73
translate russian v1s22a_35b1dea0:

    # li "Your phone is buzzing I think."
    li "Кажется, твой телефон жужжит."

# game/v1/scene22a.rpy:79
translate russian v1s22a_e738a43c:

    # u "Yeah, I should probably take this."
    u "Да, наверное, мне стоит ответить."

# game/v1/scene22a.rpy:84
translate russian v1s22a_e5ef14d1:

    # li "Alright, I need to run anyway."
    li "Ладно, мне всё равно нужно бежать."

# game/v1/scene22a.rpy:89
translate russian v1s22a_6e91440f:

    # li "Thank you, once again, for everything."
    li "Ещё раз спасибо тебе за всё."

# game/v1/scene22a.rpy:94
translate russian v1s22a_15772c9c:

    # u "Once again, always."
    u "Ещё раз, пожалуйста."

# game/v1/scene22a.rpy:104
translate russian v1s22a_446ac721:

    # u "(Oh, right, Chloe's on her way to the Wolves house. Need to rush now.)"
    u "(О, точно, Хлоя направляется в дом Волков. Нужно спешить.)"

# game/v1/scene22a.rpy:113
translate russian v1s22a_ffa6f7aa:

    # u "Yeah, oh well. It can wait."
    u "Да, ну и ладно. Это может подождать."

# game/v1/scene22a.rpy:118
translate russian v1s22a_8021f8b4:

    # li "No, no. It may be important. Besides, I need to run anyway."
    li "Нет, нет. Это может быть важно. Кроме того, мне всё равно нужно бежать."

# game/v1/scene22a.rpy:123
translate russian v1s22a_6e91440f_1:

    # li "Thank you, once again, for everything."
    li "Ещё раз спасибо тебе за всё."

# game/v1/scene22a.rpy:128
translate russian v1s22a_15772c9c_1:

    # u "Once again, always."
    u "Ещё раз, пожалуйста."

# game/v1/scene22a.rpy:138
translate russian v1s22a_901e6178:

    # u "(Let's see who this is...)"
    u "(Давай посмотрим, кто это...)"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene22a.rpy:12
translate russian v1s22a_de5328bd:

    # li "So... People invested in this vote have been treating me like the Vice President of the United States."
    li "Итак ... люди, вложившие средства в это голосование, относились ко мне как к вице-президенту Соединенных Штатов."

# game/v1/scene22a.rpy:17
translate russian v1s22a_bb6d45fb:

    # u "I mean, that's a small subset of the college. Just our friends. But what makes you say that?"
    u "Я имею в виду, это небольшая часть колледжа. Только наши друзья. Но что заставляет тебя так говорить?"

# game/v1/scene22a.rpy:22
translate russian v1s22a_c68910c4:

    # li "They either respect me, love and pray that I succeed, or completely despise me and won't even look in my direction."
    li "Они либо уважают меня, любят и молятся, чтобы я преуспела, либо полностью презирают меня и даже не смотрят в мою сторону."

# game/v1/scene22a.rpy:27
translate russian v1s22a_5302cf24:

    # u "Welcome to the world of fame, baby!"
    u "Добро пожаловать в мир славы, детка!"

# game/v1/scene22a.rpy:49
translate russian v1s22a_f439558b:

    # u "Let's hope so, yeah. And remember, it's just a small subset who care about this stuff. Regardless, these next few weeks are gonna be rough for you."
    u "Будем надеяться, что да. И помни, это лишь небольшая часть тех, кого волнуют такие вещи. Независимо от этого, следующие несколько недель будут тяжелыми для тебя."

# game/v1/scene22a.rpy:59
translate russian v1s22a_af9b579c:

    # u "You know I'm here for you, though."
    u "Ты же знаешь, что я рядом с тобой."

# game/v1/scene22a.rpy:73
translate russian v1s22a_782bc16b:

    # li "Your phone is buzzing, I think."
    li "Кажется, твой телефон гудит."

