# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene21.rpy:12
translate russian v1s21_5112e439:

    # cl "[name]!"
    cl "[name]!"

# game/v1/scene21.rpy:17
translate russian v1s21_89ab6e28:

    # cl "[name]! Wait up!"
    cl "[name]! Подожди!"

# game/v1/scene21.rpy:32
translate russian v1s21_f019be27:

    # u "What's happening? Something wrong?"
    u "Что случилось? Что-то не так?"

# game/v1/scene21.rpy:37
translate russian v1s21_7bcb7a71:

    # cl "No, no, nothing's wrong. *Panting*"
    cl "Нет, нет, всё в порядке. *Дыхание*"

# game/v1/scene21.rpy:39
translate russian v1s21_44d90d09:

    # cl "Are you busy right now?"
    cl "Ты сейчас занят?"

# game/v1/scene21.rpy:44
translate russian v1s21_5cdb1aa3:

    # u "Not at this very moment, no."
    u "Не в этот момент, нет."

# game/v1/scene21.rpy:49
translate russian v1s21_fbe540b8:

    # cl "Okay, well..."
    cl "Ладно, хорошо..."

# game/v1/scene21.rpy:51
translate russian v1s21_1ddda97d:

    # cl "When I left the library earlier, I couldn't help but feel like I didn't really translate how thankful I am for all of your help."
    cl "Когда я уходила из библиотеки, я не могла отделаться от ощущения, что не передала, насколько я благодарна тебе за твою помощь."

# game/v1/scene21.rpy:61
translate russian v1s21_d4da0610:

    # cl "You're not only there for me in private but now in public as well. That's something I can't thank you enough for."
    cl "Ты помогаешь мне не только наедине, но теперь и на публике. За это я не могу не выразить тебе свою благодарность."

# game/v1/scene21.rpy:63
translate russian v1s21_2c853fc7:

    # cl "A lot of people that I know tend to avoid hanging out with me, because of the rumors or preconceived images that float around. But..."
    cl "Многие люди, которых я знаю, избегают общаться со мной из-за слухов и предвзятых мнений, которые ходят вокруг. Но..."

# game/v1/scene21.rpy:73
translate russian v1s21_eabd8440:

    # cl "You don't care what people say. And that's what I appreciate the most."
    cl "Тебе всё равно, что говорят люди. И это то, что я ценю больше всего."

# game/v1/scene21.rpy:95
translate russian v1s21_f6b31c46:

    # u "I enjoy being there for you."
    u "Мне нравится быть рядом с тобой."

# game/v1/scene21.rpy:97
translate russian v1s21_4fd4d756:

    # u "I adore seeing you happy, a-and excited..."
    u "Я обожаю видеть тебя счастливой, радостной и возбужденной..."

# game/v1/scene21.rpy:102
translate russian v1s21_108b353e:

    # cl "*Chuckles*"
    cl "*Хихикает*"

# game/v1/scene21.rpy:107
translate russian v1s21_f54a3912:

    # u "And honestly Chloe, I'll do whatever I can to help keep that gorgeous smile on your face for as long as humanly possible."
    u "И, честно говоря, Хлоя, я сделаю всё возможное, чтобы помочь сохранить эту великолепную улыбку на твоём лице как можно дольше."

# game/v1/scene21.rpy:112
translate russian v1s21_342d5425:

    # cl "You're the most amazing person I've ever met, do you know that?"
    cl "Ты самый удивительный человек, которого я когда-либо встречала, ты знаешь это?"

# game/v1/scene21.rpy:117
translate russian v1s21_97b6b99b:

    # u "I do now, I suppose. Although, I'm not completely agreeing with you on that one..."
    u "Теперь, полагаю, да. Хотя в этом я с тобой не совсем согласен..."

# game/v1/scene21.rpy:122
translate russian v1s21_108b353e_1:

    # cl "*Chuckles*"
    cl "*Хихикает*"

# game/v1/scene21.rpy:124
translate russian v1s21_382b9b63:

    # cl "You know, I think I need to apologize in advance."
    cl "Знаешь, я думаю, мне нужно заранее извиниться."

# game/v1/scene21.rpy:129
translate russian v1s21_f53a47fb:

    # u "Apologize? For what?"
    u "Извиниться? Для чего?"

# game/v1/scene21.rpy:134
translate russian v1s21_99ae3b89:

    # cl "I mean, with the campaign being this real and competitive, I'm gonna have a lot less time than what we're used to."
    cl "Я имею в виду, что с такой реальной и конкурентной кампанией, у меня будет гораздо меньше времени, чем мы привыкли."

# game/v1/scene21.rpy:136
translate russian v1s21_cf2c7255:

    # cl "So, sadly that means we'll have less time to be together, you know?"
    cl "Так что, к сожалению, это означает, что у нас будет меньше времени, чтобы побыть вместе, понимаешь?"

# game/v1/scene21.rpy:144
translate russian v1s21_ef17d67d:

    # u "That does kinda suck. I guess we'll have to make the most out of the time we have."
    u "Это отстой. Думаю, нам придется максимально использовать то время, которое у нас есть."

# game/v1/scene21.rpy:148
translate russian v1s21_690faec6:

    # u "I'm a patient man when it comes to the things I care about, Chloe. I know your situation and I understand that this campaign is more important right now."
    u "Я терпеливый человек, когда речь идет о вещах, которые мне дороги, Хлоя. Я знаю твою ситуацию и понимаю, что эта кампания сейчас важнее."

# game/v1/scene21.rpy:153
translate russian v1s21_8fd360f1:

    # cl "*Sighs* You really are the best."
    cl "*Вздох* Ты действительно лучший."

# game/v1/scene21.rpy:158
translate russian v1s21_9f2d85b0:

    # u "Thank you, you are too."
    u "Спасибо, и ты тоже."

# game/v1/scene21.rpy:163
translate russian v1s21_75e4a699:

    # cl "You know, I have some time right now..."
    cl "Знаешь, у меня сейчас есть немного времени..."

# game/v1/scene21.rpy:168
translate russian v1s21_1c9eec7f:

    # u "Oh, yeah?"
    u "Правда?"

# game/v1/scene21.rpy:173
translate russian v1s21_351497a1:

    # cl "Yeah... I think I have a nice idea in mind to kick off our campaign as well."
    cl "Да... Думаю, у меня тоже есть хорошая идея для начала нашей кампании."

# game/v1/scene21.rpy:178
translate russian v1s21_57258cce:

    # u "What's that?"
    u "Что это?"

# game/v1/scene21.rpy:183
translate russian v1s21_ccb05df7:

    # cl "Hmm..."
    cl "Хм..."

# game/v1/scene21.rpy:188
translate russian v1s21_c41e61b3:

    # cl "*Whispers* Let me show you."
    cl "*Шёпотом* Давай я тебе покажу."

# game/v1/scene21.rpy:208
translate russian v1s21_3d20714d:

    # u "Is there a reason we're in the parking lot?"
    u "Мы на парковке не просто так?"

# game/v1/scene21.rpy:213
translate russian v1s21_a3e97ba0:

    # cl "No one ever comes back here, you know?"
    cl "Никто никогда сюда не возвращается, понимаешь?"

# game/v1/scene21.rpy:215
translate russian v1s21_a7ea0459:

    # cl "I used to sit out here and study everyday after class during my first couple of years on campus."
    cl "Я сидела здесь и занималась каждый день после занятий в первые пару лет моего пребывания в кампусе."

# game/v1/scene21.rpy:217
translate russian v1s21_4d842c87:

    # cl "One of the very few places on campus where I know I won't be bothered."
    cl "Одно из немногих мест в кампусе, где я знаю, что меня не побеспокоят."

# game/v1/scene21.rpy:222
translate russian v1s21_a4875ca5:

    # u "So, we're here to study?"
    u "Так мы здесь, чтобы учиться?"

# game/v1/scene21.rpy:227
translate russian v1s21_108b353e_2:

    # cl "*Chuckles*"
    cl "*Хихикает*"

# game/v1/scene21.rpy:232
translate russian v1s21_afebcc8c:

    # cl "We can call it that."
    cl "Мы можем называть это так"

# game/v1/scene21.rpy:257
translate russian v1s21_438d4fd4:

    # cl "Hm? Is something wrong?"
    cl "Хм? Что-то не так?"

# game/v1/scene21.rpy:262
translate russian v1s21_9974b02d:

    # u "N-No... Nothing's wrong at all, I just-"
    u "Н-нет... Всё в порядке, я просто..."

# game/v1/scene21.rpy:267
translate russian v1s21_4b1e6b98:

    # u "Look, Chloe... I don't want you to feel like you owe me anything just for supporting you."
    u "Послушай, Хлоя... Я не хочу, чтобы ты чувствовала, что ты мне что-то должна за то, что я тебя поддерживаю."

# game/v1/scene21.rpy:272
translate russian v1s21_d06ac492:

    # u "I do these things because I care about you and want you to succeed."
    u "Я делаю это, потому что забочусь о тебе и хочу, чтобы ты добились успеха."

# game/v1/scene21.rpy:274
translate russian v1s21_d548c5ce:

    # u "I don't do it just because I expect something from you in the end."
    u "Я делаю это не только потому, что жду от тебя чего-то в итоге."

# game/v1/scene21.rpy:290
translate russian v1s21_f4b07541:

    # cl "I've never been with a man like you, but I'm sure as hell happy that I am."
    cl "Я никогда не была с таким мужчиной, как ты, но я чертовски счастлива, что это так."

# game/v1/scene21.rpy:295
translate russian v1s21_659365d8:

    # u "I'm happy we're together too."
    u "Я тоже счастлив, что мы вместе."

# game/v1/scene21.rpy:297
translate russian v1s21_cdf0964b:

    # u "And I want to see you win. So you go ahead and get things done that need to be done."
    u "И я хочу, чтобы вы победили. Так что иди вперед и делай то, что должно быть сделано."

# game/v1/scene21.rpy:299
translate russian v1s21_9770c588:

    # u "I'm gonna go talk to Chris."
    u "Я пойду поговорю с Крисом."

# game/v1/scene21.rpy:304
translate russian v1s21_f6e51173:

    # cl "Thank you, baby. So much."
    cl "Спасибо, малыш. Это, так много."

# game/v1/scene21.rpy:309
translate russian v1s21_e8ae7c6e:

    # u "Always!"
    u "Всегда!"

# game/v1/scene21.rpy:314
translate russian v1s21_1a0fbfb1:

    # cl "I'll meet you there soon, okay?"
    cl "Скоро встретимся, хорошо?"

# game/v1/scene21.rpy:319
translate russian v1s21_1b28a406:

    # u "Of course."
    u "Конечно."

# game/v1/scene21.rpy:335
translate russian v1s21_91dd9879:

    # u "(She's something else.)"
    u "(Она совсем другая.)"

translate russian strings:

    # game/v1/scene21.rpy:141
    old "That does kinda suck"
    new "Это отстой."

    # game/v1/scene21.rpy:141
    old "Don't worry about it"
    new "Не волнуйся"

    # game/v1/scene21.rpy:244
    old "Continue"
    new "Продолжить"

    # game/v1/scene21.rpy:244
    old "Pull back"
    new "Оттолкнуть"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene21.rpy:63
translate russian v1s21_b3a27465:

    # cl "A lot of people tend to avoid hanging out with me because of rumors or preconceived notions that float around. But..."
    cl "Многие люди избегают общения со мной из-за слухов и предвзятых мнений, которые ходят вокруг. Но..."

# game/v1/scene21.rpy:97
translate russian v1s21_b79289b9:

    # u "I adore seeing you happy, and excited..."
    u "Я обожаю видеть тебя счастливой и взволнованной..."

# game/v1/scene21.rpy:102
translate russian v1s21_5128434c:

    # cl "Aw."
    cl "Оу."

# game/v1/scene21.rpy:112
translate russian v1s21_a34ad692:

    # cl "You're the most amazing person I've ever met. You know that?"
    cl "Ты самый удивительный человек, которого я когда-либо встречала. Ты знаешь это?"

# game/v1/scene21.rpy:117
translate russian v1s21_d954b4cb:

    # u "I do now, I suppose..."
    u "Теперь, наверное, да..."

# game/v1/scene21.rpy:132
translate russian v1s21_747c0564:

    # cl "With the campaign being so competitive, I'm gonna have a lot less time than what we're used to."
    cl "Поскольку кампания настолько конкурентная, у меня будет гораздо меньше времени, к чему мы привыкли."

# game/v1/scene21.rpy:134
translate russian v1s21_c11cb2f7:

    # cl "We'll have less time together, you know?"
    cl "У нас будет меньше времени вместе, понимаешь?"

# game/v1/scene21.rpy:142
translate russian v1s21_b0741626:

    # u "That does kinda suck. I guess we'll have to make the most of the time we have."
    u "Это действительно отстой. Думаю, нам придется максимально использовать то время, которое у нас есть."

# game/v1/scene21.rpy:161
translate russian v1s21_0302c1b5:

    # cl "For what it's worth, I have some time right now..."
    cl "Если уж на то пошло, у меня сейчас есть немного времени..."

# game/v1/scene21.rpy:171
translate russian v1s21_c80dda08:

    # cl "Yeah... I think I may have a nice idea to kick off our campaign with as well."
    cl "Да... Думаю, у меня тоже есть хорошая идея для начала нашей кампании."

# game/v1/scene21.rpy:225
translate russian v1s21_6ec7e8ae:

    # cl "Ha."
    cl "Ха."

# game/v1/scene21.rpy:230
translate russian v1s21_26e7ae07:

    # cl "We can call it that, if you want."
    cl "Мы можем назвать это так, если хочешь."

