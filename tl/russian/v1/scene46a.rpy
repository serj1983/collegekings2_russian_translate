# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene46a.rpy:16
translate russian v1s46a_d6debb9f:

    # la "Hi."
    la "Привет."

# game/v1/scene46a.rpy:21
translate russian v1s46a_3937b3c7:

    # u "Hello, Dr. Lauren. I'm here for my hypnosis appointment."
    u "Здравствуйте, доктор Лорен. Я пришёл на приём к гипнотизеру."

# game/v1/scene46a.rpy:26
translate russian v1s46a_e97457ae:

    # la "*Laughs* Come on in, silly."
    la "*Смеётся* Заходи, глупышка."

# game/v1/scene46a.rpy:41
translate russian v1s46a_0c9af005:

    # la "Go ahead and lie down on the bed."
    la "Иди и ложись на кровать."

# game/v1/scene46a.rpy:47
translate russian v1s46a_d9c883d8:

    # u "Damn, okay. I like where this is going already."
    u "Чёрт, ладно. Мне уже нравится, к чему всё идет."

# game/v1/scene46a.rpy:52
translate russian v1s46a_ed7b3da7:

    # la "*Chuckles* Stop it. You have to focus."
    la "*Хихикает* Прекрати. Ты должен сосредоточиться."

# game/v1/scene46a.rpy:58
translate russian v1s46a_9add6d20:

    # u "Okay sure, but I can't promise you that I'll be able to stay awake."
    u "Хорошо, но не могу обещать, что не засну."

# game/v1/scene46a.rpy:63
translate russian v1s46a_17dab57a:

    # la "We'll see about that..."
    la "Это мы еще посмотрим..."

# game/v1/scene46a.rpy:68
translate russian v1s46a_4c5047d3:

    # u "Haha, okay. So, we're just going straight into it? No chit-chat?"
    u "Хаха, хорошо. Значит, сразу приступаем к делу? Никакой болтовни?"

# game/v1/scene46a.rpy:73
translate russian v1s46a_3b65e04f:

    # la "Oh, right. I'm sorry, [name]. I'm just super excited to try this, haha."
    la "Ох, правда. Прости, [name]. Я просто очень хочу попробовать это, хаха."

# game/v1/scene46a.rpy:78
translate russian v1s46a_ca5f21e1:

    # u "It's fine, I'm just teasing."
    u "Всё в порядке, я просто дразню."

# game/v1/scene46a.rpy:101
translate russian v1s46a_sgb_d30ec7aa:

    # la "Okay... Now close your eyes."
    la "Хорошо... Теперь закрой глаза."

# game/v1/scene46a.rpy:106
translate russian v1s46a_sgb_f5b50837:

    # la "*Softly* Let's start by getting nice and relaxed with some breathing exercises."
    la "*Мягко* Давай начнем с расслабления с помощью дыхательных упражнений"

# game/v1/scene46a.rpy:111
translate russian v1s46a_sgb_80a8320a:

    # la "Breathe in slowly for 5 seconds... *Inhales*"
    la "Медленно вдыхай в течение 5 секунд... *Вдыхает*"

# game/v1/scene46a.rpy:116
translate russian v1s46a_sgb_fc8947b8:

    # u "*Inhales*"
    u "*Вдыхает*"

# game/v1/scene46a.rpy:121
translate russian v1s46a_sgb_aff86b91:

    # la "*Softly* Hold it for 2 seconds..."
    la "*Мягко* Задержи дыхание на 2 секунды..."

# game/v1/scene46a.rpy:126
translate russian v1s46a_sgb_7b1c1392:

    # u "..."
    u "..."

# game/v1/scene46a.rpy:132
translate russian v1s46a_sgb_36fe4c26:

    # u "*Coughs* Fuck, haha. Sorry. *Inhales*"
    u "*Кашляет* Чёрт, ха-ха. Извини. *Вдыхает*"

# game/v1/scene46a.rpy:137
translate russian v1s46a_sgb_e08394c0:

    # la "*Chuckles* Stoner much?"
    la "*Хихикает* Сильно накурился?"

# game/v1/scene46a.rpy:147
translate russian v1s46a_sgb_cebfd9c8:

    # la "Now breathe out... *Exhales* All the way..."
    la "Теперь выдохни... *Выдыхает*"

# game/v1/scene46a.rpy:152
translate russian v1s46a_sgb_95b1ef08:

    # u "*Exhales*"
    u "*Выдыхает*"

# game/v1/scene46a.rpy:157
translate russian v1s46a_sgb_076accf7:

    # la "*Softly* And again. Breathe in... breathe out... in... out..."
    la "*Мягко* И ещё раз. Вдохни... выдох... вдох... выдох..."

# game/v1/scene46a.rpy:162
translate russian v1s46a_sgb_63891b64:

    # u "*Inhales* *Exhales*"
    u "*Вдыхает* *Выдыхает*"

# game/v1/scene46a.rpy:167
translate russian v1s46a_sgb_56366f9c:

    # la "*Softly* You are now falling into deep relaxation..."
    la "*Мягко* Теперь ты погружаешься в глубокое расслабление..."

# game/v1/scene46a.rpy:172
translate russian v1s46a_sgb_df0a7769:

    # la "*Softly* Falling deeper and deeper..."
    la "*Мягко* Проваливаешься всё глубже и глубже..."

# game/v1/scene46a.rpy:174
translate russian v1s46a_sgb_cbd3915a:

    # la "*Softly* As you listen to the sound of my voice..."
    la "*Мягко*, Ты слушаешь только мой  голоса..."

# game/v1/scene46a.rpy:179
translate russian v1s46a_sgb_a54392ef:

    # la "*Softly* My voice is all you can hear as you sink deeper and deeper... into a peaceful... pleasant... relaxing state."
    la "*Мягко* Мой голос - это всё, что ты слышишь, погружаясь всё глубже и глубже... в мирное... приятное... расслабляющее состояние."

# game/v1/scene46a.rpy:184
translate russian v1s46a_sgb_fcc18885:

    # u "*Laughs*"
    u "*Смеётся*"

# game/v1/scene46a.rpy:189
translate russian v1s46a_sgb_ca864a05:

    # la "[name]! Come on... *Chuckles* Please take this seriously."
    la "[name]! Ну же... *Хихикает* Пожалуйста, отнесись к этому серьёзно."

# game/v1/scene46a.rpy:194
translate russian v1s46a_sgb_9316ff66:

    # u "Haha... Okay... Okay. Sorry."
    u "Хаха... Ладно... Ладно. Извини."

# game/v1/scene46a.rpy:196
translate russian v1s46a_sgb_0dfbe353:

    # u "I'll try again."
    u "Я попробую ещё раз."

# game/v1/scene46a.rpy:211
translate russian v1s46a_sgb_86385b92:

    # la "*Softly* You are starting to feel relaxed... You are feeling safe... And completely comfortable."
    la "*Мягко* Ты начинаешь чувствовать себя расслабленным... Ты чувствуешь себя в безопасности... И совершенно комфортно."

# game/v1/scene46a.rpy:216
translate russian v1s46a_sgb_486930b0:

    # la "*Softly* As you drift off into a deep and pleasant state of relaxation, deeper and deeper..."
    la "*Мягко* По мере того, как ты погружаешься в глубокое и приятное состояние расслабления, всё глубже и глубже..."

# game/v1/scene46a.rpy:217
translate russian v1s46a_sgb_a5192c34:

    # la "*Softly* Listening and feeling the gentle caress of the natural elements around you."
    la "*Мягко* Слушая и чувствуя нежную ласку природных стихий вокруг тебя."

# game/v1/scene46a.rpy:222
translate russian v1s46a_sgb_a10176e2:

    # la "*Softly* The longer you listen to my voice, the deeper you sink into pure relaxation."
    la "*Мягко* Чем дольше ты слушаешь мой голос, тем глубже погружаешься в чистое расслабление."

# game/v1/scene46a.rpy:227
translate russian v1s46a_sgb_d126c239:

    # u "(Oh, wow... Is this shit actually... working?)"
    u "(О, вау... Это дерьмо на самом деле... работает?)"

# game/v1/scene46a.rpy:232
translate russian v1s46a_sgb_e387f4d9:

    # la "*Softly* Deeper and deeper..."
    la "*Мягко* Глубже и глубже..."

# game/v1/scene46a.rpy:234
translate russian v1s46a_sgb_a418687c:

    # la "*Whispers* More relaxed than you've ever felt before."
    la "*Шёпотом* Более расслабленным, чем когда-либо чувствовал себя раньше."

# game/v1/scene46a.rpy:239
translate russian v1s46a_sgb_97dd9a03:

    # u "(Zzzz...)"
    u "(Zzzz...)"

# game/v1/scene46a.rpy:245
translate russian v1s46a_sgb_fbdc8e73:

    # la "*Softly* You are now walking along a beach. It's the most beautiful day. So sunny and warm, you can feel the light dancing on your skin."
    la "*Мягко* Сейчас ты идешь по пляжу. Это самый прекрасный день. Так солнечно и тепло, что чувствуешь, как солнечные лучи обжигают твою кожу."

# game/v1/scene46a.rpy:247
translate russian v1s46a_sgb_da6b690f:

    # la "*Softly* The sand between your toes is soft to the touch and feels like a freshly dried blanket. You're calm... you're relaxed..."
    la "*Мягко* Песок между пальцами ног мягкий на ощупь и похож на свежевысушенное одеяло. Ты спокоен... ты расслаблен..."

# game/v1/scene46a.rpy:252
translate russian v1s46a_sgb_53476fa2:

    # la "*Softly* You've never felt this relaxed before... You're the happiest you've ever been."
    la "*Мягко* Ты никогда раньше не чувствовал себя так расслабленно... Ты счастливее всех, кем когда-либо был."

# game/v1/scene46a.rpy:257
translate russian v1s46a_sgb_3773b09c:

    # u "(Zzzz...) *Heavy breathing*"
    u "(Zzzz...) *Тяжелое дыхание*"

# game/v1/scene46a.rpy:263
translate russian v1s46a_sgb_635aeb4c:

    # la "*Softly* There's a girl in a bikini, laying on a beach towel. Do you see her?"
    la "*Мягко* Там девушка в бикини, лежит на пляжном полотенце. Ты видишь её?"

# game/v1/scene46a.rpy:278
translate russian v1s46a_sgb_3ff79084:

    # u "Hm..."
    u "Хм..."

# game/v1/scene46a.rpy:283
translate russian v1s46a_sgb_d544fe64:

    # la "She calls over to you and asks you to rub lotion on her skin. You can help her."
    la "Она обращается к тебе и просит протереть её кожу лосьоном. Ты можешь помочь ей."

# game/v1/scene46a.rpy:300
translate russian v1s46a_sgb_f854f387:

    # la "*Softly* As you rub the lotion into her back, she removes her bikini top and turns around to face you."
    la "*Мягко* Пока ты растираешь лосьон на её спине, она снимает бикини и поворачивается к тебе лицом."

# game/v1/scene46a.rpy:301
translate russian v1s46a_sgb_db6b1ff8:

    # la "*Softly* She's now asking you to massage the lotion into her breasts."
    la "*Мягко* Теперь она просит тебя растереть лосьон на ее груди."

# game/v1/scene46a.rpy:309
translate russian v1s46a_sgb_a8fb7ce1:

    # u "(Zzzz...) Hmm..."
    u "(Zzzz...) Хм..."

# game/v1/scene46a.rpy:314
translate russian v1s46a_sgb_06ec0595:

    # la "*Softly* Rubbing into her soft skin you can feel her nipples, getting excited by your touch."
    la "*Мягко* Втирая лосьон в её нежную кожу, ты чувствуешь, как её соски возбуждаются от твоих прикосновений."

# game/v1/scene46a.rpy:334
translate russian v1s46a_sgb_78c04a02:

    # la "*Softly* She feels your... pulsing cock. *Embarrassed giggle* It's hard. Your swim trunks feel like they're about to burst at the seams..."
    la "*Мягко* Она чувствует твой... пульсирующий член. *Смущенный смешок* Это тяжело. Твои плавки, кажется, вот-вот лопнут по швам..."

# game/v1/scene46a.rpy:339
translate russian v1s46a_sgb_f16d90a1:

    # u "(Zzzz...) *Moans*"
    u "(Zzzz...) *Стонет*"

# game/v1/scene46a.rpy:354
translate russian v1s46a_sgb_e9b6654a:

    # la "*Softly* She slowly pulls them down and immediately takes your full, throbbing member in her mouth."
    la "*Мягко* Она медленно стягивает их вниз и сразу же берет твой большой, пульсирующий член в рот."

# game/v1/scene46a.rpy:355
translate russian v1s46a_sgb_2c2e24c1:

    # la "*Softly* She starts to suck on it, her soft lips hugging every inch of you. Sucking and sucking and sucking until–"
    la "*Мягко* Она начинает посасывать его, её мягкие губы обнимают каждый дюйм кожи. Сосёт, сосёт и сосёт, пока–"

# game/v1/scene46a.rpy:360
translate russian v1s46a_sgb_d3afeb29:

    # u "*Moans* Fuck..."
    u "*Стоны* Блядь..."

# game/v1/scene46a.rpy:364
translate russian v1s46_nsfwSkipLabel1_cab32c43:

    # la "*Laughs* Oh my God. I'm so sorry, I didn't think you'd-"
    la "*Смеется* О Боже мой. Мне так жаль, я не думала, что ты-"

# game/v1/scene46a.rpy:374
translate russian v1s46_nsfwSkipLabel1_00c627cb:

    # u "What the... fuck?!"
    u "Что за... блядь?!"

# game/v1/scene46a.rpy:381
translate russian v1s46_nsfwSkipLabel1_59f1255b:

    # la "I'm so sorry! *Laughs* It was fun though, right? Did it feel real?"
    la "Мне так жаль! *Смеётся* Но это было весело, правда? Это было реально?"

# game/v1/scene46a.rpy:386
translate russian v1s46_nsfwSkipLabel1_56514330:

    # u "I have cum all over myself, Lauren. It felt pretty fucking real! How am I supposed to walk home like this?!"
    u "Я весь в сперме, Лорен. Это было чертовски реально! Как я пойду домой в таком виде?!"

# game/v1/scene46a.rpy:391
translate russian v1s46_nsfwSkipLabel1_9bb33c19:

    # la "That's so crazy... You can just go in the bathroom and clean yourself up, ha."
    la "Это так безумно... Можешь просто пойти в ванную и привести себя в порядок, ха."

# game/v1/scene46a.rpy:396
translate russian v1s46_nsfwSkipLabel1_eed88133:

    # u "So funny."
    u "Очень смешно."

# game/v1/scene46a.rpy:401
translate russian v1s46_nsfwSkipLabel1_ccdffc1a:

    # la "I said sorry!"
    la "Я же извинилась!"

# game/v1/scene46a.rpy:406
translate russian v1s46_nsfwSkipLabel1_bce39fba:

    # la "But the hypnosis worked really well then, don't you think?"
    la "Но гипноз сработал очень хорошо, ты не находишь?"

# game/v1/scene46a.rpy:418
translate russian v1s46_nsfwSkipLabel1_b3e511ac:

    # u "If that's what you wanted to happen, then sure. It went great."
    u "Если ты этого хотела, то конечно. Всё прошло просто отлично."

# game/v1/scene46a.rpy:423
translate russian v1s46_nsfwSkipLabel1_9f9e69d8:

    # la "You can dry off with my beach towel in there."
    la "Ты можешь вытереться моим пляжным полотенцем."

# game/v1/scene46a.rpy:428
translate russian v1s46_nsfwSkipLabel1_79ebe5aa:

    # u "Ah, a beach towel. Good one."
    u "А, пляжное полотенце. Хорошо."

# game/v1/scene46a.rpy:445
translate russian v1s46_nsfwSkipLabel1_6c857347:

    # la "I didn't know what was going to happen, [name]. I was just exploring."
    la "Я не знала, что произойдет, [name]. Я просто исследовала."

# game/v1/scene46a.rpy:450
translate russian v1s46_nsfwSkipLabel1_e9580b50:

    # u "You were exploring and I was exploding..."
    u "Ты исследовала, а я кончил..."

# game/v1/scene46a.rpy:455
translate russian v1s46_nsfwSkipLabel1_6d1b6cf7:

    # la "*Laughs*"
    la "*Смеётся*"

# game/v1/scene46a.rpy:461
translate russian v1s46_nsfwSkipLabel1_668a87ff:

    # u "*Laughs* That was such a weird experience. Where did you learn this?"
    u "*Смеётся* Это странный опыт. Где ты этому научилась?"

# game/v1/scene46a.rpy:466
translate russian v1s46_nsfwSkipLabel1_96e7309f:

    # la "I guess I've just been watching a little too much porn lately... I can't stop thinking about sex."
    la "Наверное, в последнее время, я слишком много смотрю порно... Не могу перестать думать о сексе."

# game/v1/scene46a.rpy:471
translate russian v1s46_nsfwSkipLabel1_f4844e53:

    # u "Haha, wow..."
    u "Хаха, вау..."

# game/v1/scene46a.rpy:476
translate russian v1s46_nsfwSkipLabel1_e02d5fe5:

    # la "You still love me, right?"
    la "Ты всё ещё любишь меня, да?"

# game/v1/scene46a.rpy:483
translate russian v1s46_nsfwSkipLabel1_3f54e48a:

    # u "Of course I do."
    u "Конечно, да."

# game/v1/scene46a.rpy:492
translate russian v1s46_nsfwSkipLabel1_ab2bb88b:

    # u "Even more than I did before."
    u "Даже больше, чем раньше."

# game/v1/scene46a.rpy:497
translate russian v1s46_nsfwSkipLabel1_7557d2b4:

    # la "Really?"
    la "Правда?"

# game/v1/scene46a.rpy:502
translate russian v1s46_nsfwSkipLabel1_59734cd3:

    # u "I mean, I kinda feel like your guinea pig all the time..."
    u "Я вроде как, всё время чувствую себя твоей подопытной свинкой..."

# game/v1/scene46a.rpy:507
translate russian v1s46_nsfwSkipLabel1_87b5df08:

    # la "*Laughs* My poor guinea pig."
    la "*Смеётся* Моя бедная морская свинка."

# game/v1/scene46a.rpy:512
translate russian v1s46_nsfwSkipLabel1_c8105363:

    # u "But experimenting like this and seeing how excited you are, it reminds me of how much I like you."
    u "Подобные эксперименты и видя, как ты возбуждена, напоминает мне о том, как сильно ты мне нравишься."

# game/v1/scene46a.rpy:523
translate russian v1s46_nsfwSkipLabel1_df8289d9:

    # la "Aww."
    la "Оу."

# game/v1/scene46a.rpy:544
translate russian v1s46_nsfwSkipLabel1_4c9a0e9f:

    # la "I'm glad."
    la "Я рада."

# game/v1/scene46a.rpy:550
translate russian v1s46_nsfwSkipLabel1_2be2731a:

    # la "*Softly* You are now walking through a fresh, grassy field in your bare feet."
    la "*Мягко* Сейчас ты идёшь босиком по свежему травянистому полю."

# game/v1/scene46a.rpy:551
translate russian v1s46_nsfwSkipLabel1_76f836f7:

    # la "*Softly* It's the most beautiful day. So sunny and warm, you can feel the light dancing on your skin."
    la "*Мягко* Это самый прекрасный день. Так солнечно и тепло, что чувствуешь, как солнечные лучи обжигают твою кожу."

# game/v1/scene46a.rpy:553
translate russian v1s46_nsfwSkipLabel1_90988155:

    # la "*Softly* The grass between your toes is gentle to the touch. You're calm... you're relaxed... you're the happiest you've ever been..."
    la "*Мягко* Трава между пальцами ног нежна на ощупь. Ты спокоен... ты расслаблен... ты счастливее всех, кем когда-либо был..."

# game/v1/scene46a.rpy:558
translate russian v1s46_nsfwSkipLabel1_f68b6c38:

    # u "*Zzzz...* *Heavy breathing*"
    u "*Zzzz...* *Тяжелое дыхание*"

# game/v1/scene46a.rpy:569
translate russian v1s46_nsfwSkipLabel1_e23d452f:

    # la "*Softly* You see a majestic horse trotting towards you. The closer it gets, the more calm you feel."
    la "*Мягко* Ты видишь величественного скакуна, который рысью приближается к тебе. Чем ближе он, тем спокойнее ты себя чувствуешь."

# game/v1/scene46a.rpy:574
translate russian v1s46_nsfwSkipLabel1_c9c59225:

    # la "*Softly* You notice you have an apple in your hand."
    la "*Мягко* Ты замечаешь, что у тебя в руке яблоко."

# game/v1/scene46a.rpy:584
translate russian v1s46_nsfwSkipLabel1_a8e6ae2e:

    # la "*Softly* You feed it to the horse."
    la "*Мягко* Ты скармливаешь его жеребцу."

# game/v1/scene46a.rpy:589
translate russian v1s46_nsfwSkipLabel1_a23843e2:

    # u "*Zzzz...* Hm..."
    u "*Zzzz...* Хм..."

# game/v1/scene46a.rpy:594
translate russian v1s46_nsfwSkipLabel1_87fe17f9:

    # la "*Softly* The horse has finished the apple, but he's still hungry... He starts to eat your shirt."
    la "*Мягко* Конь доел яблоко, но он всё еще голоден... Он начинает есть твою рубашку."

# game/v1/scene46a.rpy:604
translate russian v1s46_nsfwSkipLabel1_9f1c0dac:

    # la "*Softly* You can't stop him! In an instant, he's eaten your whole shirt..."
    la "*Мягко* Ты не можешь остановить его! В одно мгновение он съел всю рубашку..."

# game/v1/scene46a.rpy:609
translate russian v1s46_nsfwSkipLabel1_e2e69cb5:

    # la "*Softly* But he's not stopping there... He's hungry for something else now. Before you know it, he's chewing at your dick."
    la "*Мягко* Но он на этом не останавливается... Теперь он жаждет чего-то другого. Не успеешь оглянуться, как он уже грызёт твой член."

# game/v1/scene46a.rpy:619
translate russian v1s46_nsfwSkipLabel1_ce72dd33:

    # u "*Zzzz...* *Grunts*"
    u "*Zzzz...* *Ворчит*"

# game/v1/scene46a.rpy:624
translate russian v1s46_nsfwSkipLabel1_7170e33a:

    # la "*Softly* He's starting to get angry. You can feel his teeth trying to bite their way through the fabric of your jeans... but you can't move."
    la "*Мягко* Он начинает злиться. Ты чувствуешь, как его зубы пытаются проложить себе путь сквозь ткань твоих джинсов... но ты не можешь пошевелиться."

# game/v1/scene46a.rpy:629
translate russian v1s46_nsfwSkipLabel1_38bb7b77:

    # la "*Softly* Your feet are glued to the ground as you try to push the horse away, but somehow he manages to rip off your pants."
    la "*Тихо* Твои ноги приклеены к земле, когда ты пытаешься оттолкнуть жеребца, но каким-то образом ему удаётся сорвать с тебя штаны."

# game/v1/scene46a.rpy:634
translate russian v1s46_nsfwSkipLabel1_83b64582:

    # la "*Softly* The horse has his eyes on the prize now... Before you know it, he takes one quick bite towards your dick and-"
    la "*Мягко* Теперь конь смотрит на приз... Не успеешь ты оглянуться, как он одним быстрым движением прикусывает твой член и-"

# game/v1/scene46a.rpy:639
translate russian v1s46_nsfwSkipLabel1_1cbd1651:

    # u "Aahhh! Fuck! Oww! M-My balls!"
    u "Ааа! Блядь! Оуу! М-Мои яйца!"

# game/v1/scene46a.rpy:660
translate russian v1s46_nsfwSkipLabel1_0a84af6a:

    # u "What the fuck?!"
    u "Какого хрена?!"

# game/v1/scene46a.rpy:665
translate russian v1s46_nsfwSkipLabel1_6994a67e:

    # la "*Laughs* I'm sorry, [name]! It was too good of an opportunity! I didn't think the horse dream would actually work on anyone... *Chuckles*"
    la "*Смеётся* Мне очень жаль, [name]! Это была слишком хорошая возможность! Я не думала, что сон про жеребца действительно на кого-то подействует... *Хихикает*"

# game/v1/scene46a.rpy:670
translate russian v1s46_nsfwSkipLabel1_9a48fa69:

    # u "What? Why did you do that?"
    u "Что? Зачем ты это сделала?"

# game/v1/scene46a.rpy:675
translate russian v1s46_nsfwSkipLabel1_0d59fa02:

    # la "All in the name of science, I promise. You should be very proud of yourself. It worked perfectly."
    la "Всё во имя науки, обещаю. Ты должен гордиться собой. Всё сработало идеально."

# game/v1/scene46a.rpy:680
translate russian v1s46_nsfwSkipLabel1_d444159c:

    # u "You're some kind of mad scientist, aren't you?"
    u "Ты сумасшедший ученый, да?"

# game/v1/scene46a.rpy:685
translate russian v1s46_nsfwSkipLabel1_dc36ba01:

    # la "*Chuckles* I watched a few videos, and I wanted to see if it worked."
    la "*Хихикает* Я посмотрела несколько видео, и мне захотелось проверить, сработает ли это."

# game/v1/scene46a.rpy:690
translate russian v1s46_nsfwSkipLabel1_04688964:

    # u "*Laughs* That was such a weird experience... My balls... They actually hurt a little."
    u "*Смеётся* Это был такой странный опыт... Мои яйца... Они на самом деле немного болят."

# game/v1/scene46a.rpy:695
translate russian v1s46_nsfwSkipLabel1_e389d1f6:

    # la "Haha! Perfect."
    la "Хаха! Идеально."

# game/v1/scene46a.rpy:702
translate russian v1s46_nsfwSkipLabel1_d47bfab1:

    # la "Now, I've gotta get caught up on homework, so."
    la "Мне нужно сделать домашнее задание, так что..."

# game/v1/scene46a.rpy:707
translate russian v1s46_nsfwSkipLabel1_b6a2f43c:

    # u "Kicking me out?"
    u "Выгоняешь меня?"

# game/v1/scene46a.rpy:712
translate russian v1s46_nsfwSkipLabel1_ffcc890a:

    # la "Yeah..."
    la "Ага..."

# game/v1/scene46a.rpy:732
translate russian v1s46_nsfwSkipLabel1_2815f46f:

    # la "Thanks for doing that with me."
    la "Спасибо, что сделал это со мной."

# game/v1/scene46a.rpy:737
translate russian v1s46_nsfwSkipLabel1_00b6f1c5:

    # u "You're welcome, I guess."
    u "Не за что, я полагаю."

# game/v1/scene46a.rpy:742
translate russian v1s46_nsfwSkipLabel1_8a4bc8a4:

    # la "Haha... We'll do something again soon?"
    la "Хаха... Скоро опять что-нибудь замутим?"

# game/v1/scene46a.rpy:747
translate russian v1s46_nsfwSkipLabel1_3ecef2b0:

    # u "Yeah sure, but if it's another hypnosis session... Maybe I'll send Imre instead."
    u "Да, конечно, но если это будет ещё один сеанс гипноза... Я пошлю Имре вместо себя."

# game/v1/scene46a.rpy:752
translate russian v1s46_nsfwSkipLabel1_c6af06d7:

    # la "*Chuckles* Please don't do that to me."
    la "*Хихикает* Пожалуйста, не надо."

# game/v1/scene46a.rpy:757
translate russian v1s46_nsfwSkipLabel1_a6a91cdf:

    # u "Haha, see ya."
    u "Хаха, увидимся."

# game/v1/scene46a.rpy:762
translate russian v1s46_nsfwSkipLabel1_ce58e489:

    # la "Bye, [name]."
    la "Пока, [name]."

# game/v1/scene46a.rpy:769
translate russian v1s46_nsfwSkipLabel1_14fdd23d:

    # u "(Hmm, well that was interesting...)"
    u "(Хм, ну это было интересно...)"

translate russian strings:

    # game/v1/scene46a.rpy:478
    old "Of course I do"
    new "Конечно, да"

    # game/v1/scene46a.rpy:478
    old "Even more"
    new "Даже больше"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene46a.rpy:16
translate russian v1s46a_a61be53f:

    # la "Hi!"
    la "Привет!"

# game/v1/scene46a.rpy:21
translate russian v1s46a_e40e3215:

    # u "Hello, Dr. Lauren. I'm here for my impromptu hypnosis appointment."
    u "Здравствуйте, доктор Лорен. Я пришёл на импровизированную встречу с гипнозом."

# game/v1/scene46a.rpy:26
translate russian v1s46a_19e7e59e:

    # la "Come on in, silly."
    la "Заходи, глупышка."

# game/v1/scene46a.rpy:47
translate russian v1s46a_235637f2:

    # u "I like where this is going already. Is this an elaborate ruse to get some you-know-what going?"
    u "Мне уже нравится, к чему всё идет. Может быть, это хитроумная уловка, чтобы нам сделать сама-знаешь-что?"

# game/v1/scene46a.rpy:52
translate russian v1s46a_991d1d7c:

    # la "Cute, but stop it. You have to focus."
    la "Это мило, но пока остановись. Ты должен сосредоточиться."

# game/v1/scene46a.rpy:68
translate russian v1s46a_4f88e75a:

    # u "Okay, so, we're just going straight into it? No chit-chat?"
    u "Хорошо, значит, мы просто перейдем прямо к делу? Никакого разговора?"

# game/v1/scene46a.rpy:73
translate russian v1s46a_1f17a174:

    # la "Oh, right. I'm sorry, [name]. I'm just super excited to try this. You know how I get about psych theory."
    la "Ох, правда. Прости, [name]. Я просто очень хочу попробовать это. Ты же знаешь, как я отношусь к психологии."

# game/v1/scene46a.rpy:101
translate russian v1s46a_sgb_a6d7ff1f:

    # la "Okay... now, close your eyes."
    la "Хорошо... теперь закрой глаза."

# game/v1/scene46a.rpy:111
translate russian v1s46a_sgb_ab741256:

    # la "Breathe in slowly for five seconds... *Inhales*"
    la "Медленно вдыхай в течение пяти секунд... *Вдыхает*"

# game/v1/scene46a.rpy:121
translate russian v1s46a_sgb_6f29996c:

    # la "*Softly* Hold it for two seconds..."
    la "*Тихо* Задержи дыхание на 2 секунды..."

# game/v1/scene46a.rpy:132
translate russian v1s46a_sgb_2fbbdfac:

    # u "*Coughs* Ah, fuck. Sorry. *Inhales*"
    u "*Кашель* Ах, чёрт. Извини. *Вдыхает*"

# game/v1/scene46a.rpy:137
translate russian v1s46a_sgb_048e09fa:

    # la "Stoner much?"
    la "Глубоко вдохнул?"

# game/v1/scene46a.rpy:167
translate russian v1s46a_sgb_ddeca30f:

    # la "*Softly* You are now falling into a deep relaxation..."
    la "*Тихо* Сейчас ты погружаешься в глубокое расслабление..."

# game/v1/scene46a.rpy:184
translate russian v1s46a_sgb_5928b156:

    # u "If you say so!"
    u "Как скажешь!"

# game/v1/scene46a.rpy:189
translate russian v1s46a_sgb_d44a1ab9:

    # la "[name]! Come on... ha, please, please try to take this seriously."
    la "[name]! Ну же... ха, пожалуйста, пожалуйста постарайся отнестись к этому серьёзно."

# game/v1/scene46a.rpy:194
translate russian v1s46a_sgb_3243b885:

    # u "Okay... okay. Sorry."
    u "Ладно... Ладно. Извини."

# game/v1/scene46a.rpy:211
translate russian v1s46a_sgb_a91af3ca:

    # la "*Softly* You are starting to feel relaxed... you feel safe... and completely comfortable."
    la "*Тихо* Ты начинаешь чувствовать себя расслабленным... Ты чувствуешь себя в безопасности... И совершенно комфортно."

# game/v1/scene46a.rpy:216
translate russian v1s46a_sgb_e9fbd13e:

    # la "*Softly* As you drift into a deep state of relaxation, deeper and deeper..."
    la "*Тихо* По мере того, как ты погружаешься в глубокое и приятное состояние расслабления, всё глубже и глубже..."

# game/v1/scene46a.rpy:217
translate russian v1s46a_sgb_7216dd33:

    # la "*Softly* Listen and feel the gentle caress of the elements around you."
    la "*Тихо* Прислушайся и почувствуй нежную ласку окружающих тебя стихий."

# game/v1/scene46a.rpy:222
translate russian v1s46a_sgb_9da69822:

    # la "*Softly* The longer you listen to my voice, the deeper you sink into pure contentment."
    la "*Тихо* Чем дольше ты слушаешь мой голос, тем глубже погружаешься в чистое расслабление."

# game/v1/scene46a.rpy:227
translate russian v1s46a_sgb_c3adf353:

    # u "(Oh, wow... is this shit actually... working? What did I sign up for here?)"
    u "(О, вау... Это дерьмо на самом деле... работает? На что я повёлся?)"

# game/v1/scene46a.rpy:247
translate russian v1s46a_sgb_5c465aac:

    # la "*Softly* The sand between your toes is soft to the touch. You're calm... you're relaxed..."
    la "*Тихо* Песок между пальцами ног мягкий на ощупь. Ты спокоен... ты расслаблен..."

# game/v1/scene46a.rpy:252
translate russian v1s46a_sgb_e251626d:

    # la "*Softly* You've never felt this unwound before... you're the happiest you've ever been."
    la "*Тихо* Ты никогда раньше не чувствовал себя таким раскованным... Ты самый счастливый, чем когда-либо был."

# game/v1/scene46a.rpy:283
translate russian v1s46a_sgb_380b5671:

    # la "She calls you over and asks you to rub lotion on her skin. You can help her."
    la "Она подзывает тебя и просит втереть лосьон в её кожу. Ты можешь ей помочь."

# game/v1/scene46a.rpy:301
translate russian v1s46a_sgb_e98de05d:

    # la "*Softly* She's now asking you to massage the lotion across her breasts."
    la "*Тихо* Теперь она просит тебя помассировать лосьоном её грудь."

# game/v1/scene46a.rpy:314
translate russian v1s46a_sgb_c48914f6:

    # la "*Softly* Rubbing against her soft skin you can feel her nipples, which are stimulated by your touch."
    la "*Тихо* Натирая её нежную кожу, ты можешь почувствовать её соски, которые возбуждаются от твоих прикосновений."

# game/v1/scene46a.rpy:364
translate russian v1s46_nsfwSkipLabel1_0c66800e:

    # la "Oh. My. God. I'm so sorry, I didn't think you'd-"
    la "О. Мой. Бог. Мне так жаль, я не думала, что ты..."

# game/v1/scene46a.rpy:374
translate russian v1s46_nsfwSkipLabel1_8e50d4ee:

    # u "What the... what the fuck?!"
    u "Что за... какого хрена?!"

# game/v1/scene46a.rpy:381
translate russian v1s46_nsfwSkipLabel1_37a8ddeb:

    # la "I'm so sorry! It was fun, though, right? Did it feel real?"
    la "Мне так жаль! Но это было весело, правда? Это было по-настоящему?"

# game/v1/scene46a.rpy:391
translate russian v1s46_nsfwSkipLabel1_64d404d5:

    # la "That's so crazy... anyway, you can just go in the bathroom and clean yourself up."
    la "Это так безумно... в любом случае, ты можешь пойти в ванную и привести себя в порядок."

# game/v1/scene46a.rpy:455
translate russian v1s46_nsfwSkipLabel1_ff0dcc1e:

    # la "You were the best participant I could've asked for."
    la "Вы был лучшим участником, о котором я могла бы попросить."

# game/v1/scene46a.rpy:461
translate russian v1s46_nsfwSkipLabel1_1a9f1e91:

    # u "Ha. That was such a weird experience. Where did you learn this?"
    u "Ха. Это был такой странный опыт. Где ты этому научилась?"

# game/v1/scene46a.rpy:466
translate russian v1s46_nsfwSkipLabel1_74ef0831:

    # la "I think I've, uh, blurred the lines between my classes' lessons and my all-new porn-watching habits."
    la "Мне кажется, я размыла границу между уроками в классе и моими новыми привычками смотреть порно."

# game/v1/scene46a.rpy:471
translate russian v1s46_nsfwSkipLabel1_73a73781:

    # u "Wow. I'm going to remember this the next time you ask to test some psych stuff on me."
    u "Ух ты. Я запомню, когда ты в следующий раз попросишь проверить на мне какие-нибудь психологические штучки."

# game/v1/scene46a.rpy:507
translate russian v1s46_nsfwSkipLabel1_4e85e700:

    # la "My poor, horny guinea pig."
    la "Моя бедная, возбужденная морская свинка."

# game/v1/scene46a.rpy:551
translate russian v1s46_nsfwSkipLabel1_7828686b:

    # la "*Softly* It's the most beautiful day. So sunny and warm. You can feel the light dancing on your skin."
    la "*Тихо* Сегодня самый прекрасный день. Такой солнечный и тёплый. Ты чувствуешь, как свет танцует на твоей коже."

# game/v1/scene46a.rpy:609
translate russian v1s46_nsfwSkipLabel1_49fc6aa7:

    # la "*Softly* But he's not stopping there... he's hungry for something else now. Before you know it, he's chewing at your... member."
    la "*Тихо* Но он не останавливается на достигнутом... теперь он жаждет чего-то большего. Не успеваешь глазом моргнуть, как он уже жуёт твой... член."

# game/v1/scene46a.rpy:634
translate russian v1s46_nsfwSkipLabel1_64d78e0e:

    # la "*Softly* The horse has his eyes on the prize now... before you know it, he takes one quick bite towards your penis and-"
    la "*Тихо* Теперь лошадь смотрит на приз... прежде чем ты успеешь опомниться, она делает один быстрый укус в сторону твоего пениса и..."

# game/v1/scene46a.rpy:639
translate russian v1s46_nsfwSkipLabel1_f5359956:

    # u "Aahhh! Fuck! Oww! M-my balls!"
    u "А-а-а! Блядь! Оуу! М-мои яйца!"

# game/v1/scene46a.rpy:665
translate russian v1s46_nsfwSkipLabel1_853c5ecd:

    # la "I'm sorry, [name]! It was too good of an opportunity! I didn't think the horse dream would actually work on anyone... says more about you than me, really..."
    la "Мне очень жаль, [name]! Это была слишком хорошая возможность! Я не думала, что сон о лошади действительно подействует на кого-то... на самом деле это говорит больше о тебе, чем обо мне..."

# game/v1/scene46a.rpy:675
translate russian v1s46_nsfwSkipLabel1_6849012f:

    # la "All in the name of psych science, I promise. You should be very proud of yourself. It worked perfectly."
    la "И всё это во имя психологии, я обещаю. Ты должен очень гордиться собой. Это сработало идеально."

# game/v1/scene46a.rpy:680
translate russian v1s46_nsfwSkipLabel1_c30e53ab:

    # u "Warped as fuck. You like to take things pretty far at the amateur hour psych evaluations, don't you?"
    u "Ебанутся. Тебе нравится заходить очень далеко в оценках психологической самодеятельности, не так ли?"

# game/v1/scene46a.rpy:685
translate russian v1s46_nsfwSkipLabel1_3615b629:

    # la "Hehe, I saw a few tutorials and just wanted to see if it worked."
    la "Хехе, я видела несколько уроков и просто хотел проверить, сработает ли это."

# game/v1/scene46a.rpy:690
translate russian v1s46_nsfwSkipLabel1_032b6208:

    # u "That was such a weird experience... my balls... they actually hurt a little. I'm going to remember this."
    u "Это был странный опыт... мои яйца... они действительно болели. Я запомню это."

# game/v1/scene46a.rpy:695
translate russian v1s46_nsfwSkipLabel1_4a4198c4:

    # la "Perfect."
    la "Идеально."

# game/v1/scene46a.rpy:702
translate russian v1s46_nsfwSkipLabel1_c05a0a9b:

    # la "Now, I've gotta get caught up on homework, so..."
    la "Мне нужно сделать домашнее задание, так что..."

# game/v1/scene46a.rpy:732
translate russian v1s46_nsfwSkipLabel1_90622431:

    # la "Thanks for doing that with me. I know I have some pretty out-there interests, but... really, thanks."
    la "Спасибо, что делаешь это со мной. Я знаю, что у меня довольно необычные интересы, но... правда, спасибо."

# game/v1/scene46a.rpy:742
translate russian v1s46_nsfwSkipLabel1_6735fd95:

    # la "We'll do something again soon, right?"
    la "Скоро опять что-нибудь замутим, правда?"

# game/v1/scene46a.rpy:747
translate russian v1s46_nsfwSkipLabel1_1998d40e:

    # u "Only if it's a different psych experiment. If it's another hypnosis session... maybe I'll send Imre instead."
    u "Только если это будет другой психологический эксперимент. Если это будет ещё один сеанс гипноза... возможно, я пришлю Имре вместо меня."

# game/v1/scene46a.rpy:752
translate russian v1s46_nsfwSkipLabel1_ac7beb1c:

    # la "Please don't do that to me."
    la "Пожалуйста, не надо."

# game/v1/scene46a.rpy:757
translate russian v1s46_nsfwSkipLabel1_af3d72dd:

    # u "Ha. See ya."
    u "Ха. Увидимся."

# game/v1/scene46a.rpy:769
translate russian v1s46_nsfwSkipLabel1_1f79ba0b:

    # u "(What the fuck just happened...)"
    u "(Хм, ну это было чертовски интересно...)"

# TODO: Translation updated at 2023-04-11 15:01

# game/v1/scene46a.rpy:227
translate russian v1s46a_sgb_dffef3e9:

    # u "(Oh, wow... Is this shit actually... working? What did I sign up for here?)"
    u "(О, вау... Это дерьмо на самом деле... работает? На что я повёлся?)"

# TODO: Translation updated at 2024-03-07 10:56

# game/v1/scene46a.rpy:347
translate russian v1s46a_sgb_4cebb7f9:

    # la "*Softly* She starts to suck on it, her soft lips hugging every inch of you. Sucking and sucking and sucking until-"
    la "*Мягко* Она начинает посасывать его, её мягкие губы обхватывают каждый дюйм кожи. Сосёт, сосёт и сосёт, пока–"

