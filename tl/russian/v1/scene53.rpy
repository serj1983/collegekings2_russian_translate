# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene53.rpy:18
translate russian v1s53_701e8bf9:

    # sa "*Singing* (Inaudible)"
    sa "*Пение* (Неразборчиво)"

# game/v1/scene53.rpy:23
translate russian v1s53_92198626:

    # u "(Is that Sam singing? Where's it coming from?)"
    u "(Это Сэм поет? Откуда это доносится?)"

# game/v1/scene53.rpy:35
translate russian v1s53_7af1e69a:

    # sa "*Singing* And she goes ooh, ooh, yeaaaah... *Laughs*"
    sa "*Пение* И она идет оооо, оооо, даааа... *Смеётся*"

# game/v1/scene53.rpy:40
translate russian v1s53_6b317fa4:

    # sa "*Singing* Maybe make her, maybe break her, the girl of your dreaaammss..."
    sa "*Пение* Может сделать её, может сломать её, девушку твоей мечты..."

# game/v1/scene53.rpy:45
translate russian v1s53_a69a13a4:

    # sa "Oh, my God! Haha... How long have you been standing there staring at me weirdo?"
    sa "О, мой Бог! Ха... Сколько ты уже стоишь и смотришь на меня, чудик?"

# game/v1/scene53.rpy:50
translate russian v1s53_708b48da:

    # u "Not too long. Looks like you're having a great time there, Sam."
    u "Не слишком долго. Похоже, ты отлично проводишь время, Сэм."

# game/v1/scene53.rpy:55
translate russian v1s53_df4e0265:

    # sa "I am."
    sa "Я знаю."

# game/v1/scene53.rpy:60
translate russian v1s53_93db26f5:

    # u "What song was that?"
    u "Что за песня была?"

# game/v1/scene53.rpy:65
translate russian v1s53_a0665ad7:

    # sa "I don't know. I was just making it up. Why, was it good?"
    sa "Я не знаю. Просто придумала. А что, было неплохо?"

# game/v1/scene53.rpy:75
translate russian v1s53_cb820f6c:

    # u "Well, I don't think those lyrics are going to win any awards... like ever. *Chuckles*"
    u "Ну, я не думаю, что эти слова получили бы какие-либо награды.... *Хихикает*"

# game/v1/scene53.rpy:80
translate russian v1s53_143df0be:

    # sa "Okay, dickhead. It's not like I was trying that hard! I'm just singing what comes naturally."
    sa "Ладно, придурок. Не то чтобы я так уж старалась! Я просто пою то, что получается само собой."

# game/v1/scene53.rpy:85
translate russian v1s53_7b33f813:

    # u "Okay, okay... *Laughs* I'm only joking with you."
    u "Ладно, ладно... *Смеётся* Я просто шучу."

# game/v1/scene53.rpy:90
translate russian v1s53_86c1c382:

    # sa "Why do guys have to be so stupid all the fucking time?"
    sa "Почему парни всегда такие тупые?"

# game/v1/scene53.rpy:95
translate russian v1s53_28bf0223:

    # u "Stupid? I said I was joking, Sam."
    u "Тупые? Я же сказал, что пошутил, Сэм."

# game/v1/scene53.rpy:100
translate russian v1s53_8d7a7560:

    # sa "Guess I forgot to laugh then..."
    sa "Видимо, я забыла посмеяться..."

# game/v1/scene53.rpy:108
translate russian v1s53_45b4cb6b:

    # u "Yeah, you've got a nice voice. You should sing more often."
    u "Да, у тебя хороший голос. Тебе стоит петь чаще."

# game/v1/scene53.rpy:118
translate russian v1s53_fe5ca4b3:

    # sa "Do you really think so? Thank you! I do enjoy it a lot, not many people know."
    sa "Ты правда так думаешь? Спасибо! Я действительно получаю от этого большое удовольствие, но не многие об этом знают."

# game/v1/scene53.rpy:123
translate russian v1s53_9e7612f2:

    # u "What are you doing down here on your own anyway?"
    u "Что ты вообще здесь делаешь?"

# game/v1/scene53.rpy:128
translate russian v1s53_6432621f:

    # sa "I just wanted some privacy."
    sa "Я просто захотела уединиться."

# game/v1/scene53.rpy:133
translate russian v1s53_ab6587d8:

    # u "Sorry, should I leave you alone?"
    u "Прости, мне оставить тебя одну?"

# game/v1/scene53.rpy:138
translate russian v1s53_436d7d16:

    # sa "Well, you're here now. You might as well come in and talk with me."
    sa "Ну, ты всё равно здесь. Можешь остаться и поболтать со мной."

# game/v1/scene53.rpy:148
translate russian v1s53_13d7bcee:

    # sa "I bought these pills today, but I don't even know what they are, haha!"
    sa "Я купила эти таблетки сегодня, но даже не знаю, что это такое, хаха!"

# game/v1/scene53.rpy:153
translate russian v1s53_ca66b066:

    # sa "My friend just said if I wanted a good time, this bag is full of them."
    sa "Мой друг сказал, что если я хочу хорошо провести время, то в этой сумке их полно."

# game/v1/scene53.rpy:158
translate russian v1s53_0c0ca133:

    # u "Sam?! You don't know what they are?"
    u "Сэм?! Ты даже не знаешь, что это?"

# game/v1/scene53.rpy:163
translate russian v1s53_beaab853:

    # sa "Oh, relax. I haven't taken any yet, daddy..."
    sa "Ох, расслабься. Я ещё ничего не принимала, папочка..."

# game/v1/scene53.rpy:168
translate russian v1s53_d105b24c:

    # u "(Daddy?) *Chuckles* Well, that's good."
    u "(Папочка?) *Хихикает* Ну, это хорошо."

# game/v1/scene53.rpy:173
translate russian v1s53_2b52e561:

    # sa "I'm thinking about it though."
    sa "Я всё же думаю об этом."

# game/v1/scene53.rpy:182
translate russian v1s53_f9b0f5c4:

    # sa "You know, I overdosed when we were in Amsterdam."
    sa "Знаешь, у меня была передозировка, когда мы были в Амстердаме."

# game/v1/scene53.rpy:188
translate russian v1s53_cee51474:

    # u "You overdosed? Shit, Sam... Cameron mentioned to me that you relapsed, but..."
    u "У тебя передозировка? Чёрт, Сэм... Кэмерон сказал мне, что у тебя был рецидив, но..."

# game/v1/scene53.rpy:193
translate russian v1s53_59fbd277:

    # u "I didn't think that meant an overdose, though. I'm so sorry."
    u "Я не думал, что это означает передозировку. Мне так жаль."

# game/v1/scene53.rpy:199
translate russian v1s53_e3f10a04:

    # sa "For what? Haha... I make my own choices, you know."
    sa "Хаха... Я сама делаю свой выбор, знаешь ли."

# game/v1/scene53.rpy:205
translate russian v1s53_9b7fa09f:

    # sa "Oh, don't be. You did the right thing. I just like to rebel. *Chuckles*"
    sa "О, не стоит. Ты поступил правильно. Мне просто нравится бунтовать. *Хихикает*"

# game/v1/scene53.rpy:211
translate russian v1s53_3c243f99:

    # u "What?! Sam, no. I had no idea."
    u "Что?! Сэм, нет. Я понятия не имел."

# game/v1/scene53.rpy:216
translate russian v1s53_f33cb56d:

    # sa "Ha, yup..."
    sa "Ха, да..."

# game/v1/scene53.rpy:221
translate russian v1s53_ff5fe615:

    # u "An overdose? I can't believe Cameron didn't tell me."
    u "Передозировка? Не могу поверить, что Кэмерон мне не сказал."

# game/v1/scene53.rpy:226
translate russian v1s53_bb742df1:

    # sa "Why would he? It's like family shame, haha. You keep that shit hidden."
    sa "Зачем ему это? Это как семейный позор, ха-ха. Он спрячет это дерьмо."

# game/v1/scene53.rpy:232
translate russian v1s53_a17939af:

    # sa "I relapsed pretty badly while you guys were away..."
    sa "У меня был рецидив, пока тебя не было..."

# game/v1/scene53.rpy:237
translate russian v1s53_c4f114c3:

    # u "Sam..."
    u "Сэм..."

# game/v1/scene53.rpy:242
translate russian v1s53_fd1e3ee6:

    # sa "Nothing bad, but... I just can't find another source of happiness or comfort, [name]."
    sa "Ничего плохого, но... Я просто не могу найти другого источника счастья или утешения, [name]."

# game/v1/scene53.rpy:247
translate russian v1s53_9b61b18d:

    # sa "Cameron knows something is up, but I can't talk to him. One mention of wanting to pop a pill, and he'll throw me into the hospital."
    sa "Кэмерон знает, что что-то случилось, но я не могу с ним об этом поговорить. Одно упоминание о том, что я хочу выпить таблетку, и он отправит меня в больницу."

# game/v1/scene53.rpy:252
translate russian v1s53_a6fc21e0:

    # u "You know you can talk to me."
    u "Ты знаешь, что можешь поговорить со мной."

# game/v1/scene53.rpy:257
translate russian v1s53_5097ce5f:

    # sa "*Chuckles*"
    sa "*Хихикает*"

# game/v1/scene53.rpy:267
translate russian v1s53_2619a426:

    # sa "Anyway, that happened. I survived. And it wasn't on purpose, just so you know."
    sa "Так или иначе, это случилось. Я выжила. И это было не специально, просто чтобы ты знал."

# game/v1/scene53.rpy:272
translate russian v1s53_31c849b6:

    # u "Well, that's one positive thing that I've heard from this conversation at least."
    u "Что ж, это положительный момент, который я, по крайней мере, услышал из этого разговора."

# game/v1/scene53.rpy:277
translate russian v1s53_3717c582:

    # sa "I was just having a good time and went too far. I make a habit of that."
    sa "Я просто хорошо проводила время и зашла слишком далеко. У меня это вошло в привычку."

# game/v1/scene53.rpy:282
translate russian v1s53_5460aa1e:

    # u "You don't need a baggie full of \"good time\" pills to-"
    u "Тебе не нужна сумка, полная таблеток \"хорошего времени\"."

# game/v1/scene53.rpy:287
translate russian v1s53_cf3e542f:

    # sa "Yeah, yeah. I don't need drugs to have fun. I get it, I know."
    sa "Да, да. Мне не нужны наркотики, чтобы веселиться. Я понимаю это и знаю."

# game/v1/scene53.rpy:292
translate russian v1s53_ed52b5e2:

    # u "So, why take them?"
    u "Так зачем ты их принимаешь?"

# game/v1/scene53.rpy:297
translate russian v1s53_b609f90d:

    # sa "I take drugs because I can't deal with any of my emotional baggage without them."
    sa "Я принимаю наркотики, потому что без них я не могу справиться со своим эмоциональным багажом."

# game/v1/scene53.rpy:302
translate russian v1s53_e0fcb5c7:

    # sa "All the things that have happened in my life... It isn't easy coming from a broken home, you know?"
    sa "Всё то, что случилось в моей жизни... не легко, учитывая, что я живу в разрушенной семье, понимаешь?"

# game/v1/scene53.rpy:312
translate russian v1s53_2c0d18d9:

    # sa "And when I'm around other people who take drugs... *Sniffles*"
    sa "А когда я нахожусь рядом с людьми, которые принимают наркотики... *Шмыгает носом*"

# game/v1/scene53.rpy:317
translate russian v1s53_4f070b66:

    # sa "It makes me feel like, like I belong somewhere... *Sniffles* Do you know what I mean?"
    sa "Это заставляет меня чувствовать, что я где-то на своем месте... *Шмыгает носом* Ты понимаешь, о чём я?"

# game/v1/scene53.rpy:322
translate russian v1s53_fbe52ea5:

    # u "(How do I want to go about this? Convince her to stay off drugs? Is that even possible? Or should I just let her continue, as she knows her own limits?"
    u "(Как я хочу поступить? Убедить её не принимать наркотики? Возможно ли это вообще? Или я должен просто позволить ей продолжать, поскольку она сама знает свои пределы?"

# game/v1/scene53.rpy:338
translate russian v1s53_e980ab4c:

    # u "I know exactly what you mean, Sam. Look at me."
    u "Я прекрасно понимаю, о чём ты, Сэм. Посмотри на меня."

# game/v1/scene53.rpy:348
translate russian v1s53_28320583:

    # u "I know it's not easy. Everyone on this planet can tell you that life isn't easy."
    u "Я знаю, что это нелегко. Каждый человек на этой планете может сказать тебе, что жизнь трудна."

# game/v1/scene53.rpy:353
translate russian v1s53_e0e0cbdc:

    # u "All I can say is what I see from the outside, and I can see that this shit is destroying your life. Look at you right now."
    u "Я могу сказать только то, что вижу со своей стороны, и я вижу, что это дерьмо разрушает твою жизнь. Посмотри на себя сейчас."

# game/v1/scene53.rpy:358
translate russian v1s53_8b0a7912:

    # sa "*Crying* But I don't know how to deal with any of my problems without drugs, [name]!"
    sa "*Плачет* Но я не знаю, как справиться со своими проблемами без наркотиков, [name]!"

# game/v1/scene53.rpy:363
translate russian v1s53_5d25cbb0:

    # sa "*Crying* All of life's shit is just sitting there, waiting for me to come down from being high."
    sa "*Плачет* Всё жизненное дерьмо просто сидит там, ожидая, что я спущусь с небес."

# game/v1/scene53.rpy:368
translate russian v1s53_90fd42f4:

    # u "Drugs give people a distraction from their problems for a moment, and that's fucking magical. I admit."
    u "Наркотики позволяют людям на мгновение отвлечься от своих проблем, и это чертовски волшебно. Я признаю."

# game/v1/scene53.rpy:374
translate russian v1s53_cce65445:

    # u "But you don't have enough self-control to live this way. You overdosed. You're risking your life, Sam."
    u "Но у тебя недостаточно самоконтроля, чтобы так жить. У тебя была передозировка. Ты рискуешь своей жизнью, Сэм."

# game/v1/scene53.rpy:380
translate russian v1s53_90028c38:

    # u "But you don't have enough self-control to live this way. You continue to relapse. You're dependent on drugs, Sam."
    u "Но у тебя недостаточно самоконтроля, чтобы так жить. Ты продолжаешь рецидивировать. Ты зависима от наркотиков, Сэм."

# game/v1/scene53.rpy:385
translate russian v1s53_e27affe3:

    # sa "*Sobbing*"
    sa "*Рыдает*"

# game/v1/scene53.rpy:390
translate russian v1s53_51d03691:

    # u "I think you need to get help on how to deal with the emotional stuff. Real help. From people who want to help. There's a way out of all this."
    u "Думаю, тебе нужно получить помощь в том, как справляться с эмоциональными проблемами. Реальная помощь. От людей, которые могут помочь. Из всего этого есть выход."

# game/v1/scene53.rpy:395
translate russian v1s53_123dae77:

    # sa "*Sniffles* ..."
    sa "*Шмыгает* ..."

# game/v1/scene53.rpy:400
translate russian v1s53_5535289b:

    # sa "Will you help me?"
    sa "Ты поможешь мне?"

# game/v1/scene53.rpy:405
translate russian v1s53_b05e2633:

    # u "Of course, I will."
    u "Конечно, помогу."

# game/v1/scene53.rpy:410
translate russian v1s53_4c2833ed:

    # u "*Chuckles* What?"
    u "*Хихикает* Что?"

# game/v1/scene53.rpy:422
translate russian v1s53_7cab5b8c:

    # u "(Oh!)"
    u "(Ох!)"

# game/v1/scene53.rpy:452
translate russian v1s53_8e3a0943:

    # u "Mmm-"
    u "Ммм-"

# game/v1/scene53.rpy:457
translate russian v1s53_d734cefc:

    # u "Sorry, I just-"
    u "Извини, я просто-"

# game/v1/scene53.rpy:462
translate russian v1s53_3f6eec6b:

    # sa "It's okay. This is bad timing."
    sa "Всё в порядке. Это неподходящее время."

# game/v1/scene53.rpy:467
translate russian v1s53_a6ebe6b2:

    # u "(Phew...)"
    u "(Уф...)"

# game/v1/scene53.rpy:472
translate russian v1s53_6627059e:

    # sa "I uh, I don't want to take these pills anymore... Could you throw them away for me?"
    sa "Я... я больше не хочу принимать эти таблетки... Не мог бы ты выбросить их для меня?"

# game/v1/scene53.rpy:477
translate russian v1s53_9467f36c:

    # u "Good idea. Absolutely."
    u "Хорошая идея."

# game/v1/scene53.rpy:487
translate russian v1s53_b8de57e2:

    # sa "I need to go to bed now... Haha."
    sa "Мне нужно идти спать... Хаха."

# game/v1/scene53.rpy:492
translate russian v1s53_12ab0372:

    # u "Another great idea."
    u "Ещё одна отличная идея."

# game/v1/scene53.rpy:497
translate russian v1s53_986f33b9:

    # sa "Thank you."
    sa "Спасибо."

# game/v1/scene53.rpy:502
translate russian v1s53_c08200b4:

    # u "You're welcome, Sam."
    u "Не за что, Сэм."

# game/v1/scene53.rpy:517
translate russian v1s53_85090988:

    # u "(*Sighs* I feel good about that. I think I did the right thing.)"
    u "(*Вздыхает* Я чувствую себя хорошо, думаю, что поступил правильно.)"

# game/v1/scene53.rpy:529
translate russian v1s53_6b6745cc:

    # u "Yeah, I know exactly what you mean."
    u "Да, я точно знаю, что ты имеешь в виду."

# game/v1/scene53.rpy:534
translate russian v1s53_8a8d4f8f:

    # u "I'm not going to judge you, Sam. We all have different coping mechanisms for the things going on in our lives."
    u "Я не собираюсь судить тебя, Сэм. У всех нас разные механизмы преодоления того, что происходит в нашей жизни."

# game/v1/scene53.rpy:539
translate russian v1s53_2c7b4b54:

    # u "This is yours. I'm sure you're capable of figuring out how to manage it better at some point."
    u "Это твоё. Уверен, что в какой-то момент ты сможешь понять, как лучше с этим справиться."

# game/v1/scene53.rpy:544
translate russian v1s53_27d479be:

    # sa "Thank you. *Chuckles*"
    sa "Спасибо. *Хихикает*"

# game/v1/scene53.rpy:549
translate russian v1s53_29561d32:

    # u "Of course, I'll be here no matter what."
    u "Конечно, я буду здесь, несмотря ни на что."

# game/v1/scene53.rpy:554
translate russian v1s53_f21047e8:

    # sa "Sometimes I feel like you're the only one who's ever fully understood me."
    sa "Иногда мне кажется, что ты единственный, кто когда-либо полностью понимал меня."

# game/v1/scene53.rpy:564
translate russian v1s53_06faf2a4:

    # u "(Oh... Wow.)"
    u "(О... Вау.)"

# game/v1/scene53.rpy:569
translate russian v1s53_1f55fc2c:

    # sa "The last guy I let inside me never had anything nice to say... I always attract the jerks."
    sa "Последний парень, которого я приняла, никогда не мог сказать ничего хорошего... Я всегда привлекаю придурков."

# game/v1/scene53.rpy:574
translate russian v1s53_77a8e55a:

    # sa "But you're different... Aren't you?"
    sa "Но ты другой... Не так ли?"

# game/v1/scene53.rpy:584
translate russian v1s53_1d768534:

    # u "(Oh my...)"
    u "(О, мой...)"

# game/v1/scene53.rpy:589
translate russian v1s53_bd61eed8:

    # sa "Take these pants off. I want you. Now."
    sa "Сними эти штаны. Я хочу тебя. Сейчас."

# game/v1/scene53.rpy:618
translate russian v1s53_sg_27f35ecb:

    # u "Um, actually... I don't think that's a good idea, Sam."
    u "Вообще-то... Я не думаю, что это хорошая идея, Сэм."

# game/v1/scene53.rpy:623
translate russian v1s53_sg_716a0809:

    # sa "What? W- Why not?! You think I'm hot, don't you?"
    sa "Что? П- Почему нет?! Ты думаешь, я несексуальная, да?"

# game/v1/scene53.rpy:628
translate russian v1s53_sg_69fe7c70:

    # u "That's not the point. You've been drinking, and I just don't think it's a good idea."
    u "Дело не в этом. Ты выпила, я просто не думаю, что это хорошая идея."

# game/v1/scene53.rpy:633
translate russian v1s53_sg_f9323bfb:

    # sa "This is my second beer, [name]."
    sa "Это моё второе пиво, [name]."

# game/v1/scene53.rpy:639
translate russian v1s53_sg_1bd2f38d:

    # sa "See, I was right! Ha! All of you are stupid dickheads. Every single one of you!"
    sa "Видишь, я была права! Ха! Все вы тупые придурки. Все до единого!"

# game/v1/scene53.rpy:645
translate russian v1s53_sg_b9c54294:

    # sa "What the fuck do you want from me then, [name]? Why did you even come talk to me?"
    sa "Какого хрена тебе тогда от меня надо, [name]? Зачем ты вообще пришёл поговорить со мной?"

# game/v1/scene53.rpy:650
translate russian v1s53_sg_c4f114c3:

    # u "Sam..."
    u "Сэм..."

# game/v1/scene53.rpy:655
translate russian v1s53_sg_65887fc6:

    # u "Sam!"
    u "Сэм!"

# game/v1/scene53.rpy:660
translate russian v1s53_sg_0abd2837:

    # u "*Sighs* (I hope she doesn't hate me in the morning.)"
    u "*Вздыхает* (Надеюсь, утром она не будет меня ненавидеть.)"

translate russian strings:

    # game/v1/scene53.rpy:327
    old "Help Samantha be clean"
    new "Помогите Саманте быть чистой"

    # game/v1/scene53.rpy:327
    old "Let Samantha enjoy drugs"
    new "Позволить Саманте наслаждаться наркотиками"

    # game/v1/scene53.rpy:427
    old "Accept the kiss"
    new "Принять поцелуй"

    # game/v1/scene53.rpy:427
    old "Reject the kiss"
    new "Отказать в поцелуи"

    # game/v1/scene53.rpy:594
    old "Take your pants off"
    new "Снять штаны"

    # game/v1/scene53.rpy:594
    old "Don't take your pants off"
    new "Не снимать штаны"

