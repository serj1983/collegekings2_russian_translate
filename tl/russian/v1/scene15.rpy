# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene15.rpy:22
translate russian v1s15_bb349984:

    # u "(Holy... shit. This is serious!)"
    u "(Святое... дерьмо. Это серьёзно!)"

# game/v1/scene15.rpy:42
translate russian v1s15_88446de6:

    # u "(I can see why Chloe was freaking out now. I don't see a single Chloe poster. She must feel like she can't compete or has already lost.)"
    u "(Теперь я понимаю, почему Хлоя так волновалась. Я не вижу ни одного плаката с Хлоей. Она, должно быть, чувствует, что не может конкурировать или уже проиграла.)"

# game/v1/scene15.rpy:47
translate russian v1s15_9299001e:

    # imre "WOLVES SAY, CHLOE FOR PRESIDENT!"
    imre "ВОЛКИ ГОВОРЯТ: ХЛОЮ В ПРЕЗИДЕНТЫ!"

# game/v1/scene15.rpy:52
translate russian v1s15_83d9d072:

    # imre "RESPECT LOYALTY, RESPECT TRADITION!"
    imre "УВАЖАЙТЕ ВЕРНОСТЬ, УВАЖАЙТЕ ТРАДИЦИИ!"

# game/v1/scene15.rpy:57
translate russian v1s15_c81bb937:

    # imre "DON'T BE A TRAITOR LIKE LINDSEY!"
    imre "НЕ БУДЬ ПРЕДАТЕЛЕМ, КАК ЛИНДСИ!"

# game/v1/scene15.rpy:62
translate russian v1s15_845062c8:

    # imre "WOLVES SAY, VOTE FOR CHLOE!"
    imre "ВОЛКИ ГОВОРЯТ, ГОЛОСУЙТЕ ЗА ХЛОЮ!"

# game/v1/scene15.rpy:72
translate russian v1s15_2de77226:

    # u "Hey man, what the hell is going on?"
    u "Эй, мужик, что, чёрт возьми, происходит?"

# game/v1/scene15.rpy:77
translate russian v1s15_b9181ec4:

    # imre "Isn't it obvious? Lindsey is running for President, dude."
    imre "Разве не очевидно? Линдси баллотируется в президенты, чувак."

# game/v1/scene15.rpy:82
translate russian v1s15_7e608899:

    # u "Yeah, so? What are you doing out here?"
    u "Да, и что? Что ты здесь делаешь?"

# game/v1/scene15.rpy:87
translate russian v1s15_62088ee9:

    # imre "Supporting Chloe..."
    imre "Поддерживаю Хлою..."

# game/v1/scene15.rpy:92
translate russian v1s15_423c67d8:

    # u "I didn't know you and Chloe were that close."
    u "Я не знал, что вы с Хлоей так близки."

# game/v1/scene15.rpy:97
translate russian v1s15_31fd76f1:

    # imre "We're not, but the Chicks President should be the hottest girl and Chloe is the hottest."
    imre "Это не так, но президентом Чикс должна быть самая сексуальная девушка, а Хлоя - самая сексуальная."

# game/v1/scene15.rpy:102
translate russian v1s15_635d9a37:

    # u "Wait, that's how you're deciding? *Chuckles*"
    u "Подожди, так вот как ты думаешь? *Хихикает*"

# game/v1/scene15.rpy:107
translate russian v1s15_6641ae50:

    # imre "That's how I decide everything. It's a very fair and balanced scale."
    imre "Да, так думаю. Это очень справедливая и сбалансированная шкала."

# game/v1/scene15.rpy:115
translate russian v1s15_0ea83b13:

    # u "Not gonna lie, that seems like a genius way to decide things, haha."
    u "Не буду лгать, это кажется гениальным способом решить вопросы, хаха."

# game/v1/scene15.rpy:120
translate russian v1s15_c9eee82c:

    # imre "Many call me the Einstein of decision making."
    imre "Многие называют меня Эйнштейном в принятии решений."

# game/v1/scene15.rpy:128
translate russian v1s15_2be8c854:

    # u "Not gonna lie, that is a ridiculously stupid way to make decisions, haha."
    u "Не буду врать, это до смешного глупый способ принимать решения, хаха."

# game/v1/scene15.rpy:133
translate russian v1s15_9d9e7dcd:

    # imre "I'm just ahead of my time."
    imre "Я просто опережаю своё время."

# game/v1/scene15.rpy:138
translate russian v1s15_baeb52d0:

    # u "Why are you saying the Wolves are supporting Chloe, though?"
    u "Почему ты говоришь, что Волки поддерживают Хлою?"

# game/v1/scene15.rpy:143
translate russian v1s15_4a8da7c6:

    # imre "Oh, 'cause that part is true. Chris and Chloe have always been tight, and they get along well as Presidents."
    imre "Ох, потому что это правда. Крис и Хлоя всегда были близки, и они хорошо ладят как президенты."

# game/v1/scene15.rpy:145
translate russian v1s15_26c6f3ac:

    # imre "There's no doubt in my mind that this is what Chris would want me to be doing. So, if you'll excuse me, I'll be getting back to impressing the pretty woma-"
    imre "Я не сомневаюсь, что Крис хотел бы, чтобы я занимался именно этим. Так что, ты меня извини, но я вернусь чтобы произвести впечатление на хорошенькую женщи-"

# game/v1/scene15.rpy:150
translate russian v1s15_fa92c5c4:

    # imre "Er, I mean, Chloe..."
    imre "Э-э, я имею в виду, Хлою..."

# game/v1/scene15.rpy:155
translate russian v1s15_ebf2fe68:

    # imre "VOTE FOR CHLOE, LOOK AT THAT RACK!"
    imre "ГОЛОСУЙ ЗА ХЛОЮ!"

# game/v1/scene15.rpy:160
translate russian v1s15_3d1a9aee:

    # u "(That man is a whole ass character, but I think he has more than one reason why he's on Chloe's side. *Laughs*)"
    u "(Он - полный придурок, но я думаю, что у него есть более чем одна причина, по которой он на стороне Хлои. *Смеётся*)"

translate russian strings:

    # game/v1/scene15.rpy:112
    old "It's genius"
    new "Это гениально"

    # game/v1/scene15.rpy:112
    old "It's idiotic"
    new "Это идиотизм"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene15.rpy:22
translate russian v1s15_407da7a0:

    # u "(Holy shit... this is serious!)"
    u "(Срань господня... это серьёзно!)."

# game/v1/scene15.rpy:102
translate russian v1s15_88ef1056:

    # u "Wait, that's how you're deciding?"
    u "Подожди, так вот как ты решил?"

# game/v1/scene15.rpy:115
translate russian v1s15_9994f43b:

    # u "Ha! Not gonna lie, that seems like a genius way to decide things."
    u "Ха! Не буду врать, это кажется гениальным способом решать проблемы."

# game/v1/scene15.rpy:128
translate russian v1s15_33cd31ac:

    # u "Not gonna lie, that is a ridiculously stupid, hilarious way to make decisions."
    u "Не буду врать, это нелепый и смешной способ принятия решений."

# game/v1/scene15.rpy:143
translate russian v1s15_d16a604d:

    # imre "Oh, 'cause that part is true. Chris and Chloe have always been tight, and they get along well as presidents."
    imre "О, потому что это правда. Крис и Хлоя всегда были близки, и они хорошо ладят, как президенты."

# game/v1/scene15.rpy:160
translate russian v1s15_860f072f:

    # u "(That man is a character. Looks like there's way more than one reason he's on Chloe's side.)"
    u "(Этот человек с характером. Похоже, есть много причин, почему он на стороне Хлои.)"

