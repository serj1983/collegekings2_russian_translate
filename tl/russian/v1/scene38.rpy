# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene38.rpy:17
translate russian v1s38_408fa180:

    # u "Jenny!"
    u "Дженни!"

# game/v1/scene38.rpy:22
translate russian v1s38_01582f75:

    # jen "Oh, hey again. *Chuckles*"
    jen "О, снова привет. *Хихикает*"

# game/v1/scene38.rpy:27
translate russian v1s38_ac1012c5:

    # u "I hung back for a quick phone call, I thought you'd be gone by now."
    u "Я отошёл, чтобы быстро позвонить, я думал, что ты уже ушла."

# game/v1/scene38.rpy:32
translate russian v1s38_8c95f043:

    # jen "Oh, I'm honestly taking my sweet time tonight."
    jen "Честно говоря, я сегодня не тороплюсь."

# game/v1/scene38.rpy:37
translate russian v1s38_b88f5f11:

    # u "It doesn't bother you to walk around alone at night?"
    u "Тебя не смущает, что ты ходишь одна по ночам?"

# game/v1/scene38.rpy:42
translate russian v1s38_e55a29fc:

    # jen "Not really, I used to do it all the time growing up. Nothing's happened so far, haha."
    jen "Не совсем, я постоянно этим занималась в детстве. Пока ничего не случилось, хаха."

# game/v1/scene38.rpy:47
translate russian v1s38_d94c33dc:

    # u "Okay, well don't say it like that. *Laughs* Hopefully nothing ever happens."
    u "Ладно, не говори так. *Смеётся* Надеюсь, ничего не случится."

# game/v1/scene38.rpy:52
translate russian v1s38_14265644:

    # jen "If you keep stalking me I won't even have to worry."
    jen "Если ты продолжишь преследовать меня, мне даже не придется беспокоиться."

# game/v1/scene38.rpy:61
translate russian v1s38_ea56debd:

    # u "I wasn't trying to-"
    u "Я не пытался..."

# game/v1/scene38.rpy:66
translate russian v1s38_ae1cde73:

    # jen "Relax, I'm kidding."
    jen "Расслабься, я шучу."

# game/v1/scene38.rpy:71
translate russian v1s38_5c5f2149:

    # u "*Laughs* Cool, I don't want you thinking I'm some creep."
    u "*Смеётся* Круто, я не хочу, чтобы ты подумала, что я какой-то урод."

# game/v1/scene38.rpy:76
translate russian v1s38_2e6ea52e:

    # jen "Trust me, if I did, I would've said something by now."
    jen "Поверь мне, если бы я знала, я бы уже что-то сказала."

# game/v1/scene38.rpy:81
translate russian v1s38_c3af0de6:

    # u "So, since you haven't said anything by now..."
    u "Итак, раз уж ты до сих пор ничего не сказала..."

# game/v1/scene38.rpy:86
translate russian v1s38_2068b08d:

    # jen "I want to be stalked by you, yes."
    jen "Я хочу, чтобы ты меня преследовал, да."

# game/v1/scene38.rpy:94
translate russian v1s38_1c6694f5:

    # u "Well maybe if you weren't so cute I wouldn't have to pretend to run into you all the time, haha."
    u "Ну, может быть, если бы ты не была такой милой, мне бы не пришлось притворяться, что я постоянно с тобой сталкиваюсь, хаха."

# game/v1/scene38.rpy:99
translate russian v1s38_d3320a0d:

    # jen "Awww, haha. Fine you can keep stalking me."
    jen "Оу, хаха. Ладно, можешь продолжать преследовать меня."

# game/v1/scene38.rpy:105
translate russian v1s38_8569c4fa:

    # u "*Chuckles*"
    u "*Хихикает*"

# game/v1/scene38.rpy:110
translate russian v1s38_87650a34:

    # jen "*Laughs*"
    jen "*Смеётся*"

# game/v1/scene38.rpy:120
translate russian v1s38_b302ca5b:

    # u "Oh, guess this is where our paths change for the night."
    u "О, похоже, здесь наши пути меняются на ночь."

# game/v1/scene38.rpy:125
translate russian v1s38_a80c6e3a:

    # jen "Yep, haha. See you soon?"
    jen "Да, хаха. Скоро увидимся?"

# game/v1/scene38.rpy:130
translate russian v1s38_ca208754:

    # u "You better."
    u "Тебе лучше."

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene38.rpy:22
translate russian v1s38_e62a6da2:

    # jen "Oh, hey again."
    jen "О, снова привет."

# game/v1/scene38.rpy:27
translate russian v1s38_f31623bb:

    # u "I hung back for a quick phone call. I thought you'd be gone by now."
    u "Я отошёл, чтобы быстро позвонить, я думал, что ты уже ушла."

# game/v1/scene38.rpy:42
translate russian v1s38_e8dad41f:

    # jen "Not really, I used to do it all the time growing up. Nothing's happened so far."
    jen "Не совсем, я постоянно этим занималась в детстве. Пока ничего не случилось."

# game/v1/scene38.rpy:47
translate russian v1s38_84a2fa32:

    # u "Okay, well, don't say it like that, ha. Hopefully nothing ever happens."
    u "Ладно, не говори так. Надеюсь, ничего не случится."

# game/v1/scene38.rpy:71
translate russian v1s38_c74437f2:

    # u "Cool. I don't want you thinking I'm some creep."
    u "Круто, я не хочу, чтобы ты подумала, что я какой-то урод."

# game/v1/scene38.rpy:94
translate russian v1s38_67528f93:

    # u "Well maybe if you weren't so cute I wouldn't have to pretend to run into you all the time."
    u "Ну, может быть, если бы ты не была такой милой, мне бы не пришлось притворяться, что я постоянно с тобой сталкиваюсь."

# game/v1/scene38.rpy:99
translate russian v1s38_d896912a:

    # jen "Awww. Fine, you can keep stalking me."
    jen "Оу. Ладно, можешь продолжать преследовать меня."

# game/v1/scene38.rpy:105
translate russian v1s38_1a6449c3:

    # u "Ha!"
    u "Ха!"

# game/v1/scene38.rpy:110
translate russian v1s38_73e6e395:

    # jen "My favorite stalker."
    jen "Мой любимый преследователь."

# game/v1/scene38.rpy:120
translate russian v1s38_5658c09e:

    # u "Oh, guess this is where our paths part for the night."
    u "О, похоже, здесь наши пути меняются на ночь."

# game/v1/scene38.rpy:125
translate russian v1s38_1e205cc9:

    # jen "Yep. See you soon?"
    jen "Да. Скоро увидимся?"

