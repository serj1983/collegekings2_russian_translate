# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene30.rpy:27
translate russian v1s30_32a51c82:

    # cl "[name], there you are!"
    cl "[name], вот ты где!"

# game/v1/scene30.rpy:37
translate russian v1s30_74e4cb3b:

    # u "Hey, how's it going?"
    u "Привет, как дела?"

# game/v1/scene30.rpy:42
translate russian v1s30_3ad042ca:

    # cl "Not too bad... Although, this thing was definitely NOT cheap."
    cl "Не так уж плохо... Хотя, эта штука определенно НЕ была дешевой."

# game/v1/scene30.rpy:47
translate russian v1s30_c095ea42:

    # u "How bad was it?"
    u "Насколько всё плохо?"

# game/v1/scene30.rpy:52
translate russian v1s30_57dd0cb5:

    # cl "Well, for it being the only one left at the auction, let's just say it was right under our budget."
    cl "Ну, поскольку он был единственным оставшимся на аукционе, скажем так, он уложился в наш бюджет."

# game/v1/scene30.rpy:57
translate russian v1s30_f31c5072:

    # u "Damn."
    u "Чёрт."

# game/v1/scene30.rpy:63
translate russian v1s30_73f6ff00:

    # cl "Chris helped a bit though, so that was nice."
    cl "Хотя Крис немного помог, так что это было здорово."

# game/v1/scene30.rpy:68
translate russian v1s30_d58ff1f4:

    # u "This better be a nice ass toy..."
    u "Надеюсь, это хорошая игрушка для задницы..."

# game/v1/scene30.rpy:73
translate russian v1s30_3764a626:

    # cl "Haha, it is pretty fancy!"
    cl "Ха-ха, это очень модно!"

# game/v1/scene30.rpy:75
translate russian v1s30_556b46e7:

    # cl "As soon as we're all set up, I'm just gonna take a few shots and then we can pick our favorites."
    cl "Как только мы всё подготовим, я сделаю несколько снимков, а потом мы сможем выбрать наиболее понравившиеся."

# game/v1/scene30.rpy:80
translate russian v1s30_e499616e:

    # u "So, it's just you and the wolf?"
    u "Значит, остались только ты и волк?"

# game/v1/scene30.rpy:85
translate russian v1s30_8adc7021:

    # cl "Yeah, I kinda felt like it was too much to ask individual people to get involved, you know?"
    cl "Да, мне казалось, что это слишком много - просить отдельных людей принять участие."

# game/v1/scene30.rpy:90
translate russian v1s30_f36ef9ff:

    # u "Or did Chris tell you that... *Chuckles*"
    u "Или Крис сказал тебе, что... *Хихикает*"

# game/v1/scene30.rpy:95
translate russian v1s30_6120c7c9:

    # cl "Okay, yes..."
    cl "Ладно, да..."

# game/v1/scene30.rpy:97
translate russian v1s30_a09604ce:

    # cl "As long as they are supporting me I'm happy."
    cl "Пока они поддерживают меня, я счастлива."

# game/v1/scene30.rpy:102
translate russian v1s30_0bbd3520:

    # u "Can't go wrong with that."
    u "С этим нельзя ошибиться."

# game/v1/scene30.rpy:107
translate russian v1s30_434b9e09:

    # cl "Okay so..."
    cl "Итак..."

# game/v1/scene30.rpy:117
translate russian v1s30_4898ed50:

    # u "You want me to take them?"
    u "Хочешь, чтобы я их забрал?"

# game/v1/scene30.rpy:122
translate russian v1s30_3e3daac6:

    # cl "Yes... Please. *Laughs*"
    cl "Да... Пожалуйста. *Смеётся*"

# game/v1/scene30.rpy:127
translate russian v1s30_9b0a5153:

    # u "Okay, ready when you are."
    u "Хорошо, буду готов, когда ты будешь."

# game/v1/scene30.rpy:149
translate russian v1s30_c2d0308a:

    # u "Okay, I think I've got a pretty good selection to choose from."
    u "Ладно, думаю, у меня есть из чего выбрать."

# game/v1/scene30.rpy:159
translate russian v1s30_264cc31d:

    # cl "How do they look? Too cheesy?"
    cl "Как оно выглядят? Слишком банально?"

# game/v1/scene30.rpy:164
translate russian v1s30_8ba88eb7:

    # u "Haha, no. You look great as always."
    u "Хаха, нет. Ты выглядишь великолепно, как всегда."

# game/v1/scene30.rpy:180
translate russian v1s30_aac0bbae:

    # cl "Thank you."
    cl "Спасибо."

# game/v1/scene30.rpy:185
translate russian v1s30_1b28a406:

    # u "Of course."
    u "Пожалуйста."

# game/v1/scene30.rpy:190
translate russian v1s30_dd62ef0e:

    # cl "Okay, we can leave the stuff here because I think some of The Wolves wanted to... play with it. *Laughs*"
    cl "Ладно, мы можем оставить вещи здесь, потому что я думаю, что некоторые из Волков хотели... поиграть с ними. *Смеётся*"

# game/v1/scene30.rpy:195
translate russian v1s30_184f1b2a:

    # u "Haha, what?"
    u "Хаха, что?"

# game/v1/scene30.rpy:200
translate russian v1s30_05676f94:

    # cl "I try not to ask too many questions, you know?"
    cl "Я стараюсь не задавать слишком много вопросов."

# game/v1/scene30.rpy:202
translate russian v1s30_20e70674:

    # cl "Alright, let's head back and look at these closely so we can pick a good one."
    cl "Хорошо, давай вернёмся и внимательно рассмотрим их, чтобы выбрать хороший вариант."

translate russian strings:

    # game/v1/scene30.rpy:129
    old "Take Picture"
    new "Сфотографировать"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene30.rpy:42
translate russian v1s30_38f0dd52:

    # cl "Not too bad... although, this thing was definitely NOT cheap."
    cl "Не так уж плохо... хотя, эта штука определенно НЕ была дешевой."

# game/v1/scene30.rpy:68
translate russian v1s30_fe81a821:

    # u "This better be a nice-ass toy..."
    u "Надеюсь, это будет классная игрушка..."

# game/v1/scene30.rpy:73
translate russian v1s30_5199e85e:

    # cl "It is pretty fancy!"
    cl "Это очень модно!"

# game/v1/scene30.rpy:90
translate russian v1s30_f23f0960:

    # u "Or did Chris tell you that..."
    u "Или Крис сказал тебе, что..."

# game/v1/scene30.rpy:97
translate russian v1s30_2ca6e59f:

    # cl "As long as they are supporting me, I'm happy."
    cl "Пока они поддерживают меня, Я счастлива."

# game/v1/scene30.rpy:122
translate russian v1s30_6148c723:

    # cl "Yes... please."
    cl "Да... Пожалуйста."

# game/v1/scene30.rpy:164
translate russian v1s30_3a10513a:

    # u "Ha, no. You look great, as always."
    u "Ха, нет. Ты выглядишь великолепно, как всегда."

# game/v1/scene30.rpy:190
translate russian v1s30_37942a4a:

    # cl "Okay, we can leave the stuff here because I think some of the Wolves wanted to... play with it."
    cl "Ладно, мы можем оставить вещи здесь, потому что я думаю, что некоторые из Волков хотели... поиграть с ними."

# game/v1/scene30.rpy:195
translate russian v1s30_9fd5fe5a:

    # u "Ha, what?"
    u "Ха, что?"

# TODO: Translation updated at 2023-04-11 15:01

# game/v1/scene30.rpy:202
translate russian v1s30_50450572:

    # cl "All right, let's head back and look at these closely so we can pick a good one."
    cl "Хорошо, давай вернёмся и внимательно рассмотрим их, чтобы выбрать хороший вариант."

