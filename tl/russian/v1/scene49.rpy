# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene49.rpy:15
translate russian v1s49_b52502ba:

    # u "(What do we have here?)"
    u "(Что у нас здесь?)"

# game/v1/scene49.rpy:35
translate russian v1s49Lindsey_PhoneContinue_f9e9813d:

    # u "(I should reply to Lindsey.)"
    u "(Я должен ответить Линдси.)"

# game/v1/scene49.rpy:42
translate russian v1s49Lindsey_PhoneContinue_33671ad1:

    # u "Hey, Lindsey."
    u "Привет, Линдси."

# game/v1/scene49.rpy:47
translate russian v1s49Lindsey_PhoneContinue_433518ef:

    # li "Hey, now is the perfect time. Text Chloe and tell her to meet you at Classico Cuisine."
    li "Эй, сейчас самое подходящее время. Напиши Хлое и попроси её встретиться с тобой в Classico Cuisine."

# game/v1/scene49.rpy:53
translate russian v1s49Lindsey_PhoneContinue_93b710e4:

    # u "Oh, right. Yeah."
    u "О, точно. Да."

# game/v1/scene49.rpy:58
translate russian v1s49Lindsey_PhoneContinue_62794ff8:

    # li "I'll stay on the phone while you do it so we can make sure she actually takes the bait."
    li "Я останусь на телефоне, пока ты это делаешь, чтобы мы могли убедиться, что она действительно заглотила наживку."

# game/v1/scene49.rpy:63
translate russian v1s49Lindsey_PhoneContinue_b8019a20:

    # u "Okay, I'm texting her now."
    u "Хорошо, я пишу ей сейчас."

# game/v1/scene49.rpy:85
translate russian v1s49Chloe_PhoneContinue_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v1/scene49.rpy:91
translate russian v1s49Chloe_PhoneContinue_ceb12c3b:

    # u "Okay, seems to me like she took the bait."
    u "Ладно, мне кажется, что она заглотила наживку."

# game/v1/scene49.rpy:96
translate russian v1s49Chloe_PhoneContinue_3b0207b3:

    # li "Okay, hang on."
    li "Ладно, погоди."

# game/v1/scene49.rpy:98
translate russian v1s49Chloe_PhoneContinue_9e717813:

    # li "..."
    li "..."

# game/v1/scene49.rpy:100
translate russian v1s49Chloe_PhoneContinue_19dedf50:

    # li "Oh! I can hear her heading to the door now. I'm just checking out the window."
    li "О! Теперь я слышу, как она направляется к двери. Я просто смотрю в окно."

# game/v1/scene49.rpy:102
translate russian v1s49Chloe_PhoneContinue_9a33edf3:

    # li "She's getting into an Uber! Nice!"
    li "Она садится в Uber! Мило!"

# game/v1/scene49.rpy:107
translate russian v1s49Chloe_PhoneContinue_c54fe661:

    # u "Success!"
    u "Это успех!"

# game/v1/scene49.rpy:112
translate russian v1s49Chloe_PhoneContinue_66c3b853:

    # li "Hurry here. Don't be long."
    li "Скорее сюда. Не задерживайтесь."

# game/v1/scene49.rpy:117
translate russian v1s49Chloe_PhoneContinue_884507e0:

    # u "Okay, boss lady. *Chuckles*"
    u "Хорошо, леди-босс. *Хихикает*"

# game/v1/scene49.rpy:129
translate russian v1s49Chloe_PhoneContinue_e1887f70:

    # u "(Time to commit the perfect crime.)"
    u "(Время совершить идеальное преступление.)"

translate russian strings:

    # game/v1/scene49.rpy:21
    old "Chloe just left the house with some other blonde girl. On their way to the concert!"
    new "Хлоя только что вышла из дома с какой-то другой блондинкой. Они едут на концерт!"

    # game/v1/scene49.rpy:22
    old "It worked! How did you send the tickets without making her suspicious? Also, blonde girl?"
    new "Сработало! Как ты отправил билеты, не вызвав у нее подозрений? И еще, блондинка?"

    # game/v1/scene49.rpy:23
    old "I just sent the e-tickets anonymously. She has plenty of admirers these days. Probably just assumed it was a gift from one of them. Easy, huh?"
    new "Я просто отправил электронные билеты анонимно. У неё много поклонников в эти дни. Возможно, она решила, что это подарок от одного из них. Легко, да?"

    # game/v1/scene49.rpy:24
    old "Oh, she said they used to know each other when they were kids. That's all I know."
    new "О, она сказала, что они были знакомы, когда были детьми. Это всё, что я знаю."

    # game/v1/scene49.rpy:25
    old "Very smooth, Linds. Money well spent."
    new "Очень гладко, Линдс. Деньги потрачены не зря."

    # game/v1/scene49.rpy:26
    old "You've got to spend money to make money :P"
    new "Чтобы заработать деньги, нужно их потратить :P"

    # game/v1/scene49.rpy:27
    old "Or in this case, steal it."
    new "Или, в данном случае, украсть его."

    # game/v1/scene49.rpy:28
    old "Haha, exactly!"
    new "Хаха, точно!"

    # game/v1/scene49.rpy:29
    old "I'm on my way."
    new "Я уже в пути."

    # game/v1/scene49.rpy:69
    old "Hey... Think I'm in the mood for a little Italian cuisine and a beautiful woman across from me..."
    new "Привет... Думаю, я в настроении для небольшой итальянской кухни и красивой женщины напротив меня..."

    # game/v1/scene49.rpy:70
    old "Haha, hi there Mr. Sweet Talk..."
    new "Хаха, привет мистер Сладкий разговор..."

    # game/v1/scene49.rpy:71
    old "I could definitely eat, and... I suppose I'm down to see you ;)"
    new "Я определенно могу поесть, и... Я полагаю, я спустился к вам ;)"

    # game/v1/scene49.rpy:72
    old "Well then, I suppose I'll see you soon :)"
    new "Ну тогда, полагаю, скоро увидимся :)"

    # game/v1/scene49.rpy:75
    old "Hey... Think we can meet at that new Italian place, Classico Cuisine? I'm hungry and need some company."
    new "Эй... Как думаешь, мы можем встретиться в том новом итальянском заведении, Classico Cuisine? Я голоден и мне нужна компания."

    # game/v1/scene49.rpy:76
    old "What am I, just a meal buddy?"
    new "Я что, просто приятель по еде?"

    # game/v1/scene49.rpy:77
    old "Sorry, these are hungry person texts, haha."
    new "Извини, это текст голодного человека, хаха."

    # game/v1/scene49.rpy:78
    old "Haha, okay. Let's go now?"
    new "Хаха, хорошо. Пойдем сейчас?"

    # game/v1/scene49.rpy:79
    old "Yeah, sounds great"
    new "Да, звучит здорово."

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene49.rpy:116
translate russian v1s49Chloe_PhoneContinue_56078fcc:

    # u "Okay, boss lady."
    u "Хорошо, леди-босс."

# TODO: Translation updated at 2023-06-23 11:22

# game/v1/scene49.rpy:33
translate russian v1s49_f9e9813d:

    # u "(I should reply to Lindsey.)"
    u "(Я должен ответить Линдси.)"

# game/v1/scene49.rpy:39
translate russian v1s49_33671ad1:

    # u "Hey, Lindsey."
    u "Привет, Линдси."

# game/v1/scene49.rpy:44
translate russian v1s49_433518ef:

    # li "Hey, now is the perfect time. Text Chloe and tell her to meet you at Classico Cuisine."
    li "Эй, сейчас самое подходящее время. Напиши Хлое и попроси её встретиться с тобой в Classico Cuisine."

# game/v1/scene49.rpy:50
translate russian v1s49_93b710e4:

    # u "Oh, right. Yeah."
    u "О, точно. Ага."

# game/v1/scene49.rpy:55
translate russian v1s49_62794ff8:

    # li "I'll stay on the phone while you do it so we can make sure she actually takes the bait."
    li "Я останусь на телефоне, пока ты это делаешь, чтобы мы могли убедиться, что она действительно заглотила наживку."

# game/v1/scene49.rpy:60
translate russian v1s49_b8019a20:

    # u "Okay, I'm texting her now."
    u "Хорошо, я пишу ей сейчас."

# game/v1/scene49.rpy:80
translate russian v1s49_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v1/scene49.rpy:85
translate russian v1s49_ceb12c3b:

    # u "Okay, seems to me like she took the bait."
    u "Ладно, мне кажется, что она заглотила наживку."

# game/v1/scene49.rpy:90
translate russian v1s49_3b0207b3:

    # li "Okay, hang on."
    li "Ладно, погоди."

# game/v1/scene49.rpy:92
translate russian v1s49_9e717813:

    # li "..."
    li "..."

# game/v1/scene49.rpy:94
translate russian v1s49_19dedf50:

    # li "Oh! I can hear her heading to the door now. I'm just checking out the window."
    li "О! Теперь я слышу, как она направляется к двери. Я просто смотрю в окно."

# game/v1/scene49.rpy:96
translate russian v1s49_9a33edf3:

    # li "She's getting into an Uber! Nice!"
    li "Она садится в Uber! Мило!"

# game/v1/scene49.rpy:101
translate russian v1s49_c54fe661:

    # u "Success!"
    u "Это успех!"

# game/v1/scene49.rpy:106
translate russian v1s49_66c3b853:

    # li "Hurry here. Don't be long."
    li "Скорее сюда. Не задерживайся."

# game/v1/scene49.rpy:111
translate russian v1s49_56078fcc:

    # u "Okay, boss lady."
    u "Хорошо, леди-босс."

# game/v1/scene49.rpy:123
translate russian v1s49_e1887f70:

    # u "(Time to commit the perfect crime.)"
    u "(Время совершить идеальное преступление.)"

