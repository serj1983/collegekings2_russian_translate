# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene31a.rpy:22
translate russian v1s31a_35bf2b5c:

    # u "Putting in a set instead of enjoying the party?"
    u "Выкладываться вместо того, чтобы наслаждаться вечеринкой?"

# game/v1/scene31a.rpy:27
translate russian v1s31a_9733a4f9:

    # ca "Can't be walking around looking like a little bitch boy. Too much partying does that to you, you know."
    ca "Не могу же я разгуливать, выглядя как маленький сучонок. Слишком много вечеринок, знаешь ли."

# game/v1/scene31a.rpy:32
translate russian v1s31a_daeffb37:

    # u "Couldn't have said it better myself."
    u "Сам бы не смог сказать лучше."

# game/v1/scene31a.rpy:37
translate russian v1s31a_94d30631:

    # ca "You need to work out too, you look like a fucking fifth grader."
    ca "Тебе тоже надо тренироваться, ты выглядишь как долбаный пятиклассник."

# game/v1/scene31a.rpy:42
translate russian v1s31a_0a3fe781:

    # ca "Ah! Couldn't have said that better myself."
    ca "Ах! Я и сам не мог бы сказать это лучше."

# game/v1/scene31a.rpy:47
translate russian v1s31a_a7361e7c:

    # u "Ha! Alright..."
    u "Ха! Хорошо..."

# game/v1/scene31a.rpy:105
translate russian v1s31a_ad0ff772:

    # ca "Okay, good form. I'll give you that. How are your sit ups?"
    ca "Ладно, хорошая форма. Я дам тебе это. Как твои приседания?"

# game/v1/scene31a.rpy:146
translate russian v1s31a_7ed98f38:

    # ca "Acceptable, but you shouldn't be lying all the way down, resting every new rep."
    ca "Приемлемо, но ты не должен лежать до конца, отдыхая после каждого нового повторения."

# game/v1/scene31a.rpy:151
translate russian v1s31a_b9475731:

    # u "Got it, coach."
    u "Понял, тренер."

# game/v1/scene31a.rpy:156
translate russian v1s31a_e3e844d5:

    # ca "Alright, stop the bullshit and say what you need to say."
    ca "Ладно, прекрати нести чушь и расскажи, зачем ты пришёл."

# game/v1/scene31a.rpy:161
translate russian v1s31a_8b258704:

    # u "Huh?"
    u "А?"

# game/v1/scene31a.rpy:166
translate russian v1s31a_95df5f96:

    # ca "I'm pretty slow, [name], but I'm not stupid. You've never come to work out with me before."
    ca "Я довольно медлительный, [name], но не тупой. Ты никогда раньше не приходил ко мне на тренировку."

# game/v1/scene31a.rpy:172
translate russian v1s31a_d886e24d:

    # ca "On top of that, you're a fucking wolf. So what is it?"
    ca "Вдобавок ко всему, ты - долбаный волк. Так зачем ты здесь?"

# game/v1/scene31a.rpy:178
translate russian v1s31a_89e0b6c7:

    # u "Damn, okay... I mean, shit..."
    u "Черт, ладно... Я имею в виду, дерьмо..."

# game/v1/scene31a.rpy:183
translate russian v1s31a_485da8d1:

    # u "I did have something I wanted to talk to you about, but I was enjoying our bonding experience."
    u "У меня есть кое-что, о чём я хотел с тобой поговорить, но я наслаждался нашей связью."

# game/v1/scene31a.rpy:188
translate russian v1s31a_1eed7aae:

    # ca "I take working out too seriously for anyone to enjoy working out with me. Let alone call it a \"bonding experience.\""
    ca "Я слишком серьёзно отношусь к тренировкам, чтобы кто-то получил удовольствие от тренировок со мной. Не говоря уже о том, чтобы назвать это связью."

# game/v1/scene31a.rpy:193
translate russian v1s31a_ec7e3892:

    # u "Ta-fucking-da! Here I am."
    u "Та-блядь-да! Вот и я."

# game/v1/scene31a.rpy:195
translate russian v1s31a_7ece1efb:

    # u "Maybe if you didn't assume that no one enjoyed you, you'd find out that they actually do."
    u "Может, если бы ты не предполагал, что ты никому не нравишься, ты бы узнал, что это так."

# game/v1/scene31a.rpy:200
translate russian v1s31a_a20b6a22:

    # ca "Yeah, yeah."
    ca "Да, да."

# game/v1/scene31a.rpy:205
translate russian v1s31a_6a42f0e0:

    # ca "What do you want?"
    ca "Чего ты хочешь?"

# game/v1/scene31a.rpy:213
translate russian v1s31a_0ad5d49d:

    # u "I wanna talk to you about what everyone is talking about."
    u "Я хочу поговорить с тобой о том, о чём все говорят."

# game/v1/scene31a.rpy:218
translate russian v1s31a_087e77dc:

    # ca "What is everyone talking about?"
    ca "О чём же все говорят?"

# game/v1/scene31a.rpy:223
translate russian v1s31a_e9476fa6:

    # u "The Chicks race?"
    u "В гонке Чикс?"

# game/v1/scene31a.rpy:228
translate russian v1s31a_f8a48efa:

    # ca "*Scoffs* I couldn't care less about that, man."
    ca "*Насмехается* Да меня это не волнует."

# game/v1/scene31a.rpy:233
translate russian v1s31a_c5446f18:

    # u "Really?"
    u "Правда?"

# game/v1/scene31a.rpy:238
translate russian v1s31a_57f3e9f4:

    # ca "Yes, really."
    ca "Да, правда."

# game/v1/scene31a.rpy:243
translate russian v1s31a_4af7d38c:

    # u "Well, sorry to tell you this but..."
    u "Прости, что говорю тебе это, но..."

# game/v1/scene31a.rpy:248
translate russian v1s31a_99c543c9:

    # ca "But what?"
    ca "Но что?"

# game/v1/scene31a.rpy:253
translate russian v1s31a_d5a89b84:

    # cl "Hey, you two."
    cl "Эй, вы двое."

# game/v1/scene31a.rpy:258
translate russian v1s31a_5031a2c4:

    # ca "Ahh shit... Look, I don't have time for this. This is not what I came out here for."
    ca "Ах дерьмо... Слушай, у меня нет на это времени. Это не то, ради чего я сюда пришёл."

# game/v1/scene31a.rpy:263
translate russian v1s31a_2000d25a:

    # cl "Yeah, you're out here because they're being annoying in there. You're welcome. I wanted to talk."
    cl "Да, ты здесь, потому что они тебя там раздражают. Пожалуйста. Я хочу поговорить."

# game/v1/scene31a.rpy:268
translate russian v1s31a_e6c899ee:

    # u "Cameron, look, just hear us out. Can you do that?"
    u "Кэмерон, послушай, просто выслушай нас. Ты можешь это сделать?"

# game/v1/scene31a.rpy:273
translate russian v1s31a_00fb4568:

    # ca "..."
    ca "..."

# game/v1/scene31a.rpy:278
translate russian v1s31a_d38aa789:

    # cl "Please?"
    cl "Пожалуйста?"

# game/v1/scene31a.rpy:283
translate russian v1s31a_15a75955:

    # ca "What is it?"
    ca "Что это?"

# game/v1/scene31a.rpy:288
translate russian v1s31a_f0f36f97:

    # ca "And keep it simple for fucks sake. Explain it to me like I'm five."
    ca "И будь проще, чёрт возьми. Объясни мне, как будто мне пять лет."

# game/v1/scene31a.rpy:293
translate russian v1s31a_1b28a406:

    # u "Of course."
    u "Разумеется."

# game/v1/scene31a.rpy:298
translate russian v1s31a_02253c97:

    # u "We want the Apes to form an alliance with Chloe, making The Chicks the sister frat of the Apes."
    u "Мы хотим, чтобы Приматы заключили союз с Хлоей, сделав Чикс сестринским братством Приматов."

# game/v1/scene31a.rpy:303
translate russian v1s31a_0d63fdce:

    # ca "Why would I care about that?"
    ca "Почему меня это должно волновать?"

# game/v1/scene31a.rpy:311
translate russian v1s31a_aab6ee38:

    # u "Duh, so Lindsey doesn't win."
    u "Чтобы Линдси не выиграла."

# game/v1/scene31a.rpy:316
translate russian v1s31a_c93b4624:

    # ca "I ain't got shit against her, haha. Why would I care who wins?"
    ca "Я ничего против неё не имею, хаха. Почему меня должно волновать, кто победит?"

# game/v1/scene31a.rpy:321
translate russian v1s31a_987a21f8:

    # cl "Because you know me, Cam."
    cl "Потому что ты знаешь меня, Кэм."

# game/v1/scene31a.rpy:326
translate russian v1s31a_53de2582:

    # ca "And I know her too. You're basically one in the same."
    ca "И я тоже её знаю. Вы практически одно целое."

# game/v1/scene31a.rpy:331
translate russian v1s31a_3c7a03e2:

    # cl "*Scoffs*"
    cl "*Смешок*"

# game/v1/scene31a.rpy:336
translate russian v1s31a_2a870042:

    # u "Chloe..."
    u "Хлоя..."

# game/v1/scene31a.rpy:341
translate russian v1s31a_fd958ebc:

    # ca "Try again."
    ca "Попробуй ещё раз."

# game/v1/scene31a.rpy:349
translate russian v1s31a_20765f15:

    # u "To strengthen the Apes."
    u "Чтобы усилить обезьян."

# game/v1/scene31a.rpy:354
translate russian v1s31a_4108a83b:

    # ca "How would an alliance with a bunch of chicks strengthen us?"
    ca "Как союз с кучкой девчонок усилит нас?"

# game/v1/scene31a.rpy:359
translate russian v1s31a_f24f56bc:

    # u "You may be slow, but you aren't stupid, right?"
    u "Может, ты и медлителен, но не туп, верно?"

# game/v1/scene31a.rpy:364
translate russian v1s31a_9dd0fbb8:

    # ca "Right."
    ca "Верно."

# game/v1/scene31a.rpy:369
translate russian v1s31a_4edef02b:

    # u "So think about it, the most popular sorority matched with the Apes. It makes the Apes look damn good and leaves the Wolves looking completely alone."
    u "Подумай только, самое популярное женское общество объединилось с Приматами. Это позволит Приматам выглядеть чертовски хорошо, а Волки будут совершенно одинокими."

# game/v1/scene31a.rpy:374
translate russian v1s31a_5a1ae958:

    # u "They can't throw a good party after an alliance like that has been made."
    u "Они не смогут устроить хорошую вечеринку после заключения такого союза."

# game/v1/scene31a.rpy:379
translate russian v1s31a_b452a00b:

    # ca "Hmm..."
    ca "Хмм..."

# game/v1/scene31a.rpy:384
translate russian v1s31a_7acd0bd4:

    # cl "Don't think too hard, this is a favor for you and the Apes just as much as it's a favor for me."
    cl "Не думай слишком много, это услуга для тебя и Приматов в той же степени, что и для меня."

# game/v1/scene31a.rpy:389
translate russian v1s31a_419e8a86:

    # cl "That's why it's called an alliance."
    cl "Вот почему он будет называться альянсом."

# game/v1/scene31a.rpy:394
translate russian v1s31a_947c5f17:

    # ca "Good point..."
    ca "Хорошо сказано..."

# game/v1/scene31a.rpy:400
translate russian v1s31a_163a20fb:

    # ca "You already got a shit ton of beef with Grayson. How would an alliance work if the President doesn't even fuck with you like that?"
    ca "У тебя и так уже куча проблем с Грейсоном. Как будет работать альянс, если президент даже не будет с тобой возиться?"

# game/v1/scene31a.rpy:405
translate russian v1s31a_3ed474c5:

    # u "That's why we came to you, you're someone he does listen to."
    u "Вот почему мы пришли к тебе, ты тот, кого он слушает."

# game/v1/scene31a.rpy:410
translate russian v1s31a_42381b6a:

    # u "Grayson doesn't have many people he'll take advice from, but you're one of those people."
    u "У Грейсона не так много людей, к советам которых он прислушивается, но ты - один из таких людей."

# game/v1/scene31a.rpy:415
translate russian v1s31a_286869e4:

    # ca "Fair enough, so what exactly are you wanting me to do?"
    ca "Справедливо, так что вы хотите, чтобы я сделал?"

# game/v1/scene31a.rpy:423
translate russian v1s31a_d4c21d99:

    # u "Tell everyone Chloe is the hotter chick and that the hottest chick should be the Chicks President."
    u "Скажите всем, что Хлоя - самая сексуальная цыпочка, и что самая сексуальная цыпочка должна быть президентом Чикс."

# game/v1/scene31a.rpy:428
translate russian v1s31a_5076a945:

    # ca "Yeah... Again, that means nothing to me."
    ca "Да... Опять же, для меня это ничего не значит."

# game/v1/scene31a.rpy:436
translate russian v1s31a_5646e356:

    # u "I don't know exactly, we haven't decided just yet."
    u "Я точно не знаю, мы еще не решили."

# game/v1/scene31a.rpy:438
translate russian v1s31a_cbee938a:

    # u "Maybe just some sort of collaborative post on Kiwii would get the ball rolling and make people start talking."
    u "Может быть, просто какой-то совместный пост на Kiwii запустит процесс и заставит людей начать говорить."

# game/v1/scene31a.rpy:443
translate russian v1s31a_9fc6ca17:

    # ca "And how do you suggest that I convince Grayson?"
    ca "И как ты предлагаешь убедить мне Грейсона?"

# game/v1/scene31a.rpy:448
translate russian v1s31a_fbec016a:

    # cl "No different than you would for anything else, just tell him what's best for the Apes... Be honest."
    cl "Не отличайся от других, просто скажи ему, что лучше для Приматов... Будь честен."

# game/v1/scene31a.rpy:454
translate russian v1s31a_92303f9e:

    # ca "I hear what you guys are saying and I get what you're trying to do, but I'm not sold on anything just yet."
    ca "Я слышу, что вы говорите, и понимаю, что вы пытаетесь сделать, но я ещё ничего не решил."

# game/v1/scene31a.rpy:459
translate russian v1s31a_22006c17:

    # cl "Cameron."
    cl "Кэмерон."

# game/v1/scene31a.rpy:464
translate russian v1s31a_dc0edd7d:

    # u "(Here come the voodoo eyes...)"
    u "(Вот и глаза вуду...)"

# game/v1/scene31a.rpy:469
translate russian v1s31a_dd3f0246:

    # cl "We never talk much but we do know each other very well."
    cl "Мы никогда много не разговаривали, но очень хорошо знаем друг друга."

# game/v1/scene31a.rpy:471
translate russian v1s31a_d176ce30:

    # cl "I know you well enough to know exactly what your aspirations are and so all I'll say is this..."
    cl "Я знаю тебя достаточно хорошо, чтобы точно знать, каковы твои устремления, поэтому я скажу лишь следующее..."

# game/v1/scene31a.rpy:476
translate russian v1s31a_6557cae4:

    # cl "If you back me in my campaign, I'll back you in any race you plan to take on. And I mean any."
    cl "Если ты поддержишь меня в моей кампании, я поддержу тебя в любой гонке, в которой ты запланируешь принять участие. Я имею в виду любую."

# game/v1/scene31a.rpy:481
translate russian v1s31a_40f34279:

    # ca "How did you know I wanted to run for President?"
    ca "Как ты узнала, что я хочу баллотироваться в президенты?"

# game/v1/scene31a.rpy:486
translate russian v1s31a_a8fa8aa0:

    # cl "Your actions say it all. You're a natural leader."
    cl "Твои действия говорят сами за себя. Ты - прирожденный лидер."

# game/v1/scene31a.rpy:491
translate russian v1s31a_36b488f6:

    # ca "*Sighs*"
    ca "*Вздох*"

# game/v1/scene31a.rpy:496
translate russian v1s31a_7fd469f1:

    # u "She's not wrong."
    u "Она не ошибается."

# game/v1/scene31a.rpy:507
translate russian v1s31a_d4a0112e:

    # ca "Fine. I'll talk to Grayson."
    ca "Отлично. Я поговорю с Грейсоном."

# game/v1/scene31a.rpy:512
translate russian v1s31a_ed0dc5d6:

    # cl "Thank you! *Squeals* Thank you so much, Cameron."
    cl "Спасибо! *Визг* Спасибо тебе большое, Кэмерон."

# game/v1/scene31a.rpy:517
translate russian v1s31a_912bad0e:

    # ca "Yep."
    ca "Ага."

# game/v1/scene31a.rpy:522
translate russian v1s31a_43ce88c9:

    # cl "I'm off to talk to a few more people. Thanks for the nice chat! I appreciate it."
    cl "Пойду поговорю ещё с несколькими людьми. Спасибо за приятную беседу! Я ценю это."

# game/v1/scene31a.rpy:527
translate russian v1s31a_5d4c75d2:

    # u "See you-"
    u "Увидимся..."

# game/v1/scene31a.rpy:537
translate russian v1s31a_c76cb4a8:

    # u "Ha... So?"
    u "Ха... Что?"

# game/v1/scene31a.rpy:542
translate russian v1s31a_ee3e607d:

    # u "President, huh?"
    u "Президент, да?"

# game/v1/scene31a.rpy:548
translate russian v1s31a_fb6d3041:

    # ca "That's Ape business only."
    ca "Это дело только Приматов."

# game/v1/scene31a.rpy:553
translate russian v1s31a_423c6f19:

    # u "Haha, okay."
    u "Хаха, хорошо."

# game/v1/scene31a.rpy:558
translate russian v1s31a_f88ca816:

    # ca "You think everything she said was true?"
    ca "Ты думаешь, всё, что она сказала, было правдой?"

# game/v1/scene31a.rpy:563
translate russian v1s31a_75f284b8:

    # u "Hm?"
    u "Хм?"

# game/v1/scene31a.rpy:568
translate russian v1s31a_721d69d8:

    # ca "Or do you think she just knows exactly what to say, to get you on board?"
    ca "Или ты думаешь, что она просто знает, что именно нужно сказать, чтобы ты согласился?"

# game/v1/scene31a.rpy:573
translate russian v1s31a_7c4f47f8:

    # u "(He's talking about Chloe...)"
    u "(Он говорит о Хлое...)"

# game/v1/scene31a.rpy:578
translate russian v1s31a_aaf0917f:

    # u "She means every word that she says. She wouldn't say it to you if she didn't believe it, or mean it."
    u "Она подразумевает каждое слово, которое говорит. Она не сказала бы тебе этого, если бы не верила в это или не имела в виду."

# game/v1/scene31a.rpy:583
translate russian v1s31a_c5b37bf2:

    # ca "Ha... Alright."
    ca "Ха... Хорошо."

# game/v1/scene31a.rpy:592
translate russian v1s31a_835b0a6c:

    # u "She knows what she needs to say. She's damn good at it too."
    u "Она знает, что ей нужно сказать. И у неё это чертовски хорошо получается."

# game/v1/scene31a.rpy:597
translate russian v1s31a_d7e5310f:

    # ca "Ha."
    ca "Ха."

# game/v1/scene31a.rpy:602
translate russian v1s31a_61a68111:

    # ca "You catch on quick."
    ca "Ты быстро схватываешь."

# game/v1/scene31a.rpy:607
translate russian v1s31a_9d3ee6dc:

    # u "Do I? *Chuckles*"
    u "А я? *Хихикает*"

# game/v1/scene31a.rpy:613
translate russian v1s31a_00fb4568_1:

    # ca "..."
    ca "..."

# game/v1/scene31a.rpy:618
translate russian v1s31a_feb97fe2:

    # ca "I'll see you later, man."
    ca "Увидимся позже, чувак."

# game/v1/scene31a.rpy:623
translate russian v1s31a_03ec2f26:

    # u "Yeah, see ya."
    u "Да, увидимся."

# game/v1/scene31a.rpy:629
translate russian v1s31a_1bb308b4:

    # ca "Yeah, so what?"
    ca "Да, и что?"

# game/v1/scene31a.rpy:634
translate russian v1s31a_92f9d0f7:

    # u "Nothing, and it's not surprising. Your whole attitude screams Apes."
    u "Ничего, и это неудивительно. Твоё поведение кричит о Примате."

# game/v1/scene31a.rpy:639
translate russian v1s31a_8f71c823:

    # ca "I'm glad you think so. I guess I should take running more seriously then?"
    ca "Я рад, что ты так думаешь. Тогда, наверное, мне стоит относиться к бегу более серьезно?"

# game/v1/scene31a.rpy:644
translate russian v1s31a_2342833f:

    # u "You should, yeah."
    u "Да, стоит."

# game/v1/scene31a.rpy:649
translate russian v1s31a_4dd73a40:

    # ca "I'm gonna go make a call."
    ca "Пойду позвоню."

# game/v1/scene31a.rpy:654
translate russian v1s31a_296836e5:

    # u "Haha, you do that."
    u "Хаха, так и есть."

# game/v1/scene31a.rpy:660
translate russian v1s31a_d697f0ad:

    # ca "And just so you know, helping Chloe wasn't even a question."
    ca "И чтобы ты знал, помочь Хлое даже не вопрос."

# game/v1/scene31a.rpy:662
translate russian v1s31a_cca86431:

    # ca "You've been there for Sam so obviously you have good judgement. If you think Chloe is what's best for us as Apes, then I'm on board."
    ca "Ты был рядом с Сэм, так что, очевидно, у тебя есть здравый смысл. Если ты думаешь, что Хлоя лучше для нас, то я с тобой."

# game/v1/scene31a.rpy:667
translate russian v1s31a_62dd49cc:

    # u "Thanks Cam."
    u "Спасибо Кэм."

# game/v1/scene31a.rpy:672
translate russian v1s31a_332f15d5:

    # ca "You bet."
    ca "Ещё бы."

# game/v1/scene31a.rpy:678
translate russian v1s31a_d263e9db:

    # u "(President Cam... *Laughs* I can't even begin to imagine how Grayson would react to that.)"
    u "(Президент Кэм... *Смеётся* Я даже не могу представить, как Грейсон отреагирует на это)."

translate russian strings:

    # game/v1/scene31a.rpy:180
    old "Butter him up"
    new "Подмажь его"

    # game/v1/scene31a.rpy:180
    old "Cut to the chase"
    new "Перейти к делу"

    # game/v1/scene31a.rpy:308
    old "So Lindsey doesn't win"
    new "Так Линдси не выиграет"

    # game/v1/scene31a.rpy:308
    old "To strengthen the Apes"
    new "Усилить Приматов"

    # game/v1/scene31a.rpy:420
    old "Say Chloe is the hotter chick"
    new "Хлоя - более сексуальная цыпочка"

    # game/v1/scene31a.rpy:420
    old "Collaboration"
    new "Сотрудничество"

    # game/v1/scene31a.rpy:575
    old "She means what she says"
    new "Она имеет в виду то, что говорит."

    # game/v1/scene31a.rpy:575
    old "She says what she needs to"
    new "Она говорит то, что ей нужно"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene31a.rpy:37
translate russian v1s31a_4b0c8e1f:

    # ca "You need to work out, too. You look like a fucking fifth grader."
    ca "Тебе тоже надо тренироваться, ты выглядишь как долбаный пятиклассник."

# game/v1/scene31a.rpy:105
translate russian v1s31a_40d23252:

    # ca "Okay, good form. I'll give you that. How are your sit-ups?"
    ca "Ладно, хорошая форма. Я дам тебе это. Как твои приседания?"

# game/v1/scene31a.rpy:326
translate russian v1s31a_990bd9b8:

    # ca "And I know her, too. You're basically one in the same."
    ca "И я тоже её знаю. Вы практически одно целое."

# game/v1/scene31a.rpy:400
translate russian v1s31a_cab0f53f:

    # ca "You already got a shit ton of beef with Grayson. How would an alliance work if the president doesn't even fuck with you like that?"
    ca "У тебя и так уже куча проблем с Грейсоном. Как будет работать альянс, если президент даже не будет с тобой возиться?"

# game/v1/scene31a.rpy:423
translate russian v1s31a_ce183539:

    # u "Tell everyone Chloe is the hotter chick and that the hottest chick should be the Chicks' president."
    u "Скажите всем, что Хлоя - самая сексуальная цыпочка, и что самая сексуальная цыпочка должна быть президентом Чикс."

# game/v1/scene31a.rpy:481
translate russian v1s31a_78e20e6f:

    # ca "How did you know I wanted to run for president?"
    ca "Как ты узнала, что я хочу баллотироваться в президенты?"

# game/v1/scene31a.rpy:592
translate russian v1s31a_d3548909:

    # u "She knows what she needs to say. She's damn good at it, too."
    u "Она знает, что ей нужно сказать. И у неё это чертовски хорошо получается."

# game/v1/scene31a.rpy:607
translate russian v1s31a_69f098ef:

    # u "Do I? Funny, that."
    u "Правда? Забавно."

# game/v1/scene31a.rpy:654
translate russian v1s31a_63314d47:

    # u "You do that."
    u "Ты сделаешь это."

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v1.0.1s
# TODO: Translation updated at 2022-06-28 14:25

# game/v1/scene31a.rpy:423
translate russian v1s31a_cf595508:

    # u "Tell everyone Chloe is the hotter chick and that the hottest chick should be the Chicks' President."
    u "Скажи всем, что Хлоя - самая горячая цыпочка, и что самая горячая цыпочка должна быть президентом Чикс."

# TODO: Translation updated at 2023-04-11 15:01

# game/v1/scene31a.rpy:47
translate russian v1s31a_bfa0d062:

    # u "Ha! All right..."
    u "Ха! Хорошо..."

# game/v1/scene31a.rpy:154
translate russian v1s31a_71c9ab32:

    # ca "All right, stop the bullshit and say what you need to say."
    ca "Ладно, прекрати нести чушь и скажи, что хотел сказать."

# game/v1/scene31a.rpy:578
translate russian v1s31a_0afe4962:

    # ca "Ha... All right."
    ca "Ха... Хорошо."

