# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene19a.rpy:12
translate russian v1s19a_a58bdbfc:

    # cl "I really think this is the most solid plan."
    cl "Я правда думаю, что это самый надежный план."

# game/v1/scene19a.rpy:17
translate russian v1s19a_300ea018:

    # u "I do too."
    u "Я тоже."

# game/v1/scene19a.rpy:23
translate russian v1s19a_8f0103b4:

    # cl "Especially since I basically already have the Wolves' support, I feel confident about this."
    cl "Тем более, что у меня уже есть поддержка Волков, я чувствую себя уверенней."

# game/v1/scene19a.rpy:28
translate russian v1s19a_14a528aa:

    # u "Oh, yeah? How'd that come about?"
    u "О, да? И как это получилось?"

# game/v1/scene19a.rpy:33
translate russian v1s19a_a8e4b929:

    # cl "How did what come about?"
    cl "Как это получилось?"

# game/v1/scene19a.rpy:38
translate russian v1s19a_68ed2d22:

    # u "The Wolves supporting you already? I ran into Imre this morning while he was chanting in your favor."
    u "Волки уже поддерживают тебя? Я столкнулся с Имре сегодня утром, когда он скандировал в твою пользу."

# game/v1/scene19a.rpy:43
translate russian v1s19a_3f31ff6c:

    # cl "Oh.. That's simple."
    cl "О... Всё просто."

# game/v1/scene19a.rpy:48
translate russian v1s19a_680f8f9b:

    # cl "Chris and I share the same principles when it comes to leadership and loyalty."
    cl "Мы с Крисом разделяем одни и те же принципы, когда речь идет о лидерстве и преданности."

# game/v1/scene19a.rpy:49
translate russian v1s19a_1d442d8c:

    # cl "You can't blame all of the issues on the leader and give up on them, throwing them away, thinking that'll change things."
    cl "Нельзя сваливать все проблемы на лидера, отказываясь и отбрасывая их, думая, что это изменит ситуацию."

# game/v1/scene19a.rpy:54
translate russian v1s19a_b38d381b:

    # cl "You have to work on improving the leadership that's already comfortably in place."
    cl "Ты должен работать над улучшением руководства, которое уже комфортно работает."

# game/v1/scene19a.rpy:59
translate russian v1s19a_57a4b11d:

    # u "So, you and Chris aren't that close? Just share some perspectives?"
    u "Значит, вы с Крисом не так уж близки? Просто обмениваетесь мнениями?"

# game/v1/scene19a.rpy:64
translate russian v1s19a_a790c4a5:

    # cl "Well, no... I mean, we're pretty close."
    cl "Ну, нет... Я имею в виду, мы довольно близки."

# game/v1/scene19a.rpy:69
translate russian v1s19a_c290fe94:

    # u "Even with Nor-"
    u "Даже с Нор..."

# game/v1/scene19a.rpy:74
translate russian v1s19a_86e36e97:

    # cl "I already know what you're gonna say, haha."
    cl "Я уже знаю, что ты скажешь, хаха."

# game/v1/scene19a.rpy:79
translate russian v1s19a_c4c61d9a:

    # cl "Whatever's happening between Chris and Nora is their business..."
    cl "Что бы ни происходило между Крисом и Норой, это их дело..."

# game/v1/scene19a.rpy:80
translate russian v1s19a_5a93c43c:

    # cl "And I think because I don't get involved as much as others, we have this unspoken respective relationship, you know?"
    cl "И я думаю, что поскольку я не так сильно вовлечена в процесс, как другие, у нас есть эти негласные соответствующие отношения, понимаешь?"

# game/v1/scene19a.rpy:85
translate russian v1s19a_91785a99:

    # u "Yeah, makes sense, the Wolves are very much about loyalty."
    u "Да, в этом есть смысл, Волки очень любят верность."

# game/v1/scene19a.rpy:90
translate russian v1s19a_2e6183d5:

    # cl "Exactly, and though Chris may support me as a leader, it doesn't mean all of the Wolves will want to..."
    cl "Именно, и хотя Крис может поддержать меня как лидера, это не значит, что все Волки захотят..."

# game/v1/scene19a.rpy:91
translate russian v1s19a_6268b72c:

    # cl "That's where a little favor from my favorite guy comes in."
    cl "Вот тут-то и приходит на помощь небольшая услуга от моего любимого парня."

# game/v1/scene19a.rpy:96
translate russian v1s19a_9f8cf748:

    # u "*Sighs* What now?"
    u "*Вздох* Что же?"

# game/v1/scene19a.rpy:101
translate russian v1s19a_c5eba73c:

    # cl "Am I asking you for too much?"
    cl "Я слишком много прошу?"

# game/v1/scene19a.rpy:106
translate russian v1s19a_a93d361f:

    # u "Haha, no... I'm just teasing."
    u "Хаха, нет... Я просто дразню."

# game/v1/scene19a.rpy:111
translate russian v1s19a_b46d2918:

    # cl "Okay good, ha. I was nervous for a second..."
    cl "Хорошо, хорошо. Я на секунду занервничала..."

# game/v1/scene19a.rpy:116
translate russian v1s19a_6860ab9b:

    # u "*Laughs* No, go ahead. What's the favor?"
    u "*Смеётся* Нет, давай. Что за услуга?"

# game/v1/scene19a.rpy:121
translate russian v1s19a_a5a16b59:

    # cl "I was wondering if you could talk to Chris about the Wolves actively supporting me."
    cl "Я хотела спросить, не мог бы ты поговорить с Крисом о том, чтобы Волки более активней поддерживали меня."

# game/v1/scene19a.rpy:126
translate russian v1s19a_6ea5c55c:

    # u "Is he not already? I told you, Imre said he was rooting for you. He was screaming it to the world in fact, haha."
    u "Разве он не уже? Я же говорил тебе, что Имре сказал, что болеет за тебя. Он кричал об этом на весь мир, в самом деле, хаха."

# game/v1/scene19a.rpy:131
translate russian v1s19a_b0b2c8b7:

    # cl "*Chuckles* Rooting for and actively supporting are two very different things."
    cl "Хихикает* Болеть за и активно поддерживать - две совершенно разные вещи."

# game/v1/scene19a.rpy:136
translate russian v1s19a_27ae8d4a:

    # cl "I need the Wolves' help with things on a grand scale if I want to beat Lindsey."
    cl "Мне нужна помощь Волков в делах большого масштаба, если я хочу победить Линдси."

# game/v1/scene19a.rpy:141
translate russian v1s19a_6a63d9ce:

    # cl "And honestly when it comes to Imre, he's had a crush on me for as long as I can remember. *Laughs*"
    cl "И если честно, когда речь заходит об Имре, он влюблен в меня столько, сколько я себя помню. *Смеётся*"

# game/v1/scene19a.rpy:142
translate russian v1s19a_0716f529:

    # cl "When I called him this morning he picked up right away and was way too excited to help..."
    cl "Когда я позвонила ему сегодня утром, он сразу же взял трубку и был очень рад помочь..."

# game/v1/scene19a.rpy:147
translate russian v1s19a_dabf04ab:

    # u "*Laughs* That's amazing."
    u "*Смеётся* Это удивительно."

# game/v1/scene19a.rpy:152
translate russian v1s19a_d90c42f3:

    # cl "So, do you think you can talk to Chris about this? Today?"
    cl "Как ты думаешь, ты сможешь поговорить об этом с Крисом? Сегодня?"

# game/v1/scene19a.rpy:160
translate russian v1s19a_d98828a0:

    # u "Of course I can."
    u "Конечно, могу."

# game/v1/scene19a.rpy:165
translate russian v1s19a_127c938a:

    # cl "Perfect! Thank you so so so much, [name]."
    cl "Отлично! Большое спасибо, [name]."

# game/v1/scene19a.rpy:171
translate russian v1s19a_6fab9b02:

    # u "(Damn, I promised to go meet Lindsey later. Do I have time for both? This \"double agent\" life might get tricky...)"
    u "(Чёрт, я обещал встретиться с Линдси позже. Есть ли у меня время для обоих? Жизнь \"двойного агента\" может стать сложной...)"

# game/v1/scene19a.rpy:178
translate russian v1s19a_d98828a0_1:

    # u "Of course I can."
    u "Конечно, могу."

# game/v1/scene19a.rpy:183
translate russian v1s19a_72c4ac08:

    # cl "Perfect! Thank you so much, [name]. This means the world to me."
    cl "Отлично! Большое спасибо, [name]. Это очень важно для меня."

# game/v1/scene19a.rpy:191
translate russian v1s19a_867b967f:

    # u "I hate to state the obvious but, Chris and I aren't on the best of terms right now... And he's got Nora on his mind, so..."
    u "Не хочу говорить очевидное, но мы с Крисом сейчас не в лучших отношениях... И у него на уме Нора, так что..."

# game/v1/scene19a.rpy:196
translate russian v1s19a_2581285b:

    # u "It's probably not best if I'm the one who talks to him."
    u "Наверное, будет не очень хорошо, если именно я буду с ним разговаривать."

# game/v1/scene19a.rpy:201
translate russian v1s19a_c445d908:

    # cl "*Sighs* Yeah, I get it. His head isn't in the right space around you. Smart thinking."
    cl "*Вздох* Да, я понимаю. Его голова находится не в том пространстве, где ты находишься. Умное мышление."

# game/v1/scene19a.rpy:206
translate russian v1s19a_91d39ecc:

    # cl "So yeah, I'll set everything up and will let you know when it's ready."
    cl "Так что да, я всё подготовлю и дам тебе знать, когда всё будет готово."

# game/v1/scene19a.rpy:211
translate russian v1s19a_dad009cb:

    # u "Awesome."
    u "Замечательно"

# game/v1/scene19a.rpy:226
translate russian v1s19a_ab246298:

    # cl "*Screams* AH!"
    cl "*Кричит* АХ!"

# game/v1/scene19a.rpy:231
translate russian v1s19a_5023a294:

    # u "Relax..."
    u "Расслабься..."

# game/v1/scene19a.rpy:236
translate russian v1s19a_68b3c00a:

    # cl "That scared me!"
    cl "Это испугало меня!"

# game/v1/scene19a.rpy:242
translate russian v1s19a_601d293b:

    # unknown "SHHH!"
    unknown "ШШШ!"

# game/v1/scene19a.rpy:247
translate russian v1s19a_184c3057:

    # cl "Cliche library kids."
    cl "Клише для детей из библиотеки."

# game/v1/scene19a.rpy:252
translate russian v1s19a_6b236514:

    # u "Haha. Just like in the movies."
    u "Хаха. Прямо как в кино."

# game/v1/scene19a.rpy:257
translate russian v1s19a_6c5d094c:

    # cl "Right. Well, I need to be on my way. Thanks again for helping me with the plan."
    cl "Да. Ну, мне пора в путь. Еще раз спасибо, что помог мне с планом."

# game/v1/scene19a.rpy:262
translate russian v1s19a_66686979:

    # u "Always."
    u "Всегда."

translate russian strings:

    # game/v1/scene19a.rpy:173
    old "Talk to Chris"
    new "Поговорить с Крисом"

    # game/v1/scene19a.rpy:173
    old "Make an excuse"
    new "Найти оправдание"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v1/scene19a.rpy:49
translate russian v1s19a_c247dcec:

    # cl "You can't blame all of the issues on the leader, give up on them, throw them away, and think that'll change things."
    cl "Нельзя сваливать все проблемы на лидера, отказываясь и отбрасывая их, думая, что это что-то изменит."

# game/v1/scene19a.rpy:74
translate russian v1s19a_b6ae06a4:

    # cl "I already know what you're gonna say."
    cl "Я уже знаю, что ты собираешься сказать."

# game/v1/scene19a.rpy:80
translate russian v1s19a_e93046dd:

    # cl "And I think because I don't get involved as much as others, we have this unspoken, respectful relationship, you know?"
    cl "И я думаю, что поскольку я не так сильно вовлечена в процесс, как другие, у нас есть эти негласные соответствующие отношения, понимаешь?"

# game/v1/scene19a.rpy:85
translate russian v1s19a_e26437cb:

    # u "Yeah, makes sense. The Wolves are very much about loyalty."
    u "Да, в этом есть смысл. Волки очень любят верность."

# game/v1/scene19a.rpy:106
translate russian v1s19a_7eb85fb2:

    # u "Ha, no... I'm just teasing."
    u "Ха, нет... Я просто дразню."

# game/v1/scene19a.rpy:111
translate russian v1s19a_f529f3a6:

    # cl "Okay good. I was nervous for a second..."
    cl "Хорошо. А то я на секунду занервничала..."

# game/v1/scene19a.rpy:116
translate russian v1s19a_30dc0154:

    # u "No, go ahead. What's the favor?"
    u "Нет, давай. Что за услуга?"

# game/v1/scene19a.rpy:126
translate russian v1s19a_1b703228:

    # u "Is he not already? I told you, Imre said he was rooting for you. He was screaming it to the world, in fact."
    u "Разве он не уже? Я же говорил тебе, что Имре сказал, что болеет за тебя. Он кричал об этом на весь мир, на самом деле."

# game/v1/scene19a.rpy:131
translate russian v1s19a_a35de9f4:

    # cl "Rooting for and actively supporting are two very different things."
    cl "Болеть за и активно поддерживать - две совершенно разные вещи."

# game/v1/scene19a.rpy:136
translate russian v1s19a_7d364c65:

    # cl "I need the Wolves' help on a grand scale if I want to beat Lindsey."
    cl "Мне нужна огромная помощь Волков, если я хочу победить Линдси."

# game/v1/scene19a.rpy:141
translate russian v1s19a_cfb2cd80:

    # cl "And honestly, when it comes to Imre, he's had a crush on me for as long as I can remember."
    cl "И, честно говоря, если говорить об Имре, он был влюблен в меня столько, сколько я себя помню."

# game/v1/scene19a.rpy:147
translate russian v1s19a_dd59b8ee:

    # u "That's amazing."
    u "Это удивительно."

# game/v1/scene19a.rpy:206
translate russian v1s19a_79cc894a:

    # cl "So, yeah, I'll set everything up and will let you know when it's ready."
    cl "Так что, да, я всё подготовлю и дам тебе знать, когда всё будет готово."

# game/v1/scene19a.rpy:252
translate russian v1s19a_8dedfa2c:

    # u "Ha, just like in the movies."
    u "Ха, прямо как в кино."

