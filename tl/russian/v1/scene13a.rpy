# TODO: Translation updated at 2022-05-11 23:19

# game/v1/scene13a.rpy:12
translate russian v1s13a_646b1da4:

    # u "(Look at that beautiful bed. Oh, the things I'm gonna do to you.)"
    u "(Посмотри на эту прекрасную кровать. О, что я с тобой сделаю.)"

# game/v1/scene13a.rpy:17
translate russian v1s13a_115fbc3c:

    # u "(I won't be letting anyone bother me tonight.)"
    u "(Я не позволю никому беспокоить меня сегодня вечером.)"

# game/v1/scene13a.rpy:27
translate russian v1s13a_fc435eb5:

    # gr "Charli's boyfriend is back!"
    gr "Чарли вернулся!"

# game/v1/scene13a.rpy:32
translate russian v1s13a_81db243d:

    # u "What the fuck did you just say?"
    u "Что, блядь, ты только что сказал?"

# game/v1/scene13a.rpy:37
translate russian v1s13a_8a988b24:

    # gr "Ha! Watch your tone, fruit cake. I was just joshing... With how defensive you got though, it does make me wonder a bit. *Chuckles*"
    gr "Ха! Следи за своим тоном, фруктовый пирог. Я просто пошутил... Но то, как ты защищаешься, заставляет меня немного задуматься. *Хихикает*"

# game/v1/scene13a.rpy:42
translate russian v1s13a_170772ca:

    # u "Where is this annoying joke coming from, Grayson?"
    u "Откуда взялась эта раздражающая шутка, Грейсон?"

# game/v1/scene13a.rpy:47
translate russian v1s13a_93e12106:

    # gr "It's pretty well-known that Charli liked fucking with you. The fact that he was on vacation and still couldn't take his mind off of you..."
    gr "Довольно хорошо известно, что Чарли нравилось трахаться с тобой. Тот факт, что он был в отпуске и всё ещё не мог отвлечься от тебя..."

# game/v1/scene13a.rpy:52
translate russian v1s13a_9ebd1e93:

    # gr "Maybe it's that beautiful sparkle in your eye... *Laughs*"
    gr "Может быть, это тот прекрасный блеск в твоих глазах... *Смеется*"

# game/v1/scene13a.rpy:57
translate russian v1s13a_184f24c5:

    # u "I can't stand that fucker. You have no idea, man. He made my life a living-"
    u "Я терпеть не могу этого ублюдка. Ты не представляешь, чувак. Он превратил мою жизнь в..."

# game/v1/scene13a.rpy:62
translate russian v1s13a_44a2e2e2:

    # gr "Keep that anger built up, brother. We're gonna need you to have that energy in a few weeks."
    gr "Держи эту злость в себе, брат. Нам понадобится твоя энергия через несколько недель."

# game/v1/scene13a.rpy:67
translate russian v1s13a_d88c225f:

    # u "For?"
    u "Длч чего?"

# game/v1/scene13a.rpy:72
translate russian v1s13a_425fc94d:

    # gr "*Sniffs* There's a smell of change in the air. And you need to be ready."
    gr "*Вдыхает* В воздухе витает запах перемен. И ты должен быть готов."

# game/v1/scene13a.rpy:77
translate russian v1s13a_95c6badb:

    # u "What exactly are you talking about?"
    u "О чём ты говоришь?"

# game/v1/scene13a.rpy:82
translate russian v1s13a_a4003519:

    # gr "All you need to know is that you should stay clean, be a good boy, and get into shape."
    gr "Всё, что тебе нужно знать, это то, что ты должен оставаться чистым, быть хорошим мальчиком и привести себя в форму."

# game/v1/scene13a.rpy:87
translate russian v1s13a_a0151c85:

    # u "Are we fighting soon?"
    u "Мы скоро будем драться?"

# game/v1/scene13a.rpy:92
translate russian v1s13a_d3ea1440:

    # gr "Just be ready. Oh yeah, and welcome back."
    gr "Просто будь готов. И да, с возвращением."

# game/v1/scene13a.rpy:97
translate russian v1s13a_e3094d17:

    # u "(He will never make any sense to me.)"
    u "(Он никогда не будет иметь для меня никакого смысла)."

# game/v1/scene13a.rpy:112
translate russian v1s13a_1eb0edab:

    # u "(Jet lag recovery begins in five... four... three... two...-)"
    u "(Восстановление после полёта начинается через пять... четыре... три... два...)"

