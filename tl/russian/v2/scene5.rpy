# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v2/scene5.rpy:18
translate russian v2s5_140c5099:

    # u "(Oh boy...)"
    u "(О боже...)"

# game/v2/scene5.rpy:45
translate russian v2s5_57721150:

    # ro "Hi, [name], how are you?"
    ro "Привет, [name], как дела?"

# game/v2/scene5.rpy:50
translate russian v2s5_64d47c10:

    # u "Um, I'm good. How are you? *Chuckles*"
    u "Эм, я в порядке. Как твои? *Хихикает*"

# game/v2/scene5.rpy:55
translate russian v2s5_fcc67416:

    # ro "I'm pleased to see that you're in a good mood. It might be easier to get through this conversation..."
    ro "Мне приятно видеть, что ты в хорошем настроении. Возможно, было бы легче пережить этот разговор..."

# game/v2/scene5.rpy:60
translate russian v2s5_99d1d4ca:

    # u "What conversation?"
    u "Какой разговор?"

# game/v2/scene5.rpy:65
translate russian v2s5_13ccdeb4:

    # ro "I heard that there's going to be a drug test for all students participating in the tournament coming up."
    ro "Я слышала, что скоро будет проведён тест на наркотики для всех студентов, участвующих в турнире."

# game/v2/scene5.rpy:70
translate russian v2s5_7e124df1:

    # u "Oh, yeah. I already heard something about that..."
    u "О, да. Я уже кое-что слышал об этом..."

# game/v2/scene5.rpy:72
translate russian v2s5_c05103c0:

    # u "Do you know exactly when they're testing us?"
    u "Ты точно знаешь, когда будут проверять?"

# game/v2/scene5.rpy:77
translate russian v2s5_27aef829:

    # ro "I don't. I just know that it's sometime soon."
    ro "Я не. Я просто знаю, что это будет очень скоро."

# game/v2/scene5.rpy:83
translate russian v2s5_58d4a7b4:

    # u "Are they testing for weed?"
    u "Они проверяют на травку?"

# game/v2/scene5.rpy:88
translate russian v2s5_510165d0:

    # ro "They will be testing for everything."
    ro "Они будут проверять на всё."

# game/v2/scene5.rpy:90
translate russian v2s5_78acadcf:

    # ro "So, I just wanted to apologize in advance in case they find you positive for the drug I put in your drink while we were in Paris."
    ro "Итак, я бы хотела заранее извиниться за случай, если они обнаружат, у тебя положительный результат на наркотик, который я подсыпала тебе в напиток, пока мы были в Париже."

# game/v2/scene5.rpy:96
translate russian v2s5_515174f4:

    # u "I should be fine... I'm not worried."
    u "Со мной всё будет в порядке... Я не волнуюсь."

# game/v2/scene5.rpy:101
translate russian v2s5_42b937b1:

    # ro "Well, I hope there's nothing to worry about."
    ro "Что ж, я буду надеятся, что это так."

# game/v2/scene5.rpy:103
translate russian v2s5_b78c2c56:

    # ro "I just wanted to apologize in advance in case they find you positive for the drug I put in your drink while we were in Paris."
    ro "Итак, я бы хотела заранее извиниться за случай, если они обнаружат, у тебя положительный результат на наркотик, который я подсыпала тебе в напиток, пока мы были в Париже."

# game/v2/scene5.rpy:105
translate russian v2s5_82020239:

    # ro "I'm not sure how long it can stay in your system but... They might detect traces of it since these tests are very sophisticated."
    ro "Я не уверена, как долго он может оставаться в твоём организме, но... Они могут обнаружить его следы, поскольку тесты будут сложными."

# game/v2/scene5.rpy:107
translate russian v2s5_617103df:

    # ro "A little bit of weed is one thing, [name]. But that was a serious drug I slipped you."
    ro "Немного травки - это одно, [name]. Но это был серьезный наркотик, который я тебе подсыпала."

# game/v2/scene5.rpy:119
translate russian v2s5_57ae9944:

    # u "*Sighs*"
    u "*Вздох*"

# game/v2/scene5.rpy:121
translate russian v2s5_4c024f30:

    # u "I'm sure it'll be okay."
    u "Я уверен, что всё будет хорошо."

# game/v2/scene5.rpy:123
translate russian v2s5_0cbafb8b:

    # u "I'll just drink loads of water or something to make sure it's all flushed out of my system. *Chuckles*"
    u "Я постараюсь выпить много воды или чего-нибудь ещё, чтобы убедиться, что всё это выведено из моего организма. *Хихикает*"

# game/v2/scene5.rpy:128
translate russian v2s5_e4cf9cad:

    # ro "*Chuckles* That's my boy..."
    ro "*Хихикает* Мой мальчик..."

# game/v2/scene5.rpy:130
translate russian v2s5_01890312:

    # ro "Thank you for not being angry with me."
    ro "Спасибо, что не сердишься на меня."

# game/v2/scene5.rpy:135
translate russian v2s5_38128f49:

    # u "Yeah, of course. It was worth it, I think..."
    u "Да, конечно. Думаю, оно того стоило..."

# game/v2/scene5.rpy:140
translate russian v2s5_b82223e2:

    # ro "I thought so too."
    ro "Я тоже так думаю."

# game/v2/scene5.rpy:145
translate russian v2s5_bf3389d3:

    # u "I- uh, I better go before your class starts showing up."
    u "Я... эм, мне лучше идти, пока твой класс не появился."

# game/v2/scene5.rpy:150
translate russian v2s5_a595b2fc:

    # ro "If they ever do... *Laughs*"
    ro "Если они когда-нибудь появятся *Смеётся*"

# game/v2/scene5.rpy:155
translate russian v2s5_fcc18885:

    # u "*Laughs*"
    u "*Смеётся*"

# game/v2/scene5.rpy:160
translate russian v2s5_c1a4954e:

    # ro "I'll see you around, [name]."
    ro "До встречи, [name]."

# game/v2/scene5.rpy:186
translate russian v2s5_6ddd6482:

    # u "Fuck, Lorraine..."
    u "Блядь, Лоррейн, ты..."

# game/v2/scene5.rpy:191
translate russian v2s5_6611f1f3:

    # ro "I'm sorry, [name]. I never thought that you'd be getting drug tested."
    ro "Прости, [name]. Я никогда не думала, что вас будут проверять на наркотики."

# game/v2/scene5.rpy:196
translate russian v2s5_a5699bdd:

    # u "Yeah, neither did I. Now look."
    u "Да, я тоже. А теперь видишь к чему пришли."

# game/v2/scene5.rpy:201
translate russian v2s5_024d19bb:

    # ro "I'm sorry."
    ro "Мне жаль."

# game/v2/scene5.rpy:206
translate russian v2s5_a515bebd:

    # u "What can they do to me? Throw me out of the tournament? Can they throw me out of college?!"
    u "Что они сделают со мной? Вышвырнут меня из турнира? А может вышвырнут из колледжа?!"

# game/v2/scene5.rpy:211
translate russian v2s5_c1b16b7f:

    # ro "I don't know about college, but they will ban you from participating in the tournament."
    ro "Я не знаю насчёт колледжа, но они запретят тебе участвовать в турнире."

# game/v2/scene5.rpy:216
translate russian v2s5_5d82e79d:

    # u "Fuck! That's so messed up!"
    u "Блядь! Это так запутанно!"

# game/v2/scene5.rpy:218
translate russian v2s5_2a578c52:

    # u "You literally drugged me, without my permission. And now-"
    u "Ты накачала меня наркотиками без моего разрешения. И теперь-"

# game/v2/scene5.rpy:223
translate russian v2s5_581a3176:

    # ro "I know you're angry, [name], but... I don't regret it."
    ro "Я знаю, ты злишься, [name], но... Я не жалею об этом."

# game/v2/scene5.rpy:225
translate russian v2s5_da48351f:

    # ro "The sex we had was one of the best experiences of my entire life."
    ro "Секс, который у нас был, был одним из лучших за всю мою жизнь."

# game/v2/scene5.rpy:234
translate russian v2s5_212f546a:

    # u "Yeah, it was amazing. You're not wrong."
    u "Да, это было потрясающе. Ты не ошиблась."

# game/v2/scene5.rpy:242
translate russian v2s5_d61929fe:

    # u "Well, not for me. It might have been fun but..."
    u "Ну, не для меня. Это могло бы быть забавно, но..."

# game/v2/scene5.rpy:244
translate russian v2s5_8cda9f49:

    # u "It's not worth losing everything I've worked for."
    u "Это не стоит того, чтобы потерять всё, ради чего я работал."

# game/v2/scene5.rpy:249
translate russian v2s5_256c6294:

    # ro "[name], please. Let me make it up to you."
    ro "[name], пожалуйста. Позволь загладить свою вину перед тобой."

# game/v2/scene5.rpy:254
translate russian v2s5_3b64fb73:

    # u "How can you make it up to me? Are you going to take the drug test for me? *Scoffs* I don't think that'll work."
    u "Как ты можешь загладить свою вину передо мной? Ты собираешься пройти тест на наркотики для меня? *Усмехается* Не думаю, что это сработает."

# game/v2/scene5.rpy:259
translate russian v2s5_47089f8b:

    # ro "I'll think of something to take your mind off it..."
    ro "Я придумаю что-нибудь, чтобы отвлечь тебя от этого..."

# game/v2/scene5.rpy:264
translate russian v2s5_c3699bad:

    # u "I need to go. This is... so fucked up."
    u "Мне нужно идти. Это... так хреново."

translate russian strings:

    # game/v2/scene5.rpy:112
    old "Be calm"
    new "Будь спокоен"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v2/scene5.rpy:50
translate russian v2s5_adb040f9:

    # u "Um, I'm good. How are you?"
    u "Эм, я в порядке. Как твои?"

# game/v2/scene5.rpy:105
translate russian v2s5_ee191deb:

    # ro "I'm not sure how long it can stay in your system but... they might detect traces of it, since these tests are sophisticated."
    ro "Я не уверена, как долго он может оставаться в твоём организме, но... Они могут обнаружить его следы, поскольку тесты будут сложными."

# game/v2/scene5.rpy:123
translate russian v2s5_5e64b4a1:

    # u "I'll just drink loads of water or something to make sure it's all flushed out of my system."
    u "Я постараюсь выпить много воды или чего-нибудь ещё, чтобы убедиться, что всё это выведено из моего организма."

# game/v2/scene5.rpy:128
translate russian v2s5_f2ae4a12:

    # ro "That's my boy..."
    ro "Мой мальчик..."

# game/v2/scene5.rpy:140
translate russian v2s5_225d05a2:

    # ro "I thought so, too."
    ro "Я тоже так думаю."

# game/v2/scene5.rpy:145
translate russian v2s5_e6930013:

    # u "I... uh, I better go before your class starts showing up."
    u "Я... эм, мне лучше идти, пока твой класс не появился."

# game/v2/scene5.rpy:150
translate russian v2s5_f6346c6c:

    # ro "If they ever do..."
    ro "Если они когда-нибудь появятся..."

# game/v2/scene5.rpy:155
translate russian v2s5_bcf04164:

    # u "Ha. Later."
    u "Ха. Увидимся."

# game/v2/scene5.rpy:242
translate russian v2s5_17fd3732:

    # u "Well, not for me. Even if my body liked it, being drugged wasn't a cool experience, and..."
    u "Ну, не для меня. Даже если моему телу это нравилось, быть накачанным наркотиками не было крутым опытом, и..."

# game/v2/scene5.rpy:244
translate russian v2s5_b35f8c31:

    # u "The sex is not worth losing everything I've worked for."
    u "Секс не стоил того, чтобы потерять всё, ради чего я работал."

# game/v2/scene5.rpy:254
translate russian v2s5_0993957e:

    # u "How can you make it up to me? Are you going to take the drug test for me? I don't think that'll work."
    u "Как ты сможешь загладить свою вину передо мной? Ты собираешься пройти тест на наркотики за меня? Не думаю, что это сработает."

