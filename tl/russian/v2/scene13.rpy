# TODO: Translation updated at 2022-08-26 15:31

# game/v2/scene13.rpy:20
translate russian v2s13_da695a4b:

    # u "(Lauren's party isn't for a while, so maybe I should get some studying in. It's not fun, but neither is failing.)"
    u "(Вечеринка у Лорен ещё не скоро, так что, может быть, мне стоит немного позаниматься. Не весело, но необходимо.)"

# game/v2/scene13.rpy:52
translate russian v2s13_03cd0b3b:

    # u "Okay, break time."
    u "Ладно, время перерыва."

# game/v2/scene13.rpy:72
translate russian v2s13_3dccfa76:

    # imre "Hey, what the hell are you snacking on sugar for? You need to maintain the shape of a fighting machine, bro."
    imre "Эй, какого чёрта ты перекусываешь сахаром? Тебе нужно поддерживать форму боевой машины, бро."

# game/v2/scene13.rpy:81
translate russian v2s13_e535c7a3:

    # u "Yeah, I know... I'll do better next time."
    u "Да, знаю... В следующий раз возьму что-нибудь другое."

# game/v2/scene13.rpy:86
translate russian v2s13_a992dad9:

    # imre "I just saved you from obesity."
    imre "Я только что спас тебя от ожирения."

# game/v2/scene13.rpy:91
translate russian v2s13_f3549920:

    # u "Ha, you did. Thanks."
    u "Ха, да. Спасибо."

# game/v2/scene13.rpy:96
translate russian v2s13_c3c8a657:

    # imre "That's what I'm here for, man!"
    imre "Вот для чего я здесь, мужик!"

# game/v2/scene13.rpy:104
translate russian v2s13_56d5d75e:

    # u "Why are you creeping outside my room, Imre? I'm trying to study here."
    u "Почему ты врываешься в мою комнату, Имре? Я пытаюсь учиться."

# game/v2/scene13.rpy:109
translate russian v2s13_d4c7eaa0:

    # imre "Whatever, man. Just showing that I care! Enjoy your high cholesterol."
    imre "Как скажешь, чувак. Просто говорю тебе, что мне не всё равно! Наслаждайтесь своим высоким уровнем холестерина."

# game/v2/scene13.rpy:114
translate russian v2s13_d6224755:

    # u "I will."
    u "Обязательно"

# game/v2/scene13.rpy:139
translate russian v2s13_PhoneContinue_d172984c:

    # u "(I should reply to Riley.)"
    u "(Я должен ответить Райли.)"

# game/v2/scene13.rpy:145
translate russian v2s13_PhoneContinue_6030afad:

    # u "(That's enough studying for right now anyway. Time to get Lauren's gift and find a costume!)"
    u "(В любом случае, достаточно обучения. Время купить подарок Лорен и найти костюм!)"

# game/v2/scene13.rpy:152
translate russian v2s13_PhoneContinue_28de7ae5:

    # u "(I'll just do an extra gym session this week to even things out.)"
    u "(Нужно провести дополнительную тренировку в тренажёрке на этой неделе, чтобы выровнять ситуацию.)"

# game/v2/scene13.rpy:162
translate russian v2s13_PhoneContinue_177d62d7:

    # u "(Next time I'll get a healthier snack...)"
    u "(В следующий раз я возьму более здоровую закуску...)"

# game/v2/scene13.rpy:192
translate russian v2s13_PhoneContinue_45257a24:

    # gr "The fuck?"
    gr "Какого хрена?"

# game/v2/scene13.rpy:197
translate russian v2s13_PhoneContinue_ef2f1671:

    # u "What?"
    u "Что?"

# game/v2/scene13.rpy:202
translate russian v2s13_PhoneContinue_9c094e90:

    # gr "If you keep eating like that, you're gonna end up looking like the bubblegum girl from that chocolate factory movie..."
    gr "Если ты продолжишь это есть, то в конце концов станешь похож на девочку с жевательной резинкой, из фильма Шоколадная фабрика..."

# game/v2/scene13.rpy:211
translate russian v2s13_PhoneContinue_899323e5:

    # u "Donuts turn you into blueberries?"
    u "Пончики превратили тебя в чернику?"

# game/v2/scene13.rpy:216
translate russian v2s13_PhoneContinue_5a912399:

    # gr "Nah, dude. They turn you into fat balloons, and I don't wanna be rolling your ass to the gym."
    gr "Нет, чувак. Они превратят тебя в жирные шары, а я не хочу катить твою задницу в спортзал."

# game/v2/scene13.rpy:221
translate russian v2s13_PhoneContinue_4e5285d0:

    # u "Ha! Nice one, dude. I actually laughed at that."
    u "Ха! Отлично, чувак. Я правда посмеялся над этим."

# game/v2/scene13.rpy:226
translate russian v2s13_PhoneContinue_c1f2f988:

    # gr "Ha, fuck you."
    gr "Ха, пошел ты."

# game/v2/scene13.rpy:233
translate russian v2s13_PhoneContinue_9662275f:

    # u "Chocolate factory movie? Are you feeling okay?"
    u "Из фильма Шоколадная фабрика? Ты хорошо себя чувствуешь?"

# game/v2/scene13.rpy:238
translate russian v2s13_PhoneContinue_73589d52:

    # gr "Yeah, you prick, I feel great. Not my fault you're a boomer."
    gr "Да, придурок, я чувствую себя великолепно. Не моя вина, что ты бумер."

# game/v2/scene13.rpy:243
translate russian v2s13_PhoneContinue_c5a72ced:

    # u "Ha, whatever you say, man."
    u "Ха, как скажешь, чувак."

# game/v2/scene13.rpy:248
translate russian v2s13_PhoneContinue_423033b0:

    # gr "Regardless, that's not the diet of a fight champion. Do better."
    gr "Как бы то ни было, это не диета чемпиона по боям. Занимайся лучше."

# game/v2/scene13.rpy:257
translate russian v2s13_PhoneContinue_3bdd0da1:

    # u "I know, I will. This is only my first, just so you know."
    u "Знаю, занимаюсь. Это мой первый раз, просто чтобы ты знал."

# game/v2/scene13.rpy:262
translate russian v2s13_PhoneContinue_ad23f517:

    # gr "Well, keep it that way. Unless you want frat boy to turn into fat boy!"
    gr "Что ж, пусть так и будет. Если только ты не хочешь, чтобы парень из братства превратился в толстяка!"

# game/v2/scene13.rpy:267
translate russian v2s13_PhoneContinue_776ca709:

    # u "Ha, wow. You're on fire today with these jokes, huh?"
    u "Ха, вау. Ты сегодня в ударе с такими шутками, да?"

# game/v2/scene13.rpy:272
translate russian v2s13_PhoneContinue_04a98a0d:

    # gr "Whatever, I didn't come here to amuse you. Tubby bitch."
    gr "Как бы то ни было, я пришел сюда не для того, чтобы развлекать тебя. Толстая сучка."

# game/v2/scene13.rpy:277
translate russian v2s13_PhoneContinue_48047903:

    # u "Alright, okay. No donuts."
    u "Хорошо, хорошо. Никаких пончиков."

# game/v2/scene13.rpy:282
translate russian v2s13_PhoneContinue_68aafa48:

    # gr "Good."
    gr "Хорошо."

# game/v2/scene13.rpy:292
translate russian v2s13_PhoneContinue_023bfa3c:

    # u "Are you serious right now? You're telling me that you never have donuts, or cake, or cola?"
    u "Ты серьёзно? Ты хочешь сказать, что у тебя никогда не бывает пончиков, тортов или колы?"

# game/v2/scene13.rpy:297
translate russian v2s13_PhoneContinue_a1d8f708:

    # gr "You think I got to be the fight king by eating that shit?"
    gr "Ты думаешь, я стану королём боя, поедая это дерьмо?"

# game/v2/scene13.rpy:302
translate russian v2s13_PhoneContinue_7f69e796:

    # u "I know for a fact that you eat this shit, Grayson. Get off my back. It's a donut."
    u "Я точно знаю, что ты ешь это дерьмо, Грейсон. Отвали от меня. Это только пончик."

# game/v2/scene13.rpy:307
translate russian v2s13_PhoneContinue_99551264:

    # gr "Suit yourself, fat boy."
    gr "Поступай как знаешь, толстяк."

# game/v2/scene13.rpy:312
translate russian v2s13_PhoneContinue_10105b5c:

    # u "(Such a dick.)"
    u "(Вот придурок.)"

# game/v2/scene13.rpy:337
translate russian v2s13_PhoneContinue2_d172984c:

    # u "(I should reply to Riley.)"
    u "(Я должен ответить Райли.)"

# game/v2/scene13.rpy:343
translate russian v2s13_PhoneContinue2_6030afad:

    # u "(That's enough studying for right now anyway. Time to get Lauren's gift and find a costume!)"
    u "(В любом случае, достаточно занятий. Время купить подарок Лорен и найти костюм!)"

# game/v2/scene13.rpy:350
translate russian v2s13_PhoneContinue2_28de7ae5:

    # u "(I'll just do an extra gym session this week to even things out.)"
    u "(Нужно провести дополнительную тренировку в тренажёрке на этой неделе, чтобы выровнять ситуацию.)"

# game/v2/scene13.rpy:360
translate russian v2s13_PhoneContinue2_177d62d7:

    # u "(Next time I'll get a healthier snack...)"
    u "(В следующий раз я возьму более здоровую закуску...)"

translate russian strings:

    # game/v2/scene13.rpy:77
    old "Agree with Imre"
    new "Согласиться с Имре"

    # game/v2/scene13.rpy:77
    old "Dismiss Imre"
    new "Отшить Имре"

    # game/v2/scene13.rpy:147
    old "Eat the donut"
    new "Съесть пончик"

    # game/v2/scene13.rpy:147
    old "Don't eat the donut"
    new "Не есть пончик"

    # game/v2/scene13.rpy:207
    old "A blueberry?"
    new "С черникой?"

    # game/v2/scene13.rpy:207
    old "Chocolate factory movie?"
    new "Фильм Шоколадная фабрика?"

    # game/v2/scene13.rpy:253
    old "I will"
    new "Так и сделаю"

    # game/v2/scene13.rpy:253
    old "You don't eat donuts?"
    new "А ты не ешь пончики?"

# TODO: Translation updated at 2023-04-11 15:01

# game/v2/scene13.rpy:276
translate russian v2s13_PhoneContinue_7cdd595c:

    # u "All right, okay. No donuts."
    u "Хорошо, хорошо. Никаких пончиков."

