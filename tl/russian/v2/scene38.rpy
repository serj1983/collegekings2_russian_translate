# TODO: Translation updated at 2024-03-07 10:56

# game/v2/scene38.rpy:52
translate russian v2s38_b08ad257:

    # li "Hey, [name], welcome to the VIP party limo!"
    li "Привет, [name], добро пожаловать в лимузин для VIP-вечеринок!"

# game/v2/scene38.rpy:57
translate russian v2s38_54cf9fc6:

    # u "Hey! I see you're all already in the party mood."
    u "Привет! Вижу, вы уже в праздничном настроении."

# game/v2/scene38.rpy:64
translate russian v2s38_06c1880d:

    # gr "Yeah, drink as much as you can! It's all included in the price."
    gr "Да, пей сколько влезет! Всё включено."

# game/v2/scene38.rpy:70
translate russian v2s38_9982b576:

    # gr "Yeah, it's more like a kid's party limo with all this juice and soda. *Scoffs*"
    gr "Да, больше похоже на детский лимузин с соком и содовой. *Усмехается*"

# game/v2/scene38.rpy:76
translate russian v2s38_1e27f698:

    # u "Ha, sup Grayson? Good to see you."
    u "Ха, что случилось, Грейсон? Рад тебя видеть. Всё в порядке?"

# game/v2/scene38.rpy:78
translate russian v2s38_b5881888:

    # u "Well well, nice of you to join us, Grayson."
    u "Ну-ну, как мило, что ты присоединился к нам, Грейсон."

# game/v2/scene38.rpy:84
translate russian v2s38_5f8f5cfc:

    # gr "Pfft, no it's not! But yeah, you too... I guess."
    gr "Пффф, нет, не в порядке! Но я тоже рад... Наверное."

# game/v2/scene38.rpy:86
translate russian v2s38_7fdc9d0b:

    # gr "Yeah, yeah... I can socialize when I want to."
    gr "Да, да... Я могу быть общительным, когда захочу."

# game/v2/scene38.rpy:92
translate russian v2s38_ee0315aa:

    # se "Hey, [name]!"
    se "Привет, [name]!"

# game/v2/scene38.rpy:98
translate russian v2s38_5ec6a5e0:

    # u "Hey, Seb. Glad you could make it."
    u "Привет, Себ. Рад, что ты смог прийти."

# game/v2/scene38.rpy:100
translate russian v2s38_d5c82af8:

    # u "Hey, Sebastian. Good to see you."
    u "Привет, Себастьян. Рад тебя видеть."

# game/v2/scene38.rpy:106
translate russian v2s38_677e8d5f:

    # se "Are you kidding, bro? When Lindsey said we're going clubbing, for free, in a limo... You being here is just a cherry on top!"
    se "Ты шутишь, бро? Когда Линдси сказала, что мы собираемся в клуб, на халяву, в лимузине... То, что ты здесь, - это просто вишенка на торте!"

# game/v2/scene38.rpy:108
translate russian v2s38_d5e85e08:

    # se "Yeah, you too. It's not often that we get to hang out without Chris and Grayson pushing us to throw down."
    se "Да, и я. Не часто нам удаётся потусоваться без того, чтобы Крис и Грейсон не подталкивали нас к драке."

# game/v2/scene38.rpy:114
translate russian v2s38_afe09ddd:

    # u "I'm glad you're looking forward to it."
    u "Я рад, что ты этого ждёшь."

# game/v2/scene38.rpy:116
translate russian v2s38_d2d027a9:

    # u "Yeah, very true!"
    u "Да, чертовски верно!"

# game/v2/scene38.rpy:122
translate russian v2s38_9406633b:

    # au "Hey, handsome."
    au "Привет, красавчик."

# game/v2/scene38.rpy:127
translate russian v2s38_73d6d652:

    # u "Hello, gorgeous. Fancy meeting you here..."
    u "Привет, красавица. Приятно встретить тебя здесь..."

# game/v2/scene38.rpy:138
translate russian v2s38_8686e81c:

    # u "Hey."
    u "Привет."

# game/v2/scene38.rpy:143
translate russian v2s38_73278001:

    # au "*Whispers* I'm pissed at you, but I'll try to be civil. Just for tonight, okay?"
    au "*Шёпотом* Я всё ещё злюсь на тебя, но я постараюсь быть вежливой. И только сегодня, хорошо?"

# game/v2/scene38.rpy:148
translate russian v2s38_cbb1da11:

    # u "*Whispers** Okay, thanks... I really don't like it when things are weird between us."
    u "*Шёпотом* Хорошо, спасибо... Мне очень не нравится, когда между нами происходит какая-то хрень."

# game/v2/scene38.rpy:153
translate russian v2s38_85eeb4bf:

    # au "Yeah..."
    au "Ага..."

# game/v2/scene38.rpy:158
translate russian v2s38_d4253444:

    # au "Me neither."
    au "Мне тоже."

# game/v2/scene38.rpy:164
translate russian v2s38_e4e2ec35:

    # u "Hey, you. Sick of me yet?"
    u "Привет. Тебя от меня ещё не тошнит?"

# game/v2/scene38.rpy:169
translate russian v2s38_84fb0223:

    # au "Sick of the guy who helped me survive an afternoon with my fucking family? Never."
    au "Тошнит от парня, который помог мне пережить день с моей грёбаной семьёй? Никогда."

# game/v2/scene38.rpy:174
translate russian v2s38_d746527f:

    # u "Well, you deserve nothing but handsome chaperones and limo rides, so get used to it."
    u "Ну, ты заслуживаешь только симпатичных сопровождающих и поездки на лимузине, так что привыкай."

# game/v2/scene38.rpy:180
translate russian v2s38_4b6648a7:

    # gr "How are things going with the Deer, Autumn?"
    gr "Как дела у Ланей, Отми?"

# game/v2/scene38.rpy:185
translate russian v2s38_8d6cd941:

    # aut "Oh! Yeah, it's going great, thanks."
    aut "Ох! Всё замечательно, спасибо!"

# game/v2/scene38.rpy:190
translate russian v2s38_bd71774f:

    # gr "I'm only asking because I never hear a damn thing about your sorority!"
    gr "Я из-за чего спрашиваю. Я никогда не слышал ни слова о твоём сестринстве!"

# game/v2/scene38.rpy:192
translate russian v2s38_4d12f038:

    # gr "Nobody likes a President that just cruises along doing nothing."
    gr "Никому не нравится президент, который просто разъезжает и ничего не делает."

# game/v2/scene38.rpy:197
translate russian v2s38_d2d781b0:

    # li "Autumn is the hardest working President I know."
    li "Отэм - самый трудолюбивый президент, которого я знаю."

# game/v2/scene38.rpy:202
translate russian v2s38_2343d209:

    # aut "Thanks, Linds..."
    aut "Спасибо, Линдс..."

# game/v2/scene38.rpy:207
translate russian v2s38_c2ed1ebb:

    # gr "Okay, whatever... What do you even do?"
    gr "Ладно, неважно... Ты чем вообще занимаешься?"

# game/v2/scene38.rpy:212
translate russian v2s38_2e026e98:

    # aut "What do you mean?"
    aut "Что значит чем?"

# game/v2/scene38.rpy:217
translate russian v2s38_cbceb0c6:

    # gr "What's your ethos, Autumn?"
    gr "Какой у вас идеал, Отэм?"

# game/v2/scene38.rpy:222
translate russian v2s38_f1d5b0c2:

    # aut "Everyone knows what the Deer are about, Grayson. Like the way people know what the Apes are about."
    aut "Все знают, что из себя представляют Лани, Грейсон. Так же, как люди знают, что из себя представляют Приматы."

# game/v2/scene38.rpy:224
translate russian v2s38_9928270b:

    # aut "Which is mostly that their President can be a real dick sometimes."
    aut "По большей части их президент иногда может быть настоящим мудаком."

# game/v2/scene38.rpy:229
translate russian v2s38_8ac90046:

    # au "Only sometimes?"
    au "Только иногда?"

# game/v2/scene38.rpy:234
translate russian v2s38_4bbeab0f:

    # gr "Hey, Aubrey. I'm talking to Autumn here."
    gr "Эй, Обри. Я тут с Отэм разговариваю."

# game/v2/scene38.rpy:239
translate russian v2s38_55dc243c:

    # au "No, you're being a dick to Autumn here."
    au "Ты ведёшь с ней себя как мудак."

# game/v2/scene38.rpy:244
translate russian v2s38_f2f75f7a:

    # li "Hey, okay- Let's just chill and enjoy the night guys, okay?"
    li "Эй, хватит. Давайте просто расслабимся и насладимся вечером, хорошо?"

# game/v2/scene38.rpy:249
translate russian v2s38_382b7520:

    # gr "Autumn started it."
    gr "Это Отэм начала."

# game/v2/scene38.rpy:258
translate russian v2s38_a946129d:

    # u "(Maybe it wasn't such a good idea to invite Grayson. I doubt Sebastian would be causing drama like this.)"
    u "(Может не стоило приглашать Грейсона? Сомневаюсь, что Себастьян стал бы устраивать такие сцены.)"

# game/v2/scene38.rpy:263
translate russian v2s38_52a7a3ee:

    # u "I wonder what the club is going to look like."
    u "Интересно, как будет выглядеть клуб."

# game/v2/scene38.rpy:268
translate russian v2s38_ee631ef8:

    # gr "There better be loads of hot girls. That's the only reason I'm going."
    gr "Там должно быть много горячих девушек. Это единственная причина, почему я иду."

# game/v2/scene38.rpy:273
translate russian v2s38_384db83e:

    # li "You're already with a bunch of hot girls!"
    li "Ты уже с кучей горячих девушек!"

# game/v2/scene38.rpy:283
translate russian v2s38_9295928f:

    # gr "I guess we've got different standards, right [name]?."
    gr "Думаю, у нас разные предпочтения, правда, [name]?"

# game/v2/scene38.rpy:288
translate russian v2s38_76b77938:

    # gr "So, who's getting laid tonight? Come on, show me your hands!"
    gr "Итак, кто сегодня собирается потрахаться? Давайте, поднимите руки!"

# game/v2/scene38.rpy:314
translate russian v2s38_508f7d76:

    # gr "That's my boy right there."
    gr "Мой мальчик."

# game/v2/scene38.rpy:316
translate russian v2s38_712e5caf:

    # gr "That's right, [name], get those hands in the air! I guess you're okay for a pussy Wolf."
    gr "Правильно, [name], подними руки! Полагаю, для киски Волка ты ничего."

# game/v2/scene38.rpy:321
translate russian v2s38_4b43a771:

    # li "Oh my God, guys, that's not what tonight is about."
    li "Боже, ребята, сегодня не об этом."

# game/v2/scene38.rpy:326
translate russian v2s38_ef89a7b7:

    # gr "It is for us."
    gr "А для нас об этом."

# game/v2/scene38.rpy:339
translate russian v2s38_9b612a61:

    # gr "Seriously? You guys are so fucking boring!"
    gr "Серьёзно? Вы, ребята, пиздец какие скучные!"

# game/v2/scene38.rpy:345
translate russian v2s38_d9068217:

    # se "I've been working so hard on things with the Wolves lately. This night out is seriously needed."
    se "В последнее время я задолбался разгребать дела в братстве. Этот ночной выезд действительно необходим."

# game/v2/scene38.rpy:350
translate russian v2s38_dbbbc59f:

    # li "Oh, I know exactly what you mean."
    li "Ох, я точно знаю, о чём ты."

# game/v2/scene38.rpy:355
translate russian v2s38_b61a943d:

    # aut "Campaigning is the hardest though. Regular work is hard, but I would hate to have to campaign again!"
    aut "Агитация - это самое сложное. Повседневная работа тоже не простая, но мне бы не хотелось снова участвовать в предвыборной кампании!"

# game/v2/scene38.rpy:360
translate russian v2s38_a7516311:

    # au "All this talk about hard work. Imagine what'll be like when we leave college and go out into the adult world... Ugh."
    au "Все эти разговоры о тяжёлой работе. Представьте, что будет, когда мы закончим колледж и выйдем во взрослый мир... Брр."

# game/v2/scene38.rpy:365
translate russian v2s38_52d0f97f:

    # u "Yeah, no thanks... That's a scary thought."
    u "Нет уж, спасибо... Об этом страшно думать."

# game/v2/scene38.rpy:370
translate russian v2s38_f5f36876:

    # se "You're telling me."
    se "Можешь не рассказывать."

# game/v2/scene38.rpy:375
translate russian v2s38_c6c1e946:

    # se "I wonder what careers we'll all end up having."
    se "Интересно, какая карьера у нас у всех в конечном итоге сложится."

# game/v2/scene38.rpy:384
translate russian v2s38_57e4bb51:

    # u "I think everyone here will be successful whatever we choose to do."
    u "Думаю, что все будут успешны, что бы мы ни выбрали."

# game/v2/scene38.rpy:389
translate russian v2s38_b205643e:

    # u "I mean, just look at us tonight. Not everyone's a VIP at college, and we're taking a limo to an all-inclusive club."
    u "Посмотри на нас. Не все в колледже ВИП, и это мы едем на лимузине в инклюзивный клуб."

# game/v2/scene38.rpy:394
translate russian v2s38_dd2c22b0:

    # li "Hell yeah! Cheers to that!"
    li "Чёрт, да! Выпьем за это!"

# game/v2/scene38.rpy:404
translate russian v2s38_2a92376f:

    # u "We'd be lucky to get any career at all... *Scoffs*"
    u "Нам повезёт, если мы вообще сделаем какую-нибудь карьеру... *Усмехается*"

# game/v2/scene38.rpy:409
translate russian v2s38_b09487e4:

    # aut "Hey! I'm smart, okay?"
    aut "Эй! Я умная, ясно?"

# game/v2/scene38.rpy:414
translate russian v2s38_8f43b029:

    # se "Yeah, Autumn will be fine... But the rest of us, well... He might be right."
    se "Да, с Отми всё будет в порядке... А вот с остальными, ну... Он может оказаться прав."

# game/v2/scene38.rpy:419
translate russian v2s38_b7370b26:

    # au "No way. I'm gonna be so successful that the rest of my family will be on their knees begging for money."
    au "Нифига подобного. Я буду настолько успешной, что остальные члены моей семьи будут на коленях просить денег."

# game/v2/scene38.rpy:424
translate russian v2s38_346236ea:

    # u "(Sheesh...)"
    u "(О, Господи...)"

# game/v2/scene38.rpy:430
translate russian v2s38_6b60cc98:

    # u "Even Uncle Ricky?"
    u "Даже дядя Рикки?"

# game/v2/scene38.rpy:435
translate russian v2s38_3568ee33:

    # au "Nah. I'm going to keep Uncle Ricky in as much cheese as he wants."
    au "Не-а. Я буду держать дядю Рикки в сыре столько, сколько он захочет."

# game/v2/scene38.rpy:440
translate russian v2s38_27d7c84e:

    # se "What the hell is an Uncle Ricky...? You're going to put him in cheese?"
    se "Что за дядя Рикки?... Зачем ты собираешься поместить его в сыр?"

# game/v2/scene38.rpy:450
translate russian v2s38_fcd9aa06:

    # aut "So, how's your campaign going, Lindsey?"
    aut "Как продвигается твоя кампания, Линдси?"

# game/v2/scene38.rpy:455
translate russian v2s38_ccdb6fca:

    # li "Great so far! Thanks for asking, Autumn."
    li "Пока отлично! Спасибо, что поинтересовалась, Отэм."

# game/v2/scene38.rpy:460
translate russian v2s38_c5761308:

    # au "You seem to be working really hard and people are definitely taking notice."
    au "Кажется, ты очень много работаешь, и люди точно обращают на это внимание."

# game/v2/scene38.rpy:465
translate russian v2s38_7a761331:

    # li "Aw, thanks for saying that. I really have been putting everything I have into this."
    li "Спасибо за такие слова. Я действительно вложила в это всё, что у меня было."

# game/v2/scene38.rpy:470
translate russian v2s38_a5b8ce69:

    # li "And [name] has been an amazing help too."
    li "И [name] тоже оказал неоценимую помощь."

# game/v2/scene38.rpy:475
translate russian v2s38_b6e182ba:

    # u "Of course, I'm happy to help. It's been kinda fun actually."
    u "Без проблем, я рад помочь. На самом деле это было довольно весело."

# game/v2/scene38.rpy:480
translate russian v2s38_d9533efd:

    # aut "You can work so hard that there's never any time for anything else, though."
    aut "Если будешь так много работать, то времени на остальное просто не останется."

# game/v2/scene38.rpy:485
translate russian v2s38_0e8d2a31:

    # li "Yeah, I haven't even thought about anything else."
    li "Да, я ни о чём другом даже и не думаю."

# game/v2/scene38.rpy:490
translate russian v2s38_79f655b9:

    # au "And no time for relationships, I bet."
    au "И времени на отношения, наверняка, нет."

# game/v2/scene38.rpy:495
translate russian v2s38_d51316c1:

    # li "Well... No, no sex."
    li "Ну... Да, никакого секса."

# game/v2/scene38.rpy:500
translate russian v2s38_dadfc6bd:

    # au "That's got to be frustrating."
    au "Это должно быть неприятно."

# game/v2/scene38.rpy:506
translate russian v2s38_2e8114f7:

    # gr "Oh, Lindsey's still a virgin anyway."
    gr "Да Линдси всё равно всё ещё девственница."

# game/v2/scene38.rpy:511
translate russian v2s38_9e0b908a:

    # li "Shut up, Grayson. I am not."
    li "Заткнись, Грейсон. Я не девственница."

# game/v2/scene38.rpy:516
translate russian v2s38_7ec7e618:

    # gr "*Snorts*"
    gr "*Фыркает*"

# game/v2/scene38.rpy:521
translate russian v2s38_5c42410c:

    # gr "I bet someone here still is though."
    gr "Бьюсь об заклад, кто-то здесь всё ещё ни разу не трахался."

# game/v2/scene38.rpy:526
translate russian v2s38_60c097f1:

    # gr "Definitely not Aubrey. Not according to everyone on campus!"
    gr "Точно не Обри. По словам всех в кампусе!"

# game/v2/scene38.rpy:531
translate russian v2s38_c13186a9:

    # u "Dude."
    u "Чувак."

# game/v2/scene38.rpy:536
translate russian v2s38_9383cf84:

    # au "Why the fuck did we invite King Ape? One day you're gonna choke on your own bullshit. I hope I'm around to watch."
    au "Какого хрена мы пригласили Короля Приматов? Однажды ты подавишься своим дерьмом. Надеюсь, я буду рядом, чтобы увидеть это."

# game/v2/scene38.rpy:541
translate russian v2s38_cb0c1ffa:

    # gr "Now, Autumn's the mystery..."
    gr "А вот Отэм - загадка..."

# game/v2/scene38.rpy:546
translate russian v2s38_9fe7decb:

    # gr "I've never seen you with anyone. No guys. Not even a girl."
    gr "Я никогда тебя ни с кем не видел. Ни с парнями. Ни даже с девушками."

# game/v2/scene38.rpy:551
translate russian v2s38_5ad0364a:

    # aut "I don't know if I should take it as a compliment or be creeped out that you care, Grayson."
    aut "Не знаю, стоит ли воспринимать это как комплимент или что тебе не всё равно, Грейсон."

# game/v2/scene38.rpy:553
translate russian v2s38_1acab090:

    # aut "Not that it's any of your business, but..."
    aut "Это не твоё дело, но..."

# game/v2/scene38.rpy:558
translate russian v2s38_e10d3490:

    # aut "I'm asexual."
    aut "Я асексуальна."

# game/v2/scene38.rpy:563
translate russian v2s38_4916d4f5:

    # gr "A... What-now?"
    gr "Асек... Это чё за фигня?"

# game/v2/scene38.rpy:569
translate russian v2s38_56f9a776:

    # se "Is it possible to live without sex?"
    se "Возможно ли прожить без секса?"

# game/v2/scene38.rpy:574
translate russian v2s38_9a03643b:

    # aut "Well, I might know a thing or two about that."
    aut "Ну, возможно, я знаю кое-что об этом."

# game/v2/scene38.rpy:579
translate russian v2s38_88cf162f:

    # se "Really, Autumn? I'm genuinely surprised, I could set you up if you're ever interested, you're a babe."
    se "Серьёзно, Отэм? Я искренне удивлён! Я могу устроить тебе встречу, если ты когда-нибудь заинтересуешься, ты же красотка."

# game/v2/scene38.rpy:584
translate russian v2s38_9d812996:

    # aut "Oh, no, that's not the issue."
    aut "О, нет, дело не в этом."

# game/v2/scene38.rpy:586
translate russian v2s38_7ef725e4:

    # aut "I haven't made it public knowledge or anything like that, but I don't mind telling you guys about it..."
    aut "Я не делала это достоянием общественности или что-то в этом роде, но я не против рассказать вам, ребята..."

# game/v2/scene38.rpy:591
translate russian v2s38_e10d3490_1:

    # aut "I'm asexual."
    aut "Я асексуальна."

# game/v2/scene38.rpy:596
translate russian v2s38_27acb584:

    # au "Wait. What does that mean exactly?"
    au "Погоди. А что это вообще значит?"

# game/v2/scene38.rpy:601
translate russian v2s38_2c8c7761:

    # aut "It basically means I don't desire sex. I'm not controlled by those... uh, primal urges you all have."
    aut "По сути это значит, что я не хочу секса. Меня не контролируют эти... Эмм... Первобытные желания, которые есть у всех вас."

# game/v2/scene38.rpy:606
translate russian v2s38_9debc918:

    # u "Makes it sound like a superpower."
    u "Это прямо как суперспособность."

# game/v2/scene38.rpy:619
translate russian v2s38_cdeac034:

    # li "So, you've never tried it?"
    li "То есть ты даже никогда не пробовала?"

# game/v2/scene38.rpy:624
translate russian v2s38_e32c471e:

    # aut "Nope. I've fooled around a little, but I haven't ever felt like I wanted to go the whole way."
    aut "Неа. Я немного дурачилась, но никогда не чувствовала, что хочу пройти весь путь до конца."

# game/v2/scene38.rpy:629
translate russian v2s38_8963aed7:

    # au "What? You're missing out. Sex is amazing!"
    au "Что? Ты столько потеряла. Секс - это потрясающе!"

# game/v2/scene38.rpy:634
translate russian v2s38_3ce52ef8:

    # aut "I'll just have to take your word for it."
    aut "Мне придётся поверить тебе на слово."

# game/v2/scene38.rpy:640
translate russian v2s38_64159df8:

    # gr "Just get laid tonight and get it over with! You don't want that hanging over you forever..."
    gr "Просто потрахайся сегодня и всего делов! Ты же не хочешь, чтобы это давило на тебя вечно..."

# game/v2/scene38.rpy:642
translate russian v2s38_5779b558:

    # gr "Who the hell wants to stay a virgin all their life?"
    gr "Кто, чёрт возьми, хочет оставаться девственником всю свою жизнь?"

# game/v2/scene38.rpy:647
translate russian v2s38_595159d2:

    # li "Grayson chimes in with with his usual tact and charm."
    li "Грейсон вступает в разговор со своим обычным тактом и обаянием."

# game/v2/scene38.rpy:652
translate russian v2s38_d62a7961:

    # gr "Um-"
    gr "Эмм..."

# game/v2/scene38.rpy:657
translate russian v2s38_92e2d549:

    # au "Grayson, just keep quiet. Adults are talking."
    au "Грейсон, просто помолчи. Взрослые разговаривают."

# game/v2/scene38.rpy:662
translate russian v2s38_9645a22b:

    # gr "*Groans* Whatever."
    gr "*Ворчит* Неважно."

# game/v2/scene38.rpy:668
translate russian v2s38_9a6c1599:

    # se "Have you ever been in a relationship?"
    se "У тебя когда-нибудь были отношения?"

# game/v2/scene38.rpy:673
translate russian v2s38_fc5f00a8:

    # aut "Not really, no."
    aut "На самом деле, нет."

# game/v2/scene38.rpy:678
translate russian v2s38_d8c3b8fb:

    # se "Man, that must be difficult."
    se "Блин, это, должно быть, тяжело."

# game/v2/scene38.rpy:683
translate russian v2s38_a2405ae6:

    # aut "I dunno, it's difficult finding people who get it."
    aut "Не знаю, трудно найти людей, которые это поймут."

# game/v2/scene38.rpy:688
translate russian v2s38_e5660898:

    # au "Do you think you'll ever want to have sex with someone?"
    au "Как думаешь, ты когда-нибудь захочешь заняться с кем-нибудь сексом?"

# game/v2/scene38.rpy:693
translate russian v2s38_92ca1cf7:

    # aut "Maybe, I think it'd have to be with someone I trust. I'm just not running out the door for it."
    aut "Может быть. Но это должно быть с кем-то, кому я доверяю. И я не собираюсь выбегать за дверь из-за этого."

# game/v2/scene38.rpy:698
translate russian v2s38_59745ecd:

    # au "You should probably try it to see if you like it, though."
    au "Наверное, стоит попробовать, может, тебе понравится."

# game/v2/scene38.rpy:703
translate russian v2s38_705479fd:

    # aut "Uh, yeah. Maybe..."
    aut "Да. Может быть..."

# game/v2/scene38.rpy:708
translate russian v2s38_fad8fd0a:

    # u "(I think she might be finished with this convo...)"
    u "(Похоже, это конец обсуждения...)"

# game/v2/scene38.rpy:715
translate russian v2s38_ce0a0730:

    # u "Okay, I think we're done interrogating for now."
    u "Ладно, думаю, на данный момент мы закончили допрос."

# game/v2/scene38.rpy:720
translate russian v2s38_86d5f6b7:

    # u "It's probably not an easy thing to talk about in front of everyone."
    u "Наверное, нелегко говорить об этом перед всеми."

# game/v2/scene38.rpy:725
translate russian v2s38_701a8922:

    # au "Oh, yeah. Of course! Sorry, Autumn."
    au "Ох, да. Конечно! Прости, Отэм."

# game/v2/scene38.rpy:730
translate russian v2s38_61aafe18:

    # aut "No, it's fine... I brought it up anyway, didn't I? Feels nice to talk about it."
    aut "Всё нормально... Я же сама затронула эту тему. Я не против поделиться этим."

# game/v2/scene38.rpy:735
translate russian v2s38_65fa306b:

    # aut "Thanks, [name]."
    aut "Спасибо, [name]."

# game/v2/scene38.rpy:740
translate russian v2s38_1ca16874:

    # u "No problem."
    u "Не за что."

# game/v2/scene38.rpy:745
translate russian v2s38_10819c0e:

    # u "So, who would you try it out with? A guy or a girl?"
    u "А с кем бы ты попробовала? С парнем или девушкой?"

# game/v2/scene38.rpy:750
translate russian v2s38_2045ef95:

    # aut "Um..."
    aut "Эм..."

# game/v2/scene38.rpy:755
translate russian v2s38_328d4120:

    # au "Ooh, a girl might actually be better for your first time..."
    au "Для первого раза лучше выбрать девушку..."

# game/v2/scene38.rpy:761
translate russian v2s38_342fde72:

    # gr "What the fuck? How?! Don't tell her that!"
    gr "Какого хрена? С чего бы это!? Ерунду не неси!"

# game/v2/scene38.rpy:767
translate russian v2s38_dc6c8782:

    # se "Wait... Really?"
    se "Погоди... Серьёзно?"

# game/v2/scene38.rpy:772
translate russian v2s38_b3c6abb1:

    # li "I think that's enough questions for now."
    li "Думаю, на сегодня достаточно."

# game/v2/scene38.rpy:777
translate russian v2s38_f5da4fcd:

    # li "You don't have to keep talking about it if you don't want to."
    li "Если не хочешь, можешь не отвечать."

# game/v2/scene38.rpy:782
translate russian v2s38_0fc8f5ab:

    # aut "Thanks, Lindsey."
    aut "Спасибо, Линдси."

# game/v2/scene38.rpy:787
translate russian v2s38_e91c9228:

    # u "Oh, right. Sorry, Autumn."
    u "Твоя правда. Прости, Отэм."

# game/v2/scene38.rpy:792
translate russian v2s38_6921958a:

    # aut "Ah, no worries. I'm the one who brought it up anyway..."
    aut "Всё нормально. Я же сама подняла эту тему..."

# game/v2/scene38.rpy:798
translate russian v2s38_3a190daf:

    # gr "Well, for the record, I'd happily volunteer to help you out with that, Autumn."
    gr "Для справки, я с радостью помогу тебе с этим, Отэм."

# game/v2/scene38.rpy:808
translate russian v2s38_667d0bbe:

    # au "Wow. How very honorable of you, Grayson."
    au "Вот это да. Как благородно с твоей стороны, Грейсон."

# game/v2/scene38.rpy:813
translate russian v2s38_58556d9f:

    # gr "Hey, I'm just trying to be a more considerate guy, you know? *Laughs*"
    gr "Эй, я просто пытаюсь быть более чутким, понимаешь? *Смеётся*"

# game/v2/scene38.rpy:819
translate russian v2s38_6a62d8cb:

    # se "Whatever you decide, just know there'll be guys lining up to take you out on a first date."
    se "Что бы ты ни решила, просто знай, что парни выстроятся в очередь, чтобы пригласить тебя на первое свидание."

# game/v2/scene38.rpy:824
translate russian v2s38_9a217c34:

    # aut "Aww, that's sweet. Thanks, Sebastian."
    aut "Оу, это так мило. Спасибо, Себастьян."

# game/v2/scene38.rpy:829
translate russian v2s38_631a5403:

    # li "Oh, don't let his amazing personality fool you. Seb is one fart away from murdering all of us."
    li "Не дай его удивительной личности одурачить тебя. Себ в одном шаге от того, чтобы убить всех нас."

# game/v2/scene38.rpy:834
translate russian v2s38_bd3ea7ba:

    # se "Linds! Quit fucking saying that to so many people!"
    se "Линдс! Нафига ты выдала мою тайну стольким людям!"

# game/v2/scene38.rpy:839
translate russian v2s38_4ef9f72f:

    # u "Wait what? Is that actually true?"
    u "Погоди, что? Это правда, что ли?"

# game/v2/scene38.rpy:844
translate russian v2s38_50b6c55e:

    # li "His ex was my best friend and not only did I hear a countless number of stories, I also heard her throwing up... Twice!"
    li "Его бывшая была моей лучшей подругой, и я не только слышала бесчисленное количество историй, но и знаю, что его вырвало при ней.... Дважды!"

# game/v2/scene38.rpy:849
translate russian v2s38_684f60a5:

    # au "Ugh! Oh my god, Seb..."
    au "Фу! Боже, Себ..."

# game/v2/scene38.rpy:854
translate russian v2s38_7df7d2e0:

    # se "I'm lactose-intolerant! I made a mistake, okay?"
    se "У меня непереносимость лактозы! Я совершил ошибку, ясно?"

# game/v2/scene38.rpy:856
translate russian v2s38_fb36d5ce:

    # se "I'm not proud of what my body does."
    se "Я не могу приказывать своему организму."

# game/v2/scene38.rpy:861
translate russian v2s38_6084ff1e:

    # aut "Twice?"
    aut "Дважды?"

# game/v2/scene38.rpy:866
translate russian v2s38_332dd553:

    # li "More times than that!"
    li "И не раз!"

# game/v2/scene38.rpy:871
translate russian v2s38_a63e7e52:

    # u "This is too good. You finally have a flaw, Fabio."
    u "Это слишком хорошо. Наконец-то у тебя есть изъян, Фабио."

# game/v2/scene38.rpy:876
translate russian v2s38_b3a35387:

    # se "*Sighs*"
    se "*Вздох*"

# game/v2/scene38.rpy:882
translate russian v2s38_2e8dfc2c:

    # u "Don't worry, man. Your stinky secret is safe with me."
    u "Не волнуйся, чувак. Я сохраню твой вонючий секрет."

# game/v2/scene38.rpy:887
translate russian v2s38_a5cd7aa8:

    # li "Not with me!"
    li "А я нет!"

# game/v2/scene38.rpy:893
translate russian v2s38_8d61c34d:

    # au "You've gotta find someone like [name]. He'd make sure your first time was special."
    au "Ну, лично я голосую за [name]. Думаю, он - лучший выбор для первого раза."

# game/v2/scene38.rpy:895
translate russian v2s38_54449914:

    # au "The real boyfriend experience."
    au "Настоящий опыт с парнем."

# game/v2/scene38.rpy:900
translate russian v2s38_fe74c6f9:

    # u "(Pretty cheesy, Aubrey.)"
    u "(Довольно пошло, Обри.)"

# game/v2/scene38.rpy:903
translate russian v2s38_7070d39b:

    # au "Well, personally, I'd vote for [name]. I think he'd be a good choice for your first time."
    au "Ну, лично я голосую за [name]. Думаю, он - лучший выбор для первого раза."

# game/v2/scene38.rpy:909
translate russian v2s38_1ce572db:

    # li "Yeah... Uh, I would second that."
    li "Да... Поддерживаю."

# game/v2/scene38.rpy:914
translate russian v2s38_71ec6877:

    # u "Well, nice to know I have the female vote..."
    u "Круто, что за меня голосуют женщины..."

# game/v2/scene38.rpy:920
translate russian v2s38_4231ca99:

    # se "Damn, [name]. How do I get your reputation?"
    se "Чёрт, [name]. Откуда у тебя такая репутация?"

# game/v2/scene38.rpy:926
translate russian v2s38_b366775a:

    # gr "What the hell are you guys drinking?! [name]? Seriously?"
    gr "Блин, вы что, пьяные, чтоли?! [name]? Серьёзно?"

# game/v2/scene38.rpy:936
translate russian v2s38_6b604826:

    # gr "What the fuck...?"
    gr "Какого хрена...?"

# game/v2/scene38.rpy:941
translate russian v2s38_0783ef76:

    # aut "Ookay. I'll keep all of this in mind. Let's change the subject now. Please."
    aut "Оокей. Буду иметь в виду. Давайте уже сменим тему. Пожалуйста."

# game/v2/scene38.rpy:946
translate russian v2s38_a3408f37:

    # li "Don't worry, Autumn. It looks like we're here!"
    li "Не волнуйся, Отэм. Кажется, мы уже приехали!"

# game/v2/scene38.rpy:951
translate russian v2s38_38cce186:

    # aut "Oh, good!"
    aut "Отлично!"

# game/v2/scene38.rpy:957
translate russian v2s38_5b4b2830:

    # gr "It's about damn time!"
    gr "Самое время, чёрт возьми!"

# game/v2/scene38.rpy:964
translate russian v2s38_73d9b557:

    # se "Let's get our groove on, baby!"
    se "Давай оторвёмся, детка!"

translate russian strings:

    # game/v2/scene38.rpy:293
    old "Raise your hands"
    new "Поднять руки"

    # game/v2/scene38.rpy:293
    old "Don't raise them"
    new "Неподнимать их"

    # game/v2/scene38.rpy:380
    old "Be encouraging"
    new "Подбодрить"

    # game/v2/scene38.rpy:710
    old "Stop the questioning"
    new "Прекратить расспросы"

    # game/v2/scene38.rpy:710
    old "Ask a question"
    new "Задать вопрос"

# TODO: Translation updated at 2024-07-03 10:01

# game/v2/scene38.rpy:92
translate russian v2s38_0f328957:

    # seb "Hey, [name]!"
    seb "Привет, [name]!"

# game/v2/scene38.rpy:106
translate russian v2s38_ff62f47c:

    # seb "Are you kidding, bro? When Lindsey said we're going clubbing, for free, in a limo... You being here is just a cherry on top!"
    seb "Ты шутишь, бро? Когда Линдси сказала, что мы собираемся в клуб, на халяву, в лимузине... То, что ты здесь, - это просто вишенка на торте!"

# game/v2/scene38.rpy:108
translate russian v2s38_e1423ff7:

    # seb "Yeah, you too. It's not often that we get to hang out without Chris and Grayson pushing us to throw down."
    seb "Да, и я. Не часто нам удаётся потусоваться без того, чтобы Крис и Грейсон не подталкивали нас к драке."

# game/v2/scene38.rpy:345
translate russian v2s38_a75861ab:

    # seb "I've been working so hard on things with the Wolves lately. This night out is seriously needed."
    seb "В последнее время я задолбался разгребать дела в братстве. Этот ночной выезд действительно необходим."

# game/v2/scene38.rpy:370
translate russian v2s38_9630b383:

    # seb "You're telling me."
    seb "Можешь не рассказывать."

# game/v2/scene38.rpy:375
translate russian v2s38_890bf8cc:

    # seb "I wonder what careers we'll all end up having."
    seb "Интересно, какая карьера у нас у всех в конечном итоге сложится."

# game/v2/scene38.rpy:414
translate russian v2s38_be04fd53:

    # seb "Yeah, Autumn will be fine... But the rest of us, well... He might be right."
    seb "Да, с Отми всё будет в порядке... А вот с остальными, ну... Он может оказаться прав."

# game/v2/scene38.rpy:440
translate russian v2s38_d215e997:

    # seb "What the hell is an Uncle Ricky...? You're going to put him in cheese?"
    seb "Что за дядя Рикки?... Зачем ты собираешься поместить его в сыр?"

# game/v2/scene38.rpy:569
translate russian v2s38_7c4cb9f0:

    # seb "Is it possible to live without sex?"
    seb "Возможно ли прожить без секса?"

# game/v2/scene38.rpy:579
translate russian v2s38_188c1cae:

    # seb "Really, Autumn? I'm genuinely surprised, I could set you up if you're ever interested, you're a babe."
    seb "Серьёзно, Отэм? Я искренне удивлён! Я могу устроить тебе встречу, если ты когда-нибудь заинтересуешься, ты же красотка."

# game/v2/scene38.rpy:668
translate russian v2s38_1dd1543d:

    # seb "Have you ever been in a relationship?"
    seb "У тебя когда-нибудь были отношения?"

# game/v2/scene38.rpy:678
translate russian v2s38_a23bc8d7:

    # seb "Man, that must be difficult."
    seb "Блин, это, должно быть, тяжело."

# game/v2/scene38.rpy:767
translate russian v2s38_3b6f8467:

    # seb "Wait... Really?"
    seb "Погоди... Серьёзно?"

# game/v2/scene38.rpy:819
translate russian v2s38_c7aa51a1:

    # seb "Whatever you decide, just know there'll be guys lining up to take you out on a first date."
    seb "Что бы ты ни решила, просто знай, что парни выстроятся в очередь, чтобы пригласить тебя на первое свидание."

# game/v2/scene38.rpy:834
translate russian v2s38_1e4ca85e:

    # seb "Linds! Quit fucking saying that to so many people!"
    seb "Линдс! Нафига ты выдала мою тайну стольким людям!"

# game/v2/scene38.rpy:854
translate russian v2s38_52ff3b3d:

    # seb "I'm lactose-intolerant! I made a mistake, okay?"
    seb "У меня непереносимость лактозы! Я совершил ошибку, ясно?"

# game/v2/scene38.rpy:856
translate russian v2s38_9cd86da2:

    # seb "I'm not proud of what my body does."
    seb "Я не могу приказывать своему организму."

# game/v2/scene38.rpy:876
translate russian v2s38_65e5b50d:

    # seb "*Sighs*"
    seb "*Вздох*"

# game/v2/scene38.rpy:920
translate russian v2s38_e7b5db4f:

    # seb "Damn, [name]. How do I get your reputation?"
    seb "Чёрт, [name]. Откуда у тебя такая репутация?"

# game/v2/scene38.rpy:964
translate russian v2s38_bb776748:

    # seb "Let's get our groove on, baby!"
    seb "Давай оторвёмся, детка!"

