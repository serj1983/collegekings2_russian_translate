# TODO: Translation updated at 2022-05-11 23:19

# game/v2/scene2.rpy:23
translate russian v2s2_61760787:

    # u "What the fuck?! Hey!"
    u "Что за нахуй?! Эй!"

# game/v2/scene2.rpy:28
translate russian v2s2_289dfb1f:

    # u "Who the fu-"
    u "Какого ху-"

# game/v2/scene2.rpy:33
translate russian v2s2_709505dc:

    # ca "Shut up, pledge!"
    ca "Заткнись, новичок!"

# game/v2/scene2.rpy:38
translate russian v2s2_c39754da:

    # u "Cameron?!"
    u "Кэмерон?!"

# game/v2/scene2.rpy:40
translate russian v2s2_a582259a:

    # u "Pledge?! We're back to this shit now? What the hell are you doing?!"
    u "Новичок?! Мы вернулись к этому дерьму? Какого чёрта ты делаешь?!"

# game/v2/scene2.rpy:47
translate russian v2s2_4d1e99b7:

    # ca "I'll be the one asking the questions!"
    ca "Вопросы буду задавать я!"

# game/v2/scene2.rpy:52
translate russian v2s2_896fe6e7:

    # ca "What the fuck were you doing with my sister?"
    ca "Какого хуя ты делал с моей сестрой?"

# game/v2/scene2.rpy:62
translate russian v2s2_bd8c0dde:

    # u "T-talking! W-we were just talking!"
    u "Р-разговаривали! М-мы просто разговаривали!"

# game/v2/scene2.rpy:67
translate russian v2s2_47140b6e:

    # ca "And drinking! I know she drinks when I'm not around."
    ca "И пили! Я знаю, что она пьёт, когда меня нет рядом."

# game/v2/scene2.rpy:73
translate russian v2s2_b8a537e6:

    # u "None of your business, Cameron."
    u "Это не твой дело, Кэмерон."

# game/v2/scene2.rpy:75
translate russian v2s2_6b1933e7:

    # u "So lay off."
    u "Так что отвинь."

# game/v2/scene2.rpy:80
translate russian v2s2_0b1353e9:

    # ca "Were you drinking?! I know she drinks when I'm not around."
    ca "Ты тоже пил?! Я знаю, что она пьёт, когда меня нет рядом."

# game/v2/scene2.rpy:86
translate russian v2s2_03c3f13c:

    # u "I heard her singing, and came down here to find her drinking, yes..."
    u "Я слышал, как она пела, потом спустился, и нашёл ей пьющей, так что да..."

# game/v2/scene2.rpy:88
translate russian v2s2_76228df5:

    # u "But we talked about how she can get help to quit."
    u "И мы беседовали о том, что она может получить помощь, чтобы бросить этим заниматься."

# game/v2/scene2.rpy:90
translate russian v2s2_6743f50d:

    # u "I'm trying to help her, Cameron. I actually care about her, okay?!"
    u "Я пытаюсь помочь ей, Кэмерон. Я правда забочусь о ней, ясно?!"

# game/v2/scene2.rpy:95
translate russian v2s2_00fb4568:

    # ca "..."
    ca "..."

# game/v2/scene2.rpy:100
translate russian v2s2_31c06cea:

    # u "Cameron?"
    u "Кэмерон?"

# game/v2/scene2.rpy:105
translate russian v2s2_5b9acb2b:

    # ca "You better be telling the truth."
    ca "Тебе лучше говорить правду."

# game/v2/scene2.rpy:110
translate russian v2s2_b3357440:

    # u "I am, man. Look at me."
    u "Это правда, чувак. Посмотри на меня."

# game/v2/scene2.rpy:120
translate russian v2s2_a3b28fa4:

    # ca "So, how are you helping her?"
    ca "Так, как ты поможешь ей?"

# game/v2/scene2.rpy:125
translate russian v2s2_c3834810:

    # u "I told her about some people who are trained to help with this kind of stuff, to get her off the drinks and drugs forever."
    u "Я рассказал ей о людях, которые обучены помогать в подобных вещах, чтобы навсегда избавить её от алкоголя и наркотиков."

# game/v2/scene2.rpy:130
translate russian v2s2_4baac5ac:

    # ca "*Scoffs* You really think that's possible?"
    ca "*С издёвкой* Ты действительно думаешь, что это возможно?"

# game/v2/scene2.rpy:135
translate russian v2s2_8f34140a:

    # u "*Sighs* Who knows, Cam. All we can do is try our best."
    u "*Вздох* Кто знает, Кэм. Всё, что мы можем сделать, это стараться изо всех сил."

# game/v2/scene2.rpy:137
translate russian v2s2_5337f114:

    # u "She needs more positivity in her life. The moment she feels like shit, she goes right back to it."
    u "Ей нужно больше позитива в жизни. В тот момент, когда она чувствует себя дерьмово, она сразу же возвращается к этому."

# game/v2/scene2.rpy:142
translate russian v2s2_d3082d9d:

    # ca "...Yeah."
    ca "...Ага."

# game/v2/scene2.rpy:144
translate russian v2s2_94eb8a8d:

    # ca "You heard her singing, huh? Ha..."
    ca "Ты слышал, как она поет, да? Ха..."

# game/v2/scene2.rpy:149
translate russian v2s2_48b26ed9:

    # u "Yeah. She seems to enjoy it."
    u "Да. Кажется, ей это нравится."

# game/v2/scene2.rpy:154
translate russian v2s2_14b2ea80:

    # ca "She always has. Even if she just sounds like a dying whale..."
    ca "Она всегда так делала. Даже если её голос звучит как умирающий кит..."

# game/v2/scene2.rpy:159
translate russian v2s2_212f59aa:

    # u "So that's something you can encourage her to do more of."
    u "Так что это то, что ты можешь поощрять её делать чаще."

# game/v2/scene2.rpy:160
translate russian v2s2_bb41b775:

    # u "It'll get her to focus on something that makes her happy instead of all the other emotional shit."
    u "Это заставит её сосредоточиться на этом, что сделает её счастливей, вместо всего остального эмоционального дерьма."

# game/v2/scene2.rpy:165
translate russian v2s2_934bcdb9:

    # ca "I guess you're right..."
    ca "Возможно, ты прав..."

# game/v2/scene2.rpy:170
translate russian v2s2_83a15af5:

    # u "(No shit.)"
    u "(Ни хрена.)"

# game/v2/scene2.rpy:175
translate russian v2s2_4d70316f:

    # ca "Sorry about the sack over your head..."
    ca "Прости за мешок на голове..."

# game/v2/scene2.rpy:184
translate russian v2s2_8c56361f:

    # u "*Chuckles* It was a bit extreme..."
    u "*Хихикает* Это было немного экстремально..."

# game/v2/scene2.rpy:189
translate russian v2s2_332cec2e:

    # ca "Yeah, but that's my style."
    ca "Ага, это мой стиль"

# game/v2/scene2.rpy:194
translate russian v2s2_5b00d35d:

    # u "Night, Cameron. Get some rest, man."
    u "Спокойной ночи, Кэмерон. Отдохни немного, чел."

# game/v2/scene2.rpy:202
translate russian v2s2_5b00d35d_1:

    # u "Night, Cameron. Get some rest, man."
    u "Спокойной ночи, Кэмерон. Отдохни немного, чел."

# game/v2/scene2.rpy:207
translate russian v2s2_7bd60257:

    # ca "Yeah, catch you later, [name]."
    ca "Да, увидимся позже, [name]."

# game/v2/scene2.rpy:213
translate russian v2s2_8c66c3f1:

    # u "I'm just letting her be who she is!"
    u "Я просто позволяю ей быть такой, какая она есть!"

# game/v2/scene2.rpy:215
translate russian v2s2_cc226844:

    # u "Do you really think you can stop her from doing what she wants all the time?"
    u "Ты правда думаешь, что можешь помешать ей делать то, что она хочет?"

# game/v2/scene2.rpy:220
translate russian v2s2_293bc574:

    # ca "Who the fuck do you think you're talking to?! I've known her all my life!"
    ca "С кем, блядь, ты думаешь ты разговариваешь?! Я знаю её всю свою жизнь!"

# game/v2/scene2.rpy:222
translate russian v2s2_4cad5c95:

    # ca "That's not who she is, she's-"
    ca "Она не такая, она..."

# game/v2/scene2.rpy:227
translate russian v2s2_a43716d5:

    # u "So, what are you going to do about it? Huh?! You can't be her prison guard forever."
    u "И что ты собираешься с этим делать? А?! Ты не можешь вечно быть её тюремным надзирателем."

# game/v2/scene2.rpy:232
translate russian v2s2_a4d92cfb:

    # ca "Keep talking to me like that, [name], and I'll knock your teeth out."
    ca "Если продолжишь со мной в таком тоне, [name], я выбью тебе зубы."

# game/v2/scene2.rpy:237
translate russian v2s2_5a74ae4f:

    # u "I accept her for who she is, Cameron. She's not a fucking kid anymore."
    u "Я принимаю её такой, какая она есть, Кэмерон. Она уже не ребёнок."

# game/v2/scene2.rpy:239
translate russian v2s2_6b2f2ecc:

    # u "You need to let her live her life the way she wants to."
    u "Ты должен позволить ей жить так, как она хочет."

# game/v2/scene2.rpy:244
translate russian v2s2_be1a9c3d:

    # ca "Stop... talking..."
    ca "Прекрати... говорить об этом..."

# game/v2/scene2.rpy:249
translate russian v2s2_b822b676:

    # u "One day, she may decide to live her life a different way, but that will be her choice. Not yours. Not mine. Hers."
    u "Однажды она может решить прожить свою жизнь по-другому, но это будет её выбор. Не твой. Не мой. Её."

# game/v2/scene2.rpy:254
translate russian v2s2_a06d5afa:

    # ca "Stay the fuck out of it, [name]!"
    ca "Блядь, не лезь в это, [name]!"

# game/v2/scene2.rpy:259
translate russian v2s2_49cb5c8f:

    # ca "If you're not going to help me stop her, then stay the hell away from her. Understand?"
    ca "Если ты не хочешь помочь мне остановить её, то держись от неё подальше. Понял?"

# game/v2/scene2.rpy:264
translate russian v2s2_9b301ce4:

    # u "*Choking* I- I-"
    u "*Удушье* Я... Я..."

# game/v2/scene2.rpy:269
translate russian v2s2_46c21650:

    # u "*Coughing*"
    u "*Кашель*"

# game/v2/scene2.rpy:284
translate russian v2s2_c9ad0bfd:

    # u "(Fuck!) *Panting*"
    u "(Блядь!) *Тяжело дыша*"

# game/v2/scene2.rpy:286
translate russian v2s2_5fc21c13:

    # u "(So, this is what living with a psychopath is like? For fuck's sake...)"
    u "(Так вот на что похожа жизнь с психопатом? Чёрт бы побрал...)"

# game/v2/scene2.rpy:289
translate russian v2s2_a03a4119:

    # u "(I wouldn't have made it out of that conversation alive if he knew what we actually did in there...)"
    u "(Я бы не вышел живым из этого разговора, если бы он узнал, чем мы там на самом деле занимались...)"

# game/v2/scene2.rpy:290
translate russian v2s2_4e7e732d:

    # u "(Damn, we're so lucky he didn't hear us.)"
    u "(Чёрт, нам так повезло, что он нас не услышал)."

# game/v2/scene2.rpy:326
translate russian v2s2_77517535:

    # u "*Sighs* (So much drama...)"
    u "*Вздох* (Столько переживаний...)"

# game/v2/scene2.rpy:328
translate russian v2s2_5221e4c7:

    # u "(I wonder how many people have had that sack treatment from Cameron.)"
    u "(Интересно, сколько людей подверглись такому обращению со стороны Кэмерона)."

# game/v2/scene2.rpy:330
translate russian v2s2_994ae2bd:

    # u "(He's one crazy motherfucking Ape.)"
    u "(Он просто чокнутый, мать его, Примат.)"

# game/v2/scene2.rpy:334
translate russian v2s2_5d77e627:

    # u "(Ah, shit... What now?)"
    u "(Ах, дерьмо... Что теперь?)"

# game/v2/scene2.rpy:362
translate russian v2s2_PhoneContinueChl_e4edb176:

    # u "(I should reply to Chloe.)"
    u "(Я должен ответить Хлое.)"

# game/v2/scene2.rpy:369
translate russian v2s2_PhoneContinueAut_cc6e6768:

    # u "(I should reply to Autumn.)"
    u "(Я должен ответить Отэм.)"

# game/v2/scene2.rpy:372
translate russian v2s2_PhoneContinueAut_c47a28cf:

    # u "(Almost forgot about that... It'll be interesting to spend some one-on-one time with Autumn.)"
    u "(Чуть не забыл об этом... Будет интересно провести некоторое время один на один с Отэм.)"

# game/v2/scene2.rpy:374
translate russian v2s2_PhoneContinueAut_41d31444:

    # u "(Should probably set an alarm...)"
    u "(Вероятно, следует установить будильник ...)"

# game/v2/scene2.rpy:376
translate russian v2s2_PhoneContinueAut_e073a9aa:

    # u "(And... done. Now, it's time for be-)"
    u "(И... готово. Теперь пришло время...)"

# game/v2/scene2.rpy:390
translate russian v2s2_PhoneContinueAut_83c30440:

    # u "(The hell?)"
    u "(Какого чёрта?)"

# game/v2/scene2.rpy:399
translate russian v2s2_PhoneContinueAut_6aaf1b96:

    # u "(\"Dress to impress your ghoulish empress...\") *Chuckles*"
    u "(\"Оденься, чтобы произвести впечатление на свою злую императрицу...\") *Хихикает*"

# game/v2/scene2.rpy:401
translate russian v2s2_PhoneContinueAut_fa544629:

    # u "(Guess I need to go gift shopping...)"
    u "(Думаю, мне нужно пройтись по магазинам подарков...)"

# game/v2/scene2.rpy:402
translate russian v2s2_PhoneContinueAut_f8727ffa:

    # u "(Maybe Autumn can give me ideas on what Lauren would like, or I can just get her some kind of gift card... She likes books, I think?)"
    u "(Может быть, Отэм подскажет мне, чего бы захотела Лорен, или я могу просто подарить ей какую-нибудь подарочную карту... Думаю, она любит книги?)"

# game/v2/scene2.rpy:412
translate russian v2s2_PhoneContinueAut_e753f65c:

    # u "(Finally...)"
    u "(Наконец-то...)"

# game/v2/scene2.rpy:430
translate russian v2s2_PhoneContinueAut_97bf1824:

    # u "*Groans* (No time for a snooze today. Need to get up.)"
    u "*Зевает* (Сегодня нет времени на валяние в постеле... Нужно вставать.)"

translate russian strings:

    # game/v2/scene2.rpy:57
    old "We were just talking"
    new "Мы только разговаривали"

    # game/v2/scene2.rpy:57
    old "None of your business"
    new "Не твоё дело"

    # game/v2/scene2.rpy:177
    old "Laugh it off"
    new "Смейся над этим"

    # game/v2/scene2.rpy:354
    old "Hey! Just reminding you that I'll be setting up the shelter tomorrow if you wanted to swing by? :)"
    new "Эй! Просто напоминаю тебе, что завтра я открою приют, заскочишь? :)"

    # game/v2/scene2.rpy:355
    old "Uhm, sure."
    new "Хм, конечно."

    # game/v2/scene2.rpy:356
    old "Of course! I'll always be there if there's puppies, haha."
    new "Конечно! Я всегда буду там, если будут щенки, хаха."

