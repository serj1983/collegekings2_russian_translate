# Перевёл: serj1983 (post.serj@gmail.com)
# TODO: Translation updated at 2022-05-11 23:19

# game/v2/scene3.rpy:22
translate russian v2s3_0057dc6e:

    # de "Oh look, and here's another member of the Ape clan. Come and sit down, [name]."
    de "О, посмотрите на это, ещё один член клана Приматов. Заходи, присаживайся [name]."

# game/v2/scene3.rpy:27
translate russian v2s3_f009213a:

    # u "(Oh, shit... Too late to turn around and run, I guess.)"
    u "(Ох, дерьмо... Слишком поздно для отхода, думаю)."

# game/v2/scene3.rpy:32
translate russian v2s3_30c457ba:

    # u "(I think I have an idea on what this is about.)"
    u "(Кажется, я догадываюсь, о чём будет речь)."

# game/v2/scene3.rpy:37
translate russian v2s3_01ede772:

    # de "I was just saying to Grayson here, that this fraternity is his responsibility."
    de "Как я уже сказала Грейсону, что братство - его ответственность."

# game/v2/scene3.rpy:42
translate russian v2s3_a139fa25:

    # de "What happens under this roof is your responsibility."
    de "За всё, что происходит под этой крышей, это ваша ответственность."

# game/v2/scene3.rpy:47
translate russian v2s3_64d46774:

    # de "And posting indecent photos of female students, is certainly something that this whole house will be punished for."
    de "А размещение непристойных фотографий студенток - это, безусловно, то, за что весь этот дом будет наказан."

# game/v2/scene3.rpy:52
translate russian v2s3_59e994f4:

    # ry "What?!"
    ry "Что?!"

# game/v2/scene3.rpy:57
translate russian v2s3_6c6dc092:

    # ca "Ha, nice one, Gray..."
    ca "Ха, молодец, Грей..."

# game/v2/scene3.rpy:62
translate russian v2s3_2aa15175:

    # de "If this type of behavior becomes a habit, Grayson, the Apes will be shut down completely. That's how serious you need to take this."
    de "Если такое поведение войдет в привычку, Грейсон, Приматы будут полностью уничтожены. Вот насколько серьезно вы должны отнестись к этому."

# game/v2/scene3.rpy:67
translate russian v2s3_96c687dd:

    # de "I'm making a note on your permanent record. All of you, in fact."
    de "Я сделаю пометку в твоём личном деле. Хотя, на самом деле, всем вам."

# game/v2/scene3.rpy:72
translate russian v2s3_d65262cf:

    # gr "I said I'm sorry, Mrs. Harrison. It was just a stupid prank-"
    gr "Я сказал, что мне очень жаль, миссис Харрисон. Это была просто глупая шутка-"

# game/v2/scene3.rpy:77
translate russian v2s3_9205442d:

    # de "No matter what your intention was, it's bullying."
    de "Независимо от того, каковы были ваши намерения, это травля."

# game/v2/scene3.rpy:82
translate russian v2s3_8b632c75:

    # gr "Again, I'm sorry. I- We obviously didn't think this through and sadly we made a bad decision as a group."
    gr "Ещё раз, мне жаль. Мы явно не продумали всё до конца и, к сожалению, приняли плохое решение всей группой."

# game/v2/scene3.rpy:87
translate russian v2s3_ab398d09:

    # u "(He's doing his best to sound sincere, and to spread the blame...)"
    u "(Он делает всё возможное, чтобы казаться искренним, и переложить вину...)"

# game/v2/scene3.rpy:92
translate russian v2s3_0282f0b7:

    # de "An apology is just a start."
    de "Извинения - это лишь начало."

# game/v2/scene3.rpy:94
translate russian v2s3_d730704b:

    # de "What I need from you all right now, is to delete the posts and the photos from your phones."
    de "Что мне сейчас нужно от вас всех, так это удалить сообщения и фотографии со своих телефонов."

# game/v2/scene3.rpy:104
translate russian v2s3_ff7329ab:

    # de "NOW!"
    de "СЕЙЧАС!"

# game/v2/scene3.rpy:109
translate russian v2s3_9c9e2f8b:

    # de "Oh, yes. I did hear that [name] was the only one smart enough to make the decision of not posting the photo."
    de "Ох, да. Я слышала, что [name] был единственным, кому хватило ума не размещать фотографию."

# game/v2/scene3.rpy:114
translate russian v2s3_507557c5:

    # de "At least there is one of you who has some integrity..."
    de "По крайней мере, среди вас есть хоть один честный человек..."

# game/v2/scene3.rpy:121
translate russian v2s3_8d0a036d:

    # ry "I knew we should've listened to you..."
    ry "Я знал, что мы должны были послушать тебя..."

# game/v2/scene3.rpy:127
translate russian v2s3_14f0f0cc:

    # ry "Way to be a team player..."
    ry "Как командный игрок..."

# game/v2/scene3.rpy:133
translate russian v2s3_ce22eb69:

    # de "Well done, [name]. You're excused from the punishment."
    de "Молодец, [name]. Ты освобождён от наказания."

# game/v2/scene3.rpy:138
translate russian v2s3_02b6fac4:

    # u "(Jeez, thanks for calling me out, Harrison!)"
    u "(Боже, спасибо, что позвала меня, Харрисон!)."

# game/v2/scene3.rpy:148
translate russian v2s3_b68c92f8:

    # ca "Done."
    ca "Сделано."

# game/v2/scene3.rpy:158
translate russian v2s3_06fe180d:

    # gr "Deleted."
    gr "Удалил."

# game/v2/scene3.rpy:168
translate russian v2s3_91c5ed35:

    # ry "Yeah, it's gone now."
    ry "Да, тоже."

# game/v2/scene3.rpy:173
translate russian v2s3_c049dcdd:

    # de "Very well. Make sure you message all the other Apes to do the same immediately."
    de "Очень хорошо. Обязательно сообщите всем остальным Приматам, чтобы они немедленно сделали то же самое."

# game/v2/scene3.rpy:178
translate russian v2s3_a6c0d3ba:

    # ry "Y-yes, ma'am."
    ry "Д-да, мэм."

# game/v2/scene3.rpy:183
translate russian v2s3_745579fa:

    # de "I'm going to be watching you all very closely from now on..."
    de "С этого момента я буду очень внимательно следить за всеми вами..."

# game/v2/scene3.rpy:188
translate russian v2s3_61097f44:

    # de "The Apes' reputation is also the reputation of SVC. We want it to have a good reputation. Remember that."
    de "Репутация Приматов - это и репутация колледжа. И мы хотим, чтобы у неё была хорошая репутация. Помните об этом."

# game/v2/scene3.rpy:198
translate russian v2s3_fcd8cef0:

    # gr "Our reputation is exactly where it needs to be! *Laughs*"
    gr "Наша репутация находится именно там, где ей нужно быть! *Смеётся*"

# game/v2/scene3.rpy:203
translate russian v2s3_5d952a09:

    # ry "That sucked."
    ry "Это было отстойно."

# game/v2/scene3.rpy:208
translate russian v2s3_7034309d:

    # gr "It doesn't matter. The posts have already done the damage. We fucking did it, boys!"
    gr "Это не имеет значения. Посты уже нанесли ущерб. Мы, блядь, сделали это, парни!"

# game/v2/scene3.rpy:213
translate russian v2s3_05d7d874:

    # ry "What about the note on our permanent record?"
    ry "А как насчет записи в нашем личном деле?"

# game/v2/scene3.rpy:218
translate russian v2s3_860c1f31:

    # gr "That doesn't mean anything, man. Come on!"
    gr "Это ничего не значит, чувак. Давай!"

# game/v2/scene3.rpy:223
translate russian v2s3_a2e8d2c6:

    # ca "Yeah, it's not even worth thinking about."
    ca "Да, об этом даже думать не стоит."

# game/v2/scene3.rpy:228
translate russian v2s3_35dfa768:

    # u "This is what we get for joining the Apes. *Chuckles*"
    u "Вот что получаешь за присоединение к Приматам. *Хихикает*"

# game/v2/scene3.rpy:233
translate russian v2s3_b1f55b05:

    # ry "You mean me? This is what I get for joining the Apes. You don't get shit."
    ry "Ты имеешь в виду меня? Вот что я получил, когда присоединился к Приматам. Ты ни хрена не понимаешь."

# game/v2/scene3.rpy:242
translate russian v2s3_d1303ba1:

    # u "Come on man, this isn't my fault."
    u "Да ладно, чувак, это не моя вина."

# game/v2/scene3.rpy:245
translate russian v2s3_77ea7120:

    # u "I tried to stop-"
    u "Я пытался остановить-"

# game/v2/scene3.rpy:250
translate russian v2s3_eebd296a:

    # ry "Maybe not, but it sure doesn't matter now, does it?"
    ry "Может нет, но сейчас это точно не имеет значения, не так ли?"

# game/v2/scene3.rpy:256
translate russian v2s3_c95051eb:

    # u "Ryan, look, I'm sorry."
    u "Райан, послушай, мне жаль."

# game/v2/scene3.rpy:259
translate russian v2s3_77ea7120_1:

    # u "I tried to stop-"
    u "Я пытался остановить..."

# game/v2/scene3.rpy:264
translate russian v2s3_4fc5a29e:

    # ry "You know what, [name]? It doesn't fucking matter."
    ry "Знаешь что, [name]? Это, блядь, не имеет значения."

# game/v2/scene3.rpy:269
translate russian v2s3_57ae9944:

    # u "*Sighs*"
    u "*Вздох*"

# game/v2/scene3.rpy:280
translate russian v2s3_81488685:

    # u "(I'll just go check on Autumn at the shelter...)"
    u "(Пойду проведаю Отэм в приюте...)"

translate russian strings:

    # game/v2/scene3.rpy:237
    old "It's not my fault"
    new "Это не моя вина"

# Перевёл: serj1983 (post.serj@gmail.com)
# Patch v0.9.1s
# TODO: Translation updated at 2022-06-08 16:11

# game/v2/scene3.rpy:27
translate russian v2s3_6153f748:

    # u "(Oh, shit... too late to turn around and run, I guess.)"
    u "(Ох, дерьмо... Слишком поздно для отхода, думаю.)"

# game/v2/scene3.rpy:37
translate russian v2s3_d426228d:

    # de "I was just saying to Grayson here that this fraternity is his responsibility."
    de "Как я уже сказала Грейсону, что братство - его ответственность."

# game/v2/scene3.rpy:47
translate russian v2s3_a78bbe81:

    # de "And posting indecent photos of female students is certainly something that this whole house will be punished for."
    de "И размещение непристойных фотографий студенток - это, безусловно, то, за что весь этот дом будет наказан."

# game/v2/scene3.rpy:82
translate russian v2s3_82a15b5f:

    # gr "Again, I'm sorry. I... we obviously didn't think this through and sadly we made a bad decision as a group."
    gr "Ещё раз, мне жаль. Мы явно не продумали всё до конца и, к сожалению, приняли плохое решение всей группой."

# game/v2/scene3.rpy:94
translate russian v2s3_c37b6a68:

    # de "What I need from you all, right now, is to delete the posts and the photos from your phones."
    de "Что мне сейчас нужно от вас всех, так это удалить сообщения и фотографии со своих телефонов."

# game/v2/scene3.rpy:198
translate russian v2s3_0b947256:

    # gr "Our reputation is exactly where it needs to be!"
    gr "Наша репутация находится именно там, где ей нужно быть!"

# game/v2/scene3.rpy:228
translate russian v2s3_15a36259:

    # u "This is what we get for joining the Apes."
    u "Вот что получаешь за присоединение к Приматам."

