# TODO: Translation updated at 2022-08-26 15:31

# game/v2/scene23.rpy:30
translate russian v2s23_977fb4da:

    # u "(Well, Halloween was a success. Thankfully, I'll never have to wear that costume ever again.)"
    u "(Ну, Хэллоуин удался. К счастью, мне больше не придётся надевать этот костюм)."

# game/v2/scene23.rpy:54
translate russian v2s23_7dabf636:

    # u "(Maybe I should take art next semester...)"
    u "(Может быть, мне стоит заняться искусством в следующем семестре...)"

# game/v2/scene23.rpy:68
translate russian v2s23_7dabf636_1:

    # u "(Maybe I should take art next semester...)"
    u "(Может быть, мне стоит заняться искусством в следующем семестре...)"

# game/v2/scene23.rpy:137
translate russian v2s23_aubrey_text_continue_f4277857:

    # u "(I should reply to Aubrey.)"
    u "(Я должен ответить Обри.)"

# game/v2/scene23.rpy:146
translate russian v2s23_aubrey_text_continue_aa2eb626:

    # u "(I wonder what the surprise is...)"
    u "(Интересно, что за сюрприз...)"

# game/v2/scene23.rpy:162
translate russian v2s23_aubrey_text_continue_d04615c9:

    # u "(Time to find out!)"
    u "(Пора узнать!)"

translate russian strings:

    # game/v2/scene23.rpy:42
    old "Draw a happy face"
    new "Нарисовать счастливое лицо"

    # game/v2/scene23.rpy:42
    old "Draw a dick"
    new "Нарисовать член"

# TODO: Translation updated at 2023-06-23 11:22

# game/v2/scene23.rpy:122
translate russian v2s23_f4277857:

    # u "(I should reply to Aubrey.)"
    u "(Я должен ответить Обри.)"

# game/v2/scene23.rpy:130
translate russian v2s23_aa2eb626:

    # u "(I wonder what the surprise is...)"
    u "(Интересно, что за сюрприз...)"

# game/v2/scene23.rpy:146
translate russian v2s23_d04615c9:

    # u "(Time to find out!)"
    u "(Пора узнать!)"

